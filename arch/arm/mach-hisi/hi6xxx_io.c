...
static struct map_desc hi6210_io_desc[] __initdata = {
	{
		.virtual = (unsigned long)GU_RESERVED_MEM_VIRT_ADDR,
		.pfn = __phys_to_pfn(GLOBAL_MEM_GU_RESERVED_ADDR),
		.length = GLOBAL_MEM_GU_RESERVED_SIZE,
		.type = MT_DEVICE,
	},
	{
		.virtual = (unsigned long)IPC_SHARE_MEM_VIRT_ADDR,
		.pfn = __phys_to_pfn(IPC_SHARE_MEM_ADDR),
		.length = IPC_SHARE_MEM_SIZE,
		.type = MT_DEVICE,
	},
	{
		.virtual = (unsigned long)GLOBAL_MEM_EXCP_BASE_ADDR_VIRT,
