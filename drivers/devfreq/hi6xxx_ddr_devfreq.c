...
static int boost_ddr_dfs_band = 0;
static int boost_ddr_dfs_last = 0;
...
ddr_devfreq_target(struct device *dev, unsigned long *freq, u32 flags)
{
    ...
    /*vote to MCU no matter whatever cur freq is*/
	(void)clk_set_rate(ddev->set, *freq);
	ddev->freq = *freq;
    ...
    if (inited == 0)
    {
        ...
        else
        {
            ...
            if (boost_ddrfreq_switch != 0x0)
            {
                ...
                int rc = input_register_handler(&ddrfreq_input_handler);
                if (rc)
...
#ifndef __aarch64__
    if(RET_OK != pwrctrl_is_func_on(PWC_SWITCH_DDFS))
    {
		pr_err("%s: ddr dfs switch is not on\n", __func__);
        return -ENODEV;
    }
#endif
	if (!np) {
...
{
	struct ddr_devfreq_device *ddev = platform_get_drvdata(pdev);
	struct devfreq_dev_profile *profile = ddev->devfreq->profile;
