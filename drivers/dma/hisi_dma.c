...
static void hisi_dma_pause_dma(struct hisi_dma_phy *phy, struct hisi_dma_dev *d, bool on)
{
	...
	if (on) {
		...
		for (timeout = 2000; timeout > 0; timeout--) {
			if (!readl_relaxed(d->base+ CH_STAT))
				break;
			udelay(1);
		}
...
static void hisi_dma_enable_dma(struct hisi_dma_dev *d, bool on)
{
	if (on) {
		...
		/* unmask irq */
		writel_relaxed(d->dma_used_chans, d ->base + INT_TC1_MASK);
		writel_relaxed(d->dma_used_chans, d ->base + INT_ERR1_MASK);
		writel_relaxed(d->dma_used_chans, d ->base + INT_ERR2_MASK);
...
void show_dma_reg(struct hisi_dma_chan *chan)
{
	struct hisi_dma_phy *d = chan->phy;
...
void show_dma_reg2(struct dma_chan *chan)
{
	struct hisi_dma_chan *c = to_hisi_chan(chan);
	...
	printk("STAT =%x\n", readl(c->phy->base + INT_STAT));
	printk("tc1 =%x\n", readl(c->phy->base + INT_TC1));
	printk("err1 =%x\n", readl(c->phy->base + INT_ERR1));
	printk("err2 =%x\n", readl(c->phy->base + INT_ERR2));

	printk("tc1_mask =%x\n", readl(c->phy->base + INT_TC1_MASK));
	printk("err1  mask=%x\n", readl(c->phy->base + INT_ERR1_MASK));
	printk("err2  mask =%x\n", readl(c->phy->base  + INT_ERR2_MASK));

}
EXPORT_SYMBOL(show_dma_reg2);
...
static int hisi_dma_probe(struct platform_device *op)
{
	...
	if(ret) {
        ...
        if(d->dma_share)
        {
                printk(KERN_ERR "here dma_share is true\n");
        }
        printk(KERN_INFO "dma_channels:0x%x dma_requests:0x%x dma_min_chan:0x%x dma_used_chans:0x%x\n",
