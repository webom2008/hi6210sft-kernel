...
#include <linux/smp.h>
#include <linux/hisi/hi6xxx-efuse.h>
...
static IFC_GEN_CALL2(MAILBOX_IFC_ACPU_TO_CCPU_SET_EFUSE_CHIPID, set_efuse_chipid_value,
              IFC_INVAR, unsigned char *, pu8Buf, 0,
              IFC_INCNT, unsigned int, uiLength, 0)

static IFC_GEN_CALL2(MAILBOX_IFC_ACPU_TO_CCPU_SET_EFUSE_AUTHKEY, set_efuse_authkey_value,
              IFC_INVAR, unsigned char *, pu8Buf, 0,
              IFC_INCNT, unsigned int, uiLength, 0)

static IFC_GEN_CALL2(MAILBOX_IFC_ACPU_TO_CCPU_GET_EFUSE_AUTHKEY, get_efuse_authkey_value,
              IFC_BIVAR, unsigned char *, pu8Buf, 0,
              IFC_INCNT, unsigned int, uiLength, 0)

static IFC_GEN_CALL1(MAILBOX_IFC_ACPU_TO_CCPU_SET_EFUSE_DEBUGMODE, set_efuse_securitydebug_value,
              IFC_INCNT, unsigned int , uiMode, 0)

static IFC_GEN_CALL2(MAILBOX_IFC_ACPU_TO_CCPU_GET_EFUSE_DEBUGMODE, get_efuse_securitydebug_value,
              IFC_BIVAR, unsigned int *, puiMode, 0,
              IFC_INCNT, unsigned int, puiModeLength, 0)

static IFC_GEN_CALL2(MAILBOX_IFC_ACPU_TO_CCPU_GET_EFUSE_DIEID, get_efuse_dieid_vlaue,
	          IFC_BIVAR, unsigned char *, pu8Buf, 0,
	          IFC_INCNT, unsigned int, uiLength, 0)
/*
...
static long efusec_ioctl(struct file *file, u_int cmd, u_long arg)
{
	...
	case HISI_EFUSE_READ_CHIPID:
		ret = get_efuse_chipid_value(efuse_buf, BYTE_4, 1000);
		if (ret < 0) {
		...
		/*send back to user*/
		if (copy_to_user(argp, efuse_buf, BYTE_4)){
			ret = -EFAULT;
			break;
		}
	...
	case HISI_EFUSE_READ_AUTHKEY:
		ret = get_efuse_authkey_value(efuse_buf, BYTE_4, 1000);
		if (ret < 0) {
	...
	case HISI_EFUSE_READ_DEBUGMODE:
		ret = get_efuse_securitydebug_value(&ulmode, BYTE_4, 1000);
		if (ret < 0) {
    ...
    case HISI_EFUSE_READ_DIEID:
        ret = get_efuse_dieid_vlaue(efuse_buf, BYTE_20, 1000);
        if (ret < 0) {
        ...
        for(i = 0; i < BYTE_20; i++)
            pr_info("line%d, get_efuse_dieid_vlaue[%d] = %2x.\n", __LINE__, i, efuse_buf[i]);

        /*send back to user*/
        if (copy_to_user(argp, efuse_buf, BYTE_20)){
            ret = -EFAULT;
            break;
        }
