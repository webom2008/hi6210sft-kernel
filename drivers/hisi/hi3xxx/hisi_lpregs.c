...
void combo_ipc_irq_show(unsigned int nsipc_state)
{
	...
	pr_info("SR:nsipc irq state:0x%x\n", nsipc_state);
	if (sysreg_base.nsipc_base == NULL) {
		pr_err("SR:nsipc base is NULL\n");
		return;
	}
...
void pm_status_show(void)
{
	...
	if (LPM3_IRQ_MAX > ap_irq) {
		pr_info("SR:AP WAKE IRQ(LPM3 NVIC): %d (%s)\n", ap_irq, lpm3_irq_name[ap_irq]);
	} else {
		pr_err("SR:AP WAKE IRQ(LPM3 NVIC): %d (no exist)\n", ap_irq);
	}
	if (IPC_COMBO_AP == ap_irq) {
		combo_ipc_irq_show(readl(sysreg_base.reserved_base + PM_AP_NSIPC_IRQ_OFFSET));
	}
}
...
void dbg_io_status_show(void)
{
	...
	for (i = 0; i < IO_LIST_LENGTH; i++) {
		...
		printk("gpio - %d  owner - %-10s\t", uregv, hisi_iocfg_lookups[i].owner);
