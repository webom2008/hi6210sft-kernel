...
static inline bool check_headset_pluged_in(struct hi6402_mbhc_platform_data *pdata)
{
	 ...
	 */
	return (0 != (hi6402_irq_read(pdata->p_irq, HI6402_REG_IRQ_0) & (1 << HI6402_HS_PLUGIN_BIT)));
}
...
void hi6402_plug_in_workfunc(struct work_struct *work)
{
...
static irqreturn_t hi6402_plugin_handler(int irq, void *data)
{
	...
	mask_irq(pdata->irq[HI6402_IRQ_PLUGIN], true);
...
static irqreturn_t hi6402_btndown_eco_handler(int irq, void *data)
{
	...
	mask_irq(pdata->irq[HI6402_IRQ_BTNDOWN_ECO], true);
	mask_irq(pdata->irq[HI6402_IRQ_BTNUP_ECO], false);
