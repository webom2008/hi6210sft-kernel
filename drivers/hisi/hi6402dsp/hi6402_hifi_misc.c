...
#define HI6402_FREQ_12P288M 0
#define HI6402_FREQ_32P576K 1

/*XXX: Some bugs will be fixed in CS version.*/
#define HI6402_VERSION_ES

/*XXX: change to 4 to enbale debug print*/
unsigned long hi6402_dsp_debug_level = 3;

static const unsigned int DSP_MSG_START_ADDR = HI6402_OCRAM1_BASE;
static const unsigned int DSP_MLIB_PARA_ADDR =
				HI6402_OCRAM1_BASE + 2 * MAX_MSG_SIZE;
...
{
	hi6402_hifi_write_reg(HI6402_GPIO0_DIR, 0x20);
	hi6402_hifi_write_reg(HI6402_GPIO0_IS , 0x0);
	hi6402_hifi_write_reg(HI6402_GPIO0_IBE, 0x0);
	hi6402_hifi_write_reg(HI6402_GPIO0_IEV, 0x20);
	hi6402_hifi_write_reg(HI6402_GPIO0_IE2, 0x20);
	hi6402_hifi_write_reg(HI6402_GPIO0_DATA_ch5, 0x0);
	hi6402_hifi_write_reg(HI6402_GPIO0_DATA_ch5, 0x20);
}
...
static irqreturn_t hi6402_msg_irq_handler(int irq, void *data)
{
	...
	if (hi6402_hifi_read_reg(HI6402_DSP_CMD_STATUS) & (0x1 << 0)) {
		HI6402_DSP_INFO("bit 0\n");
		hi6402_hifi_reg_clr_bit(HIFI_CMD_STAT, 0);
		priv.msg_send_done = 1;
		wake_up_interruptible_all(&priv.send_msg_wq);
	}

	if (hi6402_hifi_read_reg(HI6402_DSP_CMD_STATUS) & (0x1 << 1)) {
		HI6402_DSP_INFO("bit 1\n");
		hi6402_hifi_reg_clr_bit(HIFI_CMD_STAT, 1);
		priv.sync_msg_ret = 1;
		wake_up_interruptible_all(&priv.sync_msg_wq);
	}

	//clr irq
#ifdef HI6402_VERSION_ES
	if ((hi6402_hifi_read_reg(HI6402_DSP_CMD_STAT_VLD) & 0x1) == 0) {
		hi6402_hifi_reg_set_bit(HI6402_DSP_CMD_STAT_VLD, 0);
	}
	hi6402_hifi_reg_clr_bit(HI6402_DSP_CMD_STAT_VLD, 0);
#endif
	hi6402_hifi_reg_set_bit(HI6402_DSP_CMD_STAT_VLD_IRQ, 0);
...
static int hi6402_write_msg(const unsigned char *arg, const unsigned int len)
{
	...
	if (wait_event_interruptible_timeout(priv.send_msg_wq,
		...
		HI6402_DSP_ERROR("wait for send msg done timeout, ret=%d\n", ret);
		ret = BUSY;
	}
...
static int hi6402_async_write(const unsigned char *arg, const unsigned int len)
{
	int ret = OK;
	...
	/*TODO: hi6402_write need to check whether data had been changed via ssi.
	...
	do {
		...
		retry_times++;
		if (retry_times > 1) {
			HI6402_DSP_INFO("msg send timeout, retry %u\n", retry_times);
		}
	} while (ret == BUSY && retry_times < MSG_SEND_RETRIES);

	if (ret != OK) {
		HI6402_DSP_ERROR("msg send to hifi fail,ret is %d\n", ret);
		ret = ERROR;
	}
...
static int hi6402_sync_write(const unsigned char *arg, const unsigned int len)
{
	...
	ret = hi6402_async_write(arg, len);
	if (ret != OK) {
	...
	if (wait_event_interruptible_timeout(priv.sync_msg_wq,
		...
		HI6402_DSP_ERROR("wait for sync cmd done timeout, ret=%d\n", ret);
		ret = BUSY;
	}
...
static void hi6402_dsp_if_config(const char *arg, bool enable)
{
	unsigned int i = 0;
	...
	HI6402_DSP_INFO("clock_change_ifs: 0x%x\n", clock_change_ifs);
	hi6402_hifi_reg_write_bits(HI6402_DSP_I2S_DSPIF_CLK_EN,
				   enable?(~0):0,
				   clock_change_ifs/* mask */);

	for (i=0; i<DSP_IF_NUM; i++) {
		if(clock_change_ifs & (1<<i)) {
			hi6402_dsp_if_set_bypass(i, !enable);
		}
	}
...
int uart_config[][4] = {{0xBA,0xE8,0x22,0x14},/* hi6402 req :12.288M */
						{0xAB,0xAA,0x2A,0x4}};/* hi6402 req :32K */
static int hi6402_func_uart_div_switch(int hi6402_req)
{
	int i = 0;
	u32 ret = 0;
	/* set uart clk div(CFG nManager) */
	for(i = 0; i < 3; i++) {
		hi6402_hifi_write_reg(HI6402_DSP_UART_DIV_0 + i,uart_config[hi6402_req][i]);
		ret = hi6402_hifi_read_reg(HI6402_DSP_UART_DIV_0 + i);
		if(ret != uart_config[hi6402_req][i]) {
			HI6402_DSP_ERROR("set uart div fail\n");
			return -1;
		}
	}

	/* set dsp uart DHL(DSP nManager) */
	hi6402_hifi_reg_set_bit(0x2000400c,7);
	hi6402_hifi_write_reg(0x20004000,uart_config[hi6402_req][3]);
	hi6402_hifi_reg_clr_bit(0x2000400c,7);
...
static int hi6402_func_if_open(struct krn_param_io_buf *param)
{
	...
	if (dma_msg_stru->uwProcessId == MLIB_PATH_DATA_HOOK) {
		if(1 == priv.datahook_state) {
			HI6402_DSP_ERROR("datahook has been opened\n");
			goto end;
		}
		priv.datahook_state = 1;
		start_hifi();
		hi6402_dsp_if_config(param->buf_in, true);
		ret = hi6402_sync_write(param->buf_in, param->buf_size_in);
		if (ret != OK) {
			HI6402_DSP_ERROR("async_write ret=%d\n", ret);
			/* disable dsp if clk */
			hi6402_dsp_if_config(param->buf_in, false);
			goto end;
		}
	} else {
		if (dma_msg_stru->uwProcessId == MLIB_PATH_WAKEUP) {
			if(1 == priv.wakeup_state) {
				HI6402_DSP_ERROR("wakeup has been opened\n");
				goto end;
			}
			start_wakeup();
			ret = hi6402_func_uart_div_switch(HI6402_FREQ_32P576K);
			if (OK != ret)
				goto end;
		} else if(dma_msg_stru->uwProcessId == MLIB_PATH_SMARTPA) {
			if(1 == priv.smartpa_state) {
				HI6402_DSP_ERROR("smartpa has been opened\n");
				goto end;
			}
...
static int hi6402_func_if_close(struct krn_param_io_buf *param)
{
	...
	ret = hi6402_async_write(param->buf_in, param->buf_size_in);
	if (ret != OK) {
		HI6402_DSP_ERROR("async_write ret=%d\n", ret);
		goto end;
	}
	...
	if(dma_msg_stru->uwProcessId == MLIB_PATH_WAKEUP) {
		ret = hi6402_func_uart_div_switch(HI6402_FREQ_12P288M);
...
static int hi6402_func_para_set(struct krn_param_io_buf *param)
{
	...
	if (ret != OK) {
		HI6402_DSP_ERROR("async write failed\n");
		goto end;
	}
 ...
 *  dsp img download
 */
#define HI6402_HIFI_FW "hifi_6402/hifi_6402.bin"
					     ...
					     struct drv_hifi_image_sec *img_sec)
{
	...
	if ((img_sec->sn >= img_head->sections_num)
		...
		|| (img_sec->load_attib >= (unsigned char)DRV_HIFI_IMAGE_SEC_LOAD_BUTT))
	{
		HI6402_DSP_INFO("hifi: drv_hifi_check_sections ERROR.\n");
		return -1;
	}
...
static void hi6402_hifi_pwron_work(void)
{
	/* Soft reset HIFI */
	hi6402_hifi_reg_set_bit(HI6402_DSP_SW_RST_REQ, 1);

	/* Soft reset APB of power-off area */
	hi6402_hifi_reg_set_bit(HI6402_DSP_CLK_CTRL, 4);
	...
	/* Open HIFI clocks */
	hi6402_hifi_reg_set_bit(HI6402_DSP_SC_DSP_CTRL0, 0);
	...
	/* open DSPIF clocks */
	hi6402_hifi_reg_set_bit(HI6402_DSP_DAC_DP_CLK_EN_1, 4);

	/* Release HIFI reset */
	hi6402_hifi_reg_clr_bit(HI6402_DSP_SW_RST_REQ, 1);
	hi6402_hifi_reg_clr_bit(HI6402_DSP_CLK_CTRL, 4);

	/* enable axi */
	hi6402_hifi_write_reg(HI6402_DSP_RAM2AXI_CTRL, 0x01);

}

static void hi6402_hifi_pwron(void)
{
	mutex_lock(&priv.power_ref_mutex);
	if(0 == priv.power_ref)
		hi6402_hifi_pwron_work();
	priv.power_ref++;
	mutex_unlock(&priv.power_ref_mutex);
	return;
}

static void hi6402_hifi_pwroff_work(void)
{
	/* Close HIFI clock */
...
static void hi6402_hifi_pwroff(void)
{
	mutex_lock(&priv.power_ref_mutex);
	if(0 == priv.power_ref){
		mutex_unlock(&priv.power_ref_mutex);
	...
	priv.power_ref--;
	if(0 == priv.power_ref)
		hi6402_hifi_pwroff_work();
	mutex_unlock(&priv.power_ref_mutex);
	return;
}

static void hi6402_hifi_enable_pll_work(void)
{
	/* set pll */
	BUG_ON(priv.p_irq == NULL);
	hi6402_irq_pll_enable(priv.p_irq, true);

}

static void hi6402_hifi_enable_pll(void)
{
	mutex_lock(&priv.clk_mutex);
	if(0 == priv.clk_ref)
		hi6402_hifi_enable_pll_work();
	priv.clk_ref++;
	mutex_unlock(&priv.clk_mutex);
}

static void hi6402_hifi_disable_pll_work(void)
{
	/* set pll */
	BUG_ON(priv.p_irq == NULL);
	hi6402_irq_pll_enable(priv.p_irq, false);
}

static void hi6402_hifi_disable_pll(void)
{
	mutex_lock(&priv.clk_mutex);
	if(0 == priv.clk_ref){
		mutex_unlock(&priv.clk_mutex);
	...
	priv.clk_ref--;
	if(0 == priv.clk_ref)
		hi6402_hifi_disable_pll_work();
	mutex_unlock(&priv.clk_mutex);
}

static void hi6402_hifi_enable_mad_pll(void)
{
#if 0
	/* set pll */
	BUG_ON(priv.p_irq == NULL);
	hi6402_irq_mad_pll_enable(priv.p_irq, true);
#endif
}

static void hi6402_hifi_disable_mad_pll(void)
{
#if 0
	/* set pll */
	BUG_ON(priv.p_irq == NULL);
	hi6402_irq_mad_pll_enable(priv.p_irq, false);
#endif
}
...
static void hi6402_hifi_runstall_pull_down_work(void)
{
	/* Pull down runstall of HIFI*/
	hi6402_hifi_reg_clr_bit(HI6402_DSP_SC_DSP_CTRL0, 2);
}

static void hi6402_hifi_runstall_pull_down(void)
{
	mutex_lock(&priv.runstall_mutex);
	if(0 == priv.runstall_ref)
		hi6402_hifi_runstall_pull_down_work();
	priv.runstall_ref++;
	mutex_unlock(&priv.runstall_mutex);
}

static void hi6402_hifi_runstall_pull_up_work(void)
{
	/* Pull up runstall of HIFI*/
	hi6402_hifi_reg_set_bit(HI6402_DSP_SC_DSP_CTRL0, 2);
}

static void hi6402_hifi_runstall_pull_up(void)
{
	mutex_lock(&priv.runstall_mutex);
	if(0 == priv.runstall_ref){
		mutex_unlock(&priv.runstall_mutex);
	...
	priv.runstall_ref--;
	if(0 == priv.runstall_ref)
		hi6402_hifi_runstall_pull_up_work();
	mutex_unlock(&priv.runstall_mutex);
}

static void hi6402_hifi_fw_redownload(void)
{
	int ret = 0;
	const struct firmware *fw;

	/* download dsp image */
	ret = request_firmware(&fw, HI6402_HIFI_FW, priv.dev);
...
static void hi6402_hifi_fw_download(const struct firmware *fw, void *context)
{
	BUG_ON(fw == NULL);
	hi6402_hifi_enable_pll();
	hi6402_hifi_pwron();
	hi6402_hifi_download(fw);
	release_firmware(fw);
	hi6402_hifi_pwroff();
	hi6402_hifi_disable_pll();
...
{
	/* enable mad clk */
	hi6402_hifi_enable_mad_pll();
	/* hi6402 hifi power on */
	hi6402_hifi_pwron();
	/* hi6402 hifi runstall pull down */
	hi6402_hifi_runstall_pull_down();
...
{
	priv.wakeup_state = 0;
	/* hi6402 hifi runstall pull up */
	hi6402_hifi_runstall_pull_up();
	/* hi6402 hifi power off */
	hi6402_hifi_pwroff();
	/* enable mad clk */
	hi6402_hifi_disable_mad_pll();
}
...
{
	//ana_stable time 5ms
	hi6402_hifi_write_reg(HI6402_MAD_ANA_TIME_L, 0xAA);
	//pll stable time
	hi6402_hifi_write_reg(HI6402_MAD_PLL_TIME_L, 0x01);
	//adc stable time 100us
	hi6402_hifi_write_reg(HI6402_MAD_ADC_TIME_L, 0xA5);
	//mad_omit_samp
	hi6402_hifi_write_reg(HI6402_MAD_OMIT_SAMP, 0x10);
	//rms_number
	hi6402_hifi_write_reg(HI6402_MAD_RMS_NUM, 0x01);
	//mad_auto_act_number
	hi6402_hifi_write_reg(HI6402_MAD_AUTO_ACT_NUMBER, 0x0);
	//mad_rms_mode
	hi6402_hifi_write_reg(HI6402_MAD_RMS_MODE, 0xC3);
	//mad_sleep_time0
	...
	//mad_rms_thre
	hi6402_hifi_write_reg(HI6402_MAD_RMS_THRE0, 0x00);
	//mad_rms_thre
	hi6402_hifi_write_reg(HI6402_MAD_RMS_THRE1, 0xA8);
	//mad_rms_thre
	hi6402_hifi_write_reg(HI6402_MAD_RMS_THRE2, 0x61);
	//mad_rms_thre
	...
	hi6402_hifi_reg_write_bits(HI6402_MAD_FILTER_ID,
		0x3<<HI6402_MAD_FILTER_ID_BIT, 0x7<<HI6402_MAD_FILTER_ID_BIT);
	//mad_filter_id1
	hi6402_hifi_reg_write_bits(HI6402_MAD_FILTER_ID1,
		0x0<<HI6402_MAD_FILTER_ID1_BIT, 0x7<<HI6402_MAD_FILTER_ID1_BIT);
	return;
}
/* ********************************************************************
 * sysfs device, for debugging
 * ********************************************************************/
static ssize_t hi6402_debug_show(struct device *dev,
...
static ssize_t hi6402_debug_store(struct device *dev,
				  struct device_attribute *attr,
...
static ssize_t hi6402_debuglevel_show(struct device *dev,
                                      struct device_attribute *attr,
                                      char *buf)
{
	IN_FUNCTION;
...
static ssize_t hi6402_debuglevel_store(struct device *dev,
                                       struct device_attribute *attr,
                                       const char *buf, size_t size)
{
	IN_FUNCTION;
...
static DEVICE_ATTR(hi6402debug, S_IRUGO | S_IWUSR|S_IWGRP,
		   hi6402_debug_show, hi6402_debug_store);
static DEVICE_ATTR(hi6402debuglevel, S_IRUGO | S_IWUSR|S_IWGRP,
		   hi6402_debuglevel_show, hi6402_debuglevel_store);

static int hi6402_create_sysfs_file(struct device *dev,
...
static void hi6402_remove_sysfs_file(struct device *dev,
                                     const struct device_attribute *attr)
{
	BUG_ON(dev == NULL);
...
static int hi6402_create_sysfs(struct platform_device *pdev)
{
	BUG_ON(pdev == NULL);

	hi6402_create_sysfs_file(&pdev->dev, &dev_attr_hi6402debuglevel);
	hi6402_create_sysfs_file(&pdev->dev, &dev_attr_hi6402debug);

	return 0;
}

static void hi6402_remove_sysfs(struct platform_device *pdev)
{
	BUG_ON(pdev == NULL);

	hi6402_remove_sysfs_file(&pdev->dev, &dev_attr_hi6402debuglevel);
	hi6402_remove_sysfs_file(&pdev->dev, &dev_attr_hi6402debug);
...
static int hi6402_hifi_misc_probe (struct platform_device *pdev)
{
	...
	request_firmware_nowait(THIS_MODULE, FW_ACTION_NOHOTPLUG,
		HI6402_HIFI_FW,  &(pdev->dev), GFP_KERNEL,
		NULL, hi6402_hifi_fw_download);
