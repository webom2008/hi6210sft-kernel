...
int dbg_value_for_ecall = 0x12345678; /*only for testing ecall + variable*/
...
static int shell_call(char* func_name, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6)
{
    ...
    if (DBG_ECALL_VARIABLE == arg1)
    {
    	...
    	result = *(int *)addr_variable;

    	printk("variable is %s, addr = 0x%x, value = 0x%x\n", func_name, (unsigned int)addr_variable, result);
