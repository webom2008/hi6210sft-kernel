...
static int hifi_get_dmesg(unsigned long arg)
{
	int ret = OK;
	...
	g_misc_data.dsp_log_addr = (char*)ioremap_wc(DRV_DSP_UART_TO_MEM, DRV_DSP_UART_TO_MEM_SIZE);
	...
	logi("get msg: len:%d from:%p to:%p\n", len, g_dsp_dump_info[0].data_addr, dump_info_user_buf);

	ret = copy_to_user(dump_info_user_buf, g_dsp_dump_info[0].data_addr, len);
	if (OK != ret) {
...
void hifi_write_file_from_memory_regions(char *filename, u64 addr, unsigned int size)
{
	mm_segment_t fs;
	struct file *fp = NULL;
	...
	fp = filp_open(filename, file_flag, 0664);
	if (IS_ERR(fp)) {
...
static void hifi_misc_handle_mail(void *usr_para, void *mail_handle, unsigned int mail_len)
{
	...
	/* 将剩余内容copy透传到buff中 */
	ret_mail = mailbox_read_msg_data(mail_handle, (unsigned char*)recv->rev_msg.mail_buff, (unsigned int *)&recv->rev_msg.mail_buff_len);
									 ...
									 unsigned int *usr_para_size, void *usr_para_addr)
{
	...
	if ((NULL == usr_para_addr) || (NULL == usr_para_size)) {
		loge("usr_size_p=0x%x, usr_addr=0x%p\n", (unsigned int)usr_para_size, usr_para_addr);
		ret = -EINVAL;
		goto END;
	}
...
static void hifi_timer_function(unsigned int data)
{
							...
							unsigned long arg)
{
    ...
    data32 = (unsigned long)compat_ptr(arg);
...
static int hifi_misc_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	phys_page_addr = (u64)g_misc_data.hifi_priv_base_phy >> PAGE_SHIFT;
	logd("vma=0x%p\n", vma);
...
void hifi_misc_proc_init( void )
{
    ...
    entry = proc_create("hifi", 0777, hifi_misc_dir,&hifi_proc_fops);
    if (entry == NULL) {
...
static int hifi_misc_probe (struct platform_device *pdev)
{
	...
	g_misc_data.dsp_time_stamp = (unsigned int*)ioremap_wc(SYS_TIME_STAMP_REG, 0x4);
	...
	g_misc_data.hifi_priv_base_virt = (unsigned char*)ioremap_wc(HIFI_BASE_ADDR, (HIFI_SIZE - HIFI_IMAGE_SIZE));
	if (NULL == g_misc_data.hifi_priv_base_virt) {
		loge("hifi Addr ioremap_wc Error!\n");
		goto ERR;
	}
