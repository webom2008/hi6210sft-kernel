...
#define EDMAC_MAKE_LLI_ADDR( _p )   (BSP_U32)( (BSP_U32)(_p) & 0xFFFFFFE0 )
...
/*****************************************************************************
 函 数 名  : balong_dma_channel_mode_switch
 功能描述  : DSP在GU模切换到C模时，需要调用DRV提供的API接口，
 接口中把BBE16用的的通道EDMA_CHANNEL_DSP_1-EDMA_CHANNEL_DSP_8中断屏蔽掉；
 在C模切换到GU模时调用该API，接口EDMA_CHANNEL_DSP_1-EDMA_CHANNEL_DSP_8
 中断打开。DRV默认按GU mode，DSP初始化时需要调用该接口告知DRV DSP
 初始化的模式，GU切换到C模时，需要保证8个通道不在使用中。
 输入参数  : BSP_U32 u32Mode
                            #define MODE_GSM      0x01
                            #define MODE_CDMA     0x02
                            #define MODE_WCDMA    0x04
 输出参数  : NA
 返 回 值  : OK:success, ERROR:fail
 调用函数  : NA
 被调函数  : NA

 修改历史      :
  1.日    期   : 2013年09月17日
    作    者   : wangbinghui  00140341
    修改内容   : 新生成函数

*****************************************************************************/
extern BSP_S32 balong_dma_channel_mode_switch (BSP_U32 u32Mode);
#define DRV_EDMA_CHANNEL_MODE_SWITCH(u32Mode)  balong_dma_channel_mode_switch(u32Mode)
#endif
