...
#define BSP_IPC_CPU_RAW_INT(i)      (SOC_IPC_CPU_RAW_INT_ADDR((BSP_U8*)ipc_base, i))    /* CPU_i快速中断原始中断寄存器     */
#define BSP_IPC_CPU_INT_MASK(i)     (SOC_IPC_CPU_INT_MASK_ADDR((BSP_U8*)ipc_base, i))   /* CPU_i快速中断掩码寄存器         */
#define BSP_IPC_CPU_INT_STAT(i)     (SOC_IPC_CPU_INT_STAT_ADDR((BSP_U8*)ipc_base, i))   /* CPU_i屏蔽后的快速中断状态寄存器 */
#define BSP_IPC_CPU_INT_CLR(i)      (SOC_IPC_CPU_INT_CLR_ADDR((BSP_U8*)ipc_base, i))    /* CPU_i快速中断清除寄存器         */
#define BSP_IPC_INT_MASK_EN(i)      (SOC_IPC_CPU_INT_MASK_EN_ADDR((BSP_U8*)ipc_base, i))    /* CPU_i快速中断使能           */
#define BSP_IPC_INT_MASK_DIS(i)     (SOC_IPC_CPU_INT_MASK_DIS_ADDR((BSP_U8*)ipc_base, i))   /* CPU_i快速中断去使能         */

#define BSP_IPC_SEM_RAW_INT(j)      (SOC_IPC_SEM_RAW_INT_ADDR((BSP_U8*)ipc_base, j))    /* CPU_j信号量释放原始中断寄存器   */
#define BSP_IPC_SEM_INT_MASK(j)     (SOC_IPC_SEM_INT_MASK_ADDR((BSP_U8*)ipc_base, j))   /* CPU_j信号量释放中断掩码寄存器   */
#define BSP_IPC_SEM_INT_STAT(j)     (SOC_IPC_SEM_INT_STAT_ADDR((BSP_U8*)ipc_base, j))   /* CPU_j信号量释放中断状态寄存器   */
#define BSP_IPC_SEM_INT_CLR(j)      (SOC_IPC_SEM_INT_CLR_ADDR((BSP_U8*)ipc_base, j))    /* CPU_j信号量释放中断清除寄存器   */
#define BSP_IPC_HS_CTRL(j,k)        (SOC_IPC_HS_CTRL_ADDR((BSP_U8*)ipc_base, j, k))     /* CPU_j信号量k请求寄存器      */
#define BSP_IPC_HS_STAT(j,k)        (SOC_IPC_HS_STAT_ADDR((BSP_U8*)ipc_base, j, k))     /* CPU_j信号量k状态寄存器      */
