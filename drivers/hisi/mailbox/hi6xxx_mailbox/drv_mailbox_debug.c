                ...
                char*           file_name)
{
    ...
    {
        systemError(BSP_MODU_MAILBOX, err_no, 0, 0, 0);
    }
...
static int mailbox_log_fopen( const char *filename, int flags )
{
    ...
    if (fd < 0) {
        mailbox_out((KERN_ERR "mailbox_log_fopen:sys_open op fail, filename:%s, fd is %x \n", \
                                    filename, fd));
        return MAILBOX_ERRO;
    }
...
static void mailbox_log_work_proc(struct work_struct *log_work)
{
    ...
    kfree((void*)(log_work_mntn->mntn_ipc.buff));
...
static int mailbox_log_work_submit(unsigned int mailcode, int flag)
{
    ...
    if(MAILBOX_LOG_NOT_DEPEND_CNT != flag) {
        if (mailbox_log_work_cnt >= MAILBOX_LOG_FILE_CNT) {
            return 0;
        }
    }
    ...
    /*malloc memory and copy ipc mntn info*/
    ipc_debug_int_info(&mntn_ipc_addr, &mntn_ipc_size);
    mntn_ipc_buf = kmalloc(mntn_ipc_size, GFP_ATOMIC);
    if (MAILBOX_NULL == mntn_ipc_buf) {
        mailbox_out((KERN_ERR "mailbox_log_work_submit:fail to malloc for ipc mntn"RT));
        kfree(log_work_mntn);
        kfree(mntn_mb_buf);
        return -1;
    }

    memcpy((void*)mntn_ipc_buf, (void*)mntn_ipc_addr, mntn_ipc_size);
    log_work_mntn->mntn_ipc.buff = (unsigned long)mntn_ipc_buf;
