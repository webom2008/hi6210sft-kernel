...
static u32 g_ccore_logbuf_addr = 0;
...
*****************************************************************************/
static int log_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
    logger_info_stru * log_info;
    u32 read_p;
    u32 write_p;
...
static int __init log_init(void)
{
    ...
    g_ccore_logbuf_addr = (u32)ioremap_nocache(MEMORY_RAM_CORESHARE_CCORE_LOGBUF_ADDR, 
