...
int ccorereset_loadcbin(void)
{
    ...
    iresult_read = drv_read_bin((const char*)image_name, 0, sizeof(CCORE_IMAGE_HEAD_STRU), header_buff);
    if (iresult_read < 0)
    {
    ...
    for(i = 0; i < header->uwSecNum; i++)
    {
        ...
        if (header->astSections[i].enLoadAttrib == CCORE_IMAGE_SEC_LOAD_BUFFER )
        {
            ...
            iresult_read = drv_read_bin((const char*)image_name, header->astSections[i].uwSrcOffset, header->astSections[i].uwSize, (char*)(iviraddr_dest1 + sizeof(CCORE_IMAGE_SEC_STRU)));
            ...
            || (header->astSections[i].enLoadAttrib == CCORE_IMAGE_SEC_LOAD_STATIC ))
        {
            ...
            iresult_read = drv_read_bin((const char*)image_name, header->astSections[i].uwSrcOffset, header->astSections[i].uwSize, iviraddr);
...
int ccorereset_loadbbebin(void)
{
    ...
    iRet = drv_read_bin((const char*)image_name, 0, image_load_size, (char *)buff);
    if(RFILE_OK != iRet)
    {
