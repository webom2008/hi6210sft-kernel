...
#include "../secure_boot/crypto_driver/crypto_driver_defs.h"
#include "../secure_boot/include/dx_pal_types.h"
#include "../secure_boot/include/dx_pal_types_plat.h"
#include "../secure_boot/include/sb_hash.h"
#include "../secure_boot/platform/nvm/nvm.h"
#include "../secure_boot/boot_images_verifier/bootimagesverifier_def.h"
#include "../secure_boot/boot_images_verifier/bootimagesverifier_api.h"
#include "../secure_boot/boot_images_verifier/bootimagesverifier_parser.h"
#include "../secure_boot/boot_images_verifier/bootimagesverifier_swvalidation_anddecryption.h"

#include "../secure_boot/include/secureboot_basetypes.h"
               ...
               unsigned int *image_size)
{
    ...
    vrl_header = (VRL_Header_t *)((unsigned int)vrl_data + ADDITIONAL_DATA_SIZE);
    ...
    /* software version */
    vrl_sw_version = (VRL_SW_Version_t *)((unsigned int)vrl_data + size);
    ...
    if (img_num > 1) {
        ...
        g_image_load_addr = (*(unsigned int *)((unsigned int)vrl_data + size));
    }
    ...
    if (img_num) {
        *image_size = (*(unsigned int *)((unsigned int)vrl_data + size + 4))*4;
    }
...
int check_image_in_ram(unsigned int *image_buf)
{
    ...
    ret = DX_BIV_GetPrimaryKeyHash(NVM_ReadHASHPubKey, virAddr,
                                   pubKeyHASH, 0, 0);
    if(DX_OK != ret) {
    ...
    ret = DX_BIV_VRLParser((DxUint8_t)DX_FALSE,
                           ...
                           pubKeyHASH,
                           swVersion,
                           (DxUint32_t *)((unsigned int)image_buf + vrl_parse_st.vrl1_size),
                           &secVrlAddress_ptr,
                           secondaryNHash_ptr);
    if(DX_OK != ret) {
    ...
    /* 计算二级VRL的大小 */
    ret = (unsigned int)parse_vrl((unsigned int *)((unsigned int)image_buf +
                     vrl_parse_st.vrl1_size),
                     &vrl_parse_st.vrl2_size,
                     &image_num,
                     &vrl_parse_st.image_size);
    if(DX_OK != ret) {
    ...
    /* 获取二级VRL中需要安全校验的映像在image_buf中的地址，将其保存到全局变量g_image_addr中 */
    g_image_addr = (int)image_buf + VRL_SIZE;
