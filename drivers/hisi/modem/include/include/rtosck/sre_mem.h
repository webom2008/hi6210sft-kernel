...
6v3DaudNLcS51sXs7BamKwkXkzw7dkDLj2dzb+JCDH07yzBzgyJwgWvFSKYll/QtXHD0EUMG
GZ/AeNq8+L4ZzTRnhs1J/elOwLZe+epa/AtPhyi0FtoketvaahJQcFqBsWMwOQ==*/
/*--------------------------------------------------------------------------------------------------------------------------*/
/**
* @file sre_mem.h
*
 ...
 *
 */
#define OS_ERRNO_MEM_FREE_SH_DAMAGED                        SRE_ERRNO_OS_ERROR(OS_MID_MEM, 0x11)
 ...
 * 解决方案: 查看输入的算法是否正确。
 */
#define OS_ERRNO_MEM_ARITH_INVALID                          SRE_ERRNO_OS_ERROR(OS_MID_MEM, 0x16)
 ...
 */
#define OS_ERRNO_MEM_SHARE_PARA_INCONSISTENT                SRE_ERRNO_OS_ERROR(OS_MID_MEM, 0x1a)
 ...
 *@retval #OS_ERRNO_MEM_FREE_SH_DAMAGED             0x02000111，内存释放时要释放的内存块的头被破坏，或已释放。
 ...
 *@retval #OS_ERRNO_MEM_SHARE_PARA_INCONSISTENT      0x0200011a，共享分区参数配置不统一。不同核创建相同编号的共享分区时参数应相同。
 ...
 *@retval #OS_ERRNO_MEM_ARITH_INVALID   0x02000116，内存算法类型非法或OS不支持该内存算法。
...
/**
 *@ingroup SRE_mem
 *@brief 查询内存使用情况，将信息返回给pstMemPtUsage所指向的结构体。
 *
 *@par 描述:
 ...
 *
 *@param ucPtNo         [IN] 类型#UINT8， 分区号，范围[0,#OS_MEM_MAX_PT_NUM+2)。
 *@param pstMemPtUsage  [OUT]类型#OS_MEM_PT_USAGE_S *，内存分区使用情况汇总,保存内存分区的使用状态信息。
 *@param pusMaxNum      [IN/OUT]类型#UINT16 *，输入缓冲区数组的个数，输出填写在缓冲区pstMemTask中有效的元素个数。
 *@param pstMemThread   [OUT]类型#OS_MEM_TASK_S *，缓冲区，用于填写任务和软中断的内存占用信息。
 *
 *@retval #OS_ERRNO_MEM_NOT_SUPPORT                 0x02000118，功能不支持。
 *@retval #OS_ERRNO_MEMINFO_CHECK_ALL_ADDR_NULL     0x02000115，输入的有效参数都为NULL。
 *@retval #OS_ERRNO_MEMINFO_TSK_SINGLE_ADDR_NULL    0x02000116，查询单个内存使用情况时，输入的两个有效参数其中一个为NULL。
 ...
 *@see SRE_MemPtCreate
 */
extern UINT32 SRE_MemPtUsage(UINT8 ucPtNo, OS_MEM_PT_USAGE_S *pstMemPtUsage, UINT16 *pusMaxNum, OS_MEM_TASK_S *pstMemThread);

#if (OS_HAVE_UNCACHE_MEM == YES)
/**
 *@ingroup SRE_mem
