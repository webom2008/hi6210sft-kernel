...
6v3DauxHbC81Wwwqr7tZHMt3if37ykVq3p5HrKkEjSIFwVGeir0Wk2GW2Wx8qupmUVGMQ0KG
j/13zIjrxEq/NbsX8Smq+ICES9PAUx3VqzB8x5rc61FPNYGe5kCssT7V5RhTKw==*/
/*--------------------------------------------------------------------------------------------------------------------------*/
/**
 * @file sre_msgm.h
 *
 ...
 * 值: 0x02002b04
 *
 * 解决方案: 队列深度必须是非0值，且GLBAL MSGM不能超过2048，LOCAL MSGM不能超过1024。
 *
 */
...
/**
 * @ingroup SRE_msgm
 * 多核消息驱动错误码：配置QPORT的队列深度时，队列处于被使能状态。
 *
 * 值: 0x02002b05
 *
 ...
 * 值: 0x02002b08
 *
 * 解决方案: tag值在SD6182和SD6157上有效取值范围：1~7，对应于128bit消息中第4个WORD的低3bit。
 *
 */
 ...
 * 值: 0x02002b0f
 *
 * 解决方案: 指定的目的核必须是实际存在的核号或者目的核还没有启动完成。
 *
 */
...
/**
 * @ingroup SRE_msgm
 * 多核消息驱动错误码：在使用组装os消息类型的消息组装接口时，指定的目的线程消息队列非法。
 *
 * 值: 0x02002b12
 *
 * 解决方案: 指定的目的线程的消息队列必须为0。
 *
 */
#define OS_ERRNO_MSGM_QID_INVALID                               SRE_ERRNO_OS_ERROR(OS_MID_MSGM, 0x12)
 ...
 * 值: 0x02002b13
 *
 * 解决方案: 不允许发送给os固定占用的线程(idle任务，idle软中断，tick软中断)，且必须是已创建的有效线程号。
 *
 */
 ...
 * 值: 0x02002b16
 *
 * 解决方案: 请确保MSGM的ID值为0或1(1对SD6182有效)。
 *
 */
...
/**
 * @ingroup SRE_msgm
 * 多核消息驱动错误码：在调用SRE_QportMsgCreate接口组装消息时，传入的消息体句柄非法（错误或已删除或句柄指针为NULL），或者消息体正在被使用
 *
 * 值: 0x02002b21
 *
 ...
 * 值: 0x02002b24
 *
 * 解决方案: 共享消息队列共32个，所以可用ID范围为[0-31]。
 *
 */
 ...
 * 值: 0x02002b2c
 *
 * 解决方案: 最大支持16个同步消息队列，有效ID范围为[0, 15]。
 *
 */
 ...
 * 值: 0x02002b40
 *
 * 解决方案: 请确保传入的MSGM队列起始地址不超过最大支持的地址，对于Global MSGM普通队列，最大不超过2047，对于Local MSGM普通队列，以及共享队列，最大不超过1023。
 */
 ...
 * 值: 0x02002b41
 *
 * 解决方案: 请确保传入的MSGM队列长度不超过最大支持的长度，对于Global MSGM普通队列，最大不超过2048，对于Local MSGM普通队列，以及共享队列，最大不超过1024。
 */
 ...
 * 值: 0x02002b42
 *
 * 解决方案: 请确保传入的MSGM队列起始地址和长度决定的区间合法，对于Global MSGM普通队列，有效范围为[0,2047]，对于Local MSGM普通队列，以及共享队列，有效范围为[0,1023]。
 */
 ...
 * 值: 0x02002b4b
 *
 * 解决方案: MSGM ID有效值为SRE_GLOBAL_MSGM或SRE_LOCAL_MSGM(SRE_LOCAL_MSGM只对SD6182有效)。
 *
 */
...
typedef struct tagSreSynQueAttr
{
    ...
    UINT32 uwSynCnt;         /**< 同步计数值，有效范围[0, 31]  */
    ...
    UINT32 uwSynMask;        /**< 过滤消息的掩码，仅对消息的最后一个word进行掩码 */
    UINT32 uwIrqDstCore;     /**< 中断通知模式时，通知的目标核号。(仅中断通知模式有效) */
    VOID * pMsgDstAddr;      /**< 消息通知模式时，消息通知的目的地址。(仅消息通知模式有效, 16字节对齐) */
 ...
 * <ul>
 * <li>OS使用了0号QPORT的0号队列；0号QPORT为OS管理，创建为中断方式，故用户必须通过中断方式接收0号QPORT队列中消息(接收时通过用户自己注册的钩子函数实现消息接收处理)。</li>
 * <li>DSP核使用的所有队列深度加起来不能超过2048，HACCC使用的所有队列深度加起来不能超过1024。</li>
 * <li>队列起始位置和队列深度必须配置成4的整数倍。</li>
 * <li>必须主控核调用该接口。</li>
 * </ul>
 *
 * @param uwCoreID       [IN] 类型#UINT32，指定的核号,取值范围[0,33]。
 ...
 * @see 无
 */
extern UINT32 SRE_QueStartDepthSet(UINT32 uwCoreID, UINT32 uwQportID, UINT32 uwQueIdx, UINT32 uwQueStart, UINT32 uwQueDepth);
 ...
 * </ul>
 *
 * @param uwTag      [IN] 类型#UINT32，Qport消息标识Tag，在SD6182和SD6157上取值范围：1~7(业务自定义消息); OS消息使用的Tag为0。
 ...
 *
 * @retval #OS_ERRNO_MSGM_QID_INVALID                0x02002b12，指定的线程消息队列id非法。
 ...
 * @param pTraceBufAddr  [IN] VOID *，TRACE BUFFER的首地址，注：需保证为256bit对齐。
 ...
 * </ul>
 *
 * @param uwMsgmID      [IN] UINT32，MSGM ID，有效值为SRE_GLOBAL_MSGM或SRE_LOCAL_MSGM(SRE_LOCAL_MSGM只对SD6182有效)。
 ...
 * </ul>
 *
 * @param uwQueID       [IN] UINT32，队列物理ID，有效值范围为[0, 31]。
 * @param uwFifoStart   [IN] UINT32，队列FIFO起始游标值。有效范围为[0, 1023]。
 * @param uwFifoDepth   [IN] UINT32，队列FIFO深度，有效范围为[1, 1024]。且uwFifoStart + uwFifoDepth不能超出1024。
 ...
 * </ul>
 *
 * @param uwQueID       [IN] UINT32，队列物理ID，有效值范围为[0, 15]。
...
#if (OS_HARDWARE_PLATFORM == OS_RISC220)
/**
 * @ingroup SRE_msgm
 ...
 * </ul>
 *
 * @param uwMsgmID      [IN] UINT32，待初始化复位的MSGM，有效值为SRE_GLOBAL_MSGM或SRE_LOCAL_MSGM(SRE_LOCAL_MSGM只对SD6182有效)。
 *
...
/**
 * History:
 * 2012-7-1 z00161222: Create this file.
 *
