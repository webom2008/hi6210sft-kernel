...
6v3DauREl6fIhPeF+I8BeO7ieVdQa/pmngdpIEvyuaBZ+n3kXJZzNmioOc7dL8mLhe5aRPqc
Ysb1k8ulUCDyfbpaH9PlmsjwkOALLmyGhRWWKYeEUbrHMKmYV+t6zrNSR9SLCw==*/
/*--------------------------------------------------------------------------------------------------------------------------*/
/**
 * @file sre_shell.h
 *
...
/**
 * @ingroup sre_shell
 * HShell错误码：Hshell Server端运行的核号配置错误。
 *
 * 值: 0x02002d07
 *
 * 解决方案: 建议检查OS_SHELL_SERVER_COREID的配置项:SD6182的Shell Server部署在cpu cluster，核号为12~16，其他核号配置不合法;SD6108/81 的Shell Server只能部署在0核。
 *
 */
#define OS_ERRNO_SHELL_SHOW_SERVER_COREID_CONFIG_ERROR                          SRE_ERRNO_OS_ERROR(OS_MID_SHELL, 0x07)

/**
 * @ingroup sre_shell
 * HShell错误码：Hshell Server端使用UART通道时，配置创建UART所使用的中断号有错误。
 *
 * 值: 0x02002d08
 *
 * 解决方案: 建议检查OS_SHELL_INCLUDE_UART_HWINO的配置项:SD6182其UART所使用的中断号为19~26，其他中断号有特殊用途，不能配置;SD6108/81其UART默认所使用的中断号为24，配置无效。
 *
 */
#define OS_ERRNO_SHELL_CHANNEL_UART_HWINO_CONFIG_ERROR                          SRE_ERRNO_OS_ERROR(OS_MID_SHELL, 0x08)
...
* @attention
* <ul>
* <li>目前只有host shell支持格式化信息输出函数。</li>
* </ul>
*
