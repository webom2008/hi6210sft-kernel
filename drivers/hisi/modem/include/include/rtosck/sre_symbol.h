...
6v3DatQIhep59opl8EZo429sm8FqMkpi8BnQjipjuElkHtOeVv32v2EEuvfEj9pZf+ydfkC9
Qc/IxgtKDC/a9TApKUhiK+PVrHSseM/w9CStwhonqPTDCD+DzN9hotNyOuBeaw==*/
/*--------------------------------------------------------------------------------------------------------------------------*/
/**
 * @file sre_symbol.h
 *
...
typedef struct tagSymbolNode   /**< HASH_SYMBOL_NODE - entry in symbol table */
{
    ...
    UINT8       ucSymGroupID;      /**< 符号动态添加的组ID，0~0xfe为LP使用(值为LP的GroupID+1)，0xff为shell使用     */
    UINT16      usUnitInterNo;     /**< 符号的所属单元ID，0~9999为LP使用(值为LP的内部补丁单元号)，0xfffe为shell使用*/
    UINT32      uwSize;            /**< 符号的长度                      */
} OS_SYMBOL_NODE_S;
 ...
 * 值: 0x02002909
 *
 * 解决方案: SD6182/SD6108/SD6181/SD6157平台，符号表模块分区号不能配置为0。
 *
 */
