...
/**
 * @ingroup SRE_task
 * 任务错误码：配置的任务数量太多，最多126个，不包括Idle任务且不能为0。
 *
 * 值: 0x0200081a
 *
 * 解决方案: 在任务配置项中将最大任务数改为小于等于126个且大于0。
 */
...
typedef struct tagTskContext
{
    ...
    UINT32 uwLBEG2;                    /**< Loop Begin2寄存器               */
    UINT32 uwLEND2;                    /**< Loop End2寄存器                 */
    UINT32 uwLCOUNT2;                  /**< Loop Count2寄存器               */
    UINT32 uwReserve2;                 /**< 保留                            */
    UINT32 uwLBEG3;                    /**< Loop Begin3寄存器               */
    UINT32 uwLEND3;                    /**< Loop End3寄存器                 */
    UINT32 uwLCOUNT3;                  /**< Loop Count3寄存器               */
    UINT32 uwReserve3;                 /**< 保留                            */
 ...
 *
 * @param  uwTaskPID [IN] 类型#UINT32，任务PID。
 * @param  uwTaskPID [IN] 类型#TSK_PRIOR_T *，保存任务优先级指针。
 *
 * @retval #OS_ERRNO_TSK_PTR_NULL               0x02000801，指针参数为空。
 * @retval #OS_ERRNO_TSK_ID_INVALID             0x02000807，任务PID非法。
 * @retval #OS_ERRNO_TSK_NOT_CREATED            0x0200080a，任务未创建。
 ...
 *
 * @retval #OS_ERRNO_TSK_PTR_NULL           0x02000801，获取所以任务的PID信息时，输入入参为NULL。
 * @retval #OS_ERRNO_TSK_INPUT_NUM_ERROR    0x02000821，获取所以任务的PID信息时，任务数为0个。
 * @retval #SRE_OK                          0x00000000，成功。
 * @par 依赖:
 * <ul><li>sre_task.h：该接口声明所在的头文件。</li></ul>
 * @since RTOSck V100R001C01
