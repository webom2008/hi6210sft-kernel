...
6v3DapdPQvc4+f1u3di3L5aLfmvFP43kkvNtGoJW+dVwX5m1Jh4cEUE4799A7LNYjSkSWvXo
06jRl7/UIQPU8vrtb+jMoV/OLx2kaDmz9b+Hi2ekR7UIqq2t0g1fIIUQCnXcQA==*/
/*--------------------------------------------------------------------------------------------------------------------------*/
/**
 * @file sre_timer.h
 *
...
/**
 * @ingroup sre_timer
 * 定时器回调函数定义，带5个参数、返回值类型为void的函数指针类型
 */
...
typedef enum tagTimerType
{
    ...
    SRE_TIMER_HARDWARE_SHARED,                        /**< 共享硬件定时器,仅SD6182和SD6157支持(所有核共享的外设全局定时器)  */
...
/*======================================================SHARED HWTMR ERRNO============================================*/
/**@ingroup sre_timer
 * 共享硬件定时器错误码:创建共享硬件定时器时传入的定时超时处理函数非空。
 *
 * 值: 0x02002e70
 *
 * 解决方案:确保传入的共享硬件定时器超时处理函数为空。
 */
#define OS_ERRNO_SHARED_TIMER_FUNC_NO_NULL                  SRE_ERRNO_OS_ERROR(OS_MID_TIMER,0x70)

/**@ingroup sre_timer
 * 共享硬件定时器错误码:创建共享硬件定时器时传入的定时器已经创建。
 *
 * 值: 0x02002e71
 *
 * 解决方案:确保传入的共享硬件定时器ID对应的定时器已经删除。
 */
#define OS_ERRNO_SHARED_TIMER_CREATED                       SRE_ERRNO_OS_ERROR(OS_MID_TIMER,0x71)
 ...
 * @attention
 * <ul>
 * <li>DSP220/RISC220/HACCC/RISC170/SD6183平台可以使用2个硬件定时器资源,DSP170平台可以使用3个硬件定时器资源,Cortex_AX及Cortex_M4平台不支持硬件定时器功能。</li>
 ...
 * <li>TIMER_CREATE_PARA_S参数里面的uwInterval元素表示定时器周期，软件定时器单位是ms，硬件定时器单位是us，设置时间间隔的时候请注意适配，过大会出现溢出。</li>
 * </ul>
 *
 ...
 * @retval #OS_ERRNO_SHARED_TIMER_ID_INVALID         0x02002e72，传入的共享硬件定时器ID不可用。
 * @retval #OS_ERRNO_SHARED_TIMER_FUNC_NO_NULL       0x02002e70，创建共享硬件定时器时传入的定时超时处理函数非空。
 ...
 *
 * @par 描述:
 * 将定时器逻辑ID为uwTmrHandle的定时器由创建状态变成启动状态，对于被停止过的软件定时器，相当于从上次暂停的时刻继续计时。
 *
 * @attention
 * <ul>
 ...
 * @attention
 * <ul>
 * <li>该接口只有SD6182和SD6157共享硬件定时器使用，其他定时器不可使用。</li>
