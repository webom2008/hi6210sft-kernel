...
typedef struct tagIKEShellHardware
{
    ULONG (*pfIKE_SH_Proposal2Hard) (IKE_PROPOSAL_CONTENT_S *pstIKEProp,
                                     ULONG ulFlag);
    ULONG (*pfIKE_SH_KeepAlive2Hard) (ULONG ulOpMode, UCHAR ucFlag,
                                                               ULONG ulTimeVal);
    ULONG (*pfIKE_SH_LocalName2Hard) (UCHAR *pucLocalName);
    ULONG (*pfIKE_SH_Peer2Hard) (IKE_PEER_CONTENT_S *pstIKEPeer, ULONG ulFlag);
    ULONG (*pfIKE_SH_NatKeepAlive2Hard) (ULONG ulSecond);
    ULONG (*pfIKE_SH_ClearSa2Hard) (ULONG ulConnID, ULONG ulIfIndex);
    ULONG (*pfIKE_SH_RSALocal2Hard) (ULONG ulSubRackNum, ULONG ulSlotNo,
                                      CHAR *pucLocalName, ULONG ulKeyLength);
}IKE_SHELL_HARDWARE_S;
#endif
...
/* IPSEC6.ITR2.STORY4 */
/* Structure removed and new struct IKE_DPD_VERDICT_INFO*/
#if 0
/*  Holds the data about the vendor id negociation result,
    used to form SADB msg   */
typedef struct tagIKE_DPD_VID_S
{
    CHAR szPeerName[IKE_PEER_NAME_MAX_LEN];/* The peer as per DPD message sent*/
    ULONG ulIfindex;
    USHORT usMsgType;/* The SADB_MSG to be sent to IPSec */
    CHAR  szResv[2];
}IKE_DPD_VID_S;
#endif
