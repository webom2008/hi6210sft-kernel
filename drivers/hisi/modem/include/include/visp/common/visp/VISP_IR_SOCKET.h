...
#define HASH_ADDR(ulFAddr) \
(((USHORT)((ULONG)(ulFAddr)>>16)) ^ ((USHORT)(ulFAddr)))

#define HASHFUNC6_SECOND(usFPort, pstFAddr) \
           (USHORT)((((HASH_ADDR((pstFAddr)->s6_addr32[0]) ^ HASH_ADDR((pstFAddr)->s6_addr32[1]) ^ \
      HASH_ADDR((pstFAddr)->s6_addr32[2]) ^ HASH_ADDR((pstFAddr)->s6_addr32[3])) \
      + (usFPort))  &   HASH_SECONDLEVEL_MASK ))
...
#define SO_HEADCACHE  0x00400000/*add by y36299 for HeadCache 2005/03/22*/
...
/* 原来是(FILENUM + 1)，把它更改成下面值，如果创建VRF的个数等于FILENUM + 1,则会存在风险，目前PTN最大支持VPN个数为512 ,支持预配置*/
...
#define FD_SETSIZE      3072 /*Modified by x36317, 2005-1-5*/
...
typedef struct tagSOCK_MOD_INFO_S
{
    ...
    ULONG ulHC_ENTRY_MAINTYPE;    /*add by y36299 for Head Cache 2005/03/23*/
    ULONG ulSKT_GLOBAL_SOCKET_EN; /*Added by x36317, 2005-09-26*/
...
typedef struct tagSOCKET
{
    ...
    ULONG               so_sOptions;        /* from socket call, see socket.h
                                             changed by yuyong&mayun 2002/10/30 for ip address overlap*/
...
#define so_stCallBack      so_unAsynSock.so_unstCallBack
    ...
    /*add by y36299 for Head Cache 2005/03/22*/
...
#define     EXPANDNUM       FILENUM      /* Modified by x36317 for SWFD08426, 2005-5-19*/
