...
typedef struct tagSFE_PKTINFO_USERPRIVATE
{
    ...
    UINT32 u32UsrPrivateLen;                            /* 产品私有数据长度 */
    UINT8  au8PktUsrPrivate[SFE_MBUF_PRIVATE_DATA_LEN]; /* 产品私有数据缓存区 */
    UINT8 *pu8UserPointer1;     /* 产品私有信息指针1,该指针指向的内存由产品负责申请和释放. */
    UINT8 *pu8UserPointer2;     /* 产品私有信息指针2,该指针指向的内存由产品负责申请和释放. */
...
/* 记录MBUF扩展信息,包括产品私有信息和MBUF新增字段信息,后续MBUF新增字段统一在该结构中增加 */
typedef struct tagSFE_MBUF_EXPAND
{
    ...
    SFE_PKTINFO_USERPRIVATE_S    stUsrPrivateData;  /* 产品私有信息 */
...
typedef struct tagSFE_MBuf
{
    ...
    SFE_IPADDR_UN                    unNextHopIp;                       /* 下一跳IP地址 */
    UINT32                           u32ModuleID;                       /* 创建该MBUF的模块ID */
    UINT32                           u32DataBlockNumber;                /* mbuf中数据块的个数 */

    SFE_MBUF_EXPAND_S               *pstExpandInfo;                     /* 记录MBUF扩展信息,包括产品私有信息和MBUF新增字段信息 */
