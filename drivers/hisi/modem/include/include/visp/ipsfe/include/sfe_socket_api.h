...
typedef enum enumSfeSockErrCode
{
    ...
    SFE_SOCK_BIND_EFAULT,                           /* 81 绑定socket,传入IP地址非法 */
    ...
    SFE_SOCK_CONNECT_EFAULT,                        /* 98 连接socket,传入IP地址非法 */
    ...
    SFE_SOCK_SET_CONNINFO_DIRC_INVALID,             /* 126 设置SOCK用户信息时,方向信息非法 */
    SFE_SOCK_SET_CONNINFO_VCPUID_INVALID,           /* 127 设置SOCK用户信息时,获取的vcpu无效 */
...
{
    UINT32 s_addr;                              /* IP address of the socket */
}SFE_IN_ADDR_S;
...
typedef struct tagSFE_SOCKADDR_IN
{
    ...
    SFE_IN_ADDR_S   sin_addr;                   /* 32-bit IP address */
    INT8            sin_zero[8];                /* must be 0 */
}SFE_SOCKADDRIN_S;
...
typedef struct tagSFE_SOCK_NOTIFY_INFO
{
    ...
    UINT32 u32LocalIP;                            /* 本地IP地址,报文中的目的地址(网络字节序) */
    UINT32 u32RemoteIP;                           /* 远端IP地址,报文中的源地址(网络字节序) */
...
INT32 SFE_SetSocketOpt(INT32 i32Fd, INT32 i32Level, INT32 i32OptName,
                       INT8 *pi8OptVal, INT32 i32OptLen);


INT32 SFE_GetSocketOpt(INT32 i32Fd, INT32 i32Level, INT32 i32OptName,
                       INT8 *pi8OptVal, INT32 *pi32OptLen);


INT32 SFE_Ioctl(INT32 i32Fd, UINT32 u32ReqType, INT8 *pi8Value);
