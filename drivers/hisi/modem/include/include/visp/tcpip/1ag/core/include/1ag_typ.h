...
typedef struct  tagOAM1AGVLANTAG
{
    ...
    USHORT  usCfi      :1;                            /* 0表示MAC地址是经典格式，1表示MAC地址不是经典格式，默认是0 */
...
{
    UCHAR   ucHdrDstMacAddr[OAM_1AG_MACADDR_LEN];     /*目的MAC地址   */
    UCHAR   ucHdrSrcMacAddr[OAM_1AG_MACADDR_LEN];     /*源MAC地址     */
    USHORT             usLen;                         /*以太网幀长度  */
...
typedef struct tagOAM_1AG_RMEP
{
    ...
    UCHAR  aucMacAddress[OAM_1AG_MACADDR_LEN];      /*RMEP mac地址*/
...
typedef struct tagOAM_1AG_REPLYINGRESSTLV
{
    ...
    UCHAR aucIngressMac[OAM_1AG_MACADDR_LEN];    /* 入接口MAC地址 */
...
typedef struct tagOAM_1AG_REPLYEGRESSTLV
{
    ...
    UCHAR aucEgressMac[OAM_1AG_MACADDR_LEN];     /* 出接口MAC地址 */
...
typedef struct tagOAM_1AG_LTM_SENDOUTDATA
{
    ...
    UCHAR aucTargetMac[OAM_1AG_MACADDR_LEN]; /* 目的MAC地址 */
    UCHAR aucReserve[2];
}OAM_1AG_LTM_SENDOUTDATA_S;
/* 发送LT的任务所需的参数 */
typedef struct tagOAM_1AG_LT_PARA
{
    UCHAR aucTargetMac[OAM_1AG_MACADDR_LEN];    /* 目的MP的MAC地址 */
    UCHAR aucSourceMac[OAM_1AG_MACADDR_LEN];    /* 发起MP的MAC地址 */
    ULONG ulExecID;                             /* 用户ID */
    ULONG ulIfIndex;                            /* 出接口 */
...
enum eMDNAMETYPE
{
    ...
    OAM_1AG_MDNAMETYPE_MACADDR_ADD_2BYTEINT = 3,    /* MAC address + 2-octet integer */
...
/* Modified by w60786 for BC3D00986 2008-12-29 */
/*Rmep超时检测值*/
enum eRMEPOUTTIME
{
...
{
    OAM_1AG_RLY_HIT = 1,    /* The LTM reached an MP whose MAC address matches the target MAC address. */
