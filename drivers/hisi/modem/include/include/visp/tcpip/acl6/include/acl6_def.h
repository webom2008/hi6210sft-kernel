...
{\
    ...
    (VOID)VOS_Mem_Set(szAddrZero,0,ACL6_IPV6_ADDR_LEN);\
...
{\
    ...
    {\
        (VOID)VOS_Mem_Set(pstGroupInfo->szDescription, 0, (ACL6_DESCRIPTIONSTR_LEN + 1));\
    }\
...
{\
    ...
    (VOID)VOS_Mem_Set((CHAR*)pSubAddr, 0,ACL6_IPV6_ADDR_LEN);\
    (VOID)VOS_Mem_Set(szMask, 0,ACL6_IPV6_ADDR_LEN);\
    (VOID)VOS_Mem_Set(szMask, ucLastByte, (ulMaskLen / ulTemp));\
    if ((ulMaskLen) % ulTemp)\
    {\
