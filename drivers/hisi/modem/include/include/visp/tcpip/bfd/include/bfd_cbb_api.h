...
typedef enum tagBFD_ERR_E
{
    ...
    BFD_ERR_INVALID_ADDR           , /* 19 IP地址非法 */
    BFD_ERR_INTF_NOT_EXIST         , /* 20 指定接口不存在 */
    BFD_ERR_DISMATCH_IP_INDEX      , /* 21 指定接口索引和IP地址不匹配 */
...
typedef struct tagBFD_SESSION_INFO_S
{
    ...
    ULONG ulDstIPAddress;     /* 会话的目的IP地址，主机序 */
    ULONG ulSrcIPAddress;     /* 会话的源IP地址，主机序 */
...
typedef struct tagBFD6_SESSION_INFO_S
{
    ...
    ULONG ulMyDiscriminator;  /* 本端描述符 */
    ULONG ulYourDiscriminator;/* 对端端描述符 */
    ULONG ulDstIPAddress[4];     /* 会话的目的IP地址，主机序 */
    ULONG ulSrcIPAddress[4];     /* 会话的源IP地址，主机序 */
    ULONG ulIsMultiHop;       /* 会话是否是多跳，0:不是 1:是 */
    ULONG ulSentPakcets;      /* 会话发送的报文总个数 */
    ULONG ulRcvPackets;       /* 会话接收的报文总个数 */
    ULONG ulLastSentTime;     /* 上一个报文发送的时间，通过VOS_Tm_Now获取的低32位，单位:ms */
...
typedef struct tagBfdStatistic
{
    ...
    ULONG ulSrcAddr[4];      /*BFD会话的源IP地址（主机序）*/
    ULONG ulPeerAddr[4];     /*BFD会话的目的IP地址（主机序）*/
    /* 协商统计信息  接收*/
...
/* 外部通知IP地址增删事件 供BFD_IpAddrEventNotify和*/
typedef enum tagBFD_EVENT_E
{
    BFD_IPCHANGE_ADD_EVENT  = 0,                /* 增加IP地址 */
    BFD_IPCHANGE_DEL_EVENT,                     /* 删除IP地址 */
...
/* End of Added by w00207740, 2013/8/27 */
...
/*End of Modified by w00207740, 2014/2/19 */
/* End of Added by w00207740, 2013/8/27 */
...
typedef struct tagBFD_PPI_S
{
    ...
    ULONG ulDstIPAddress;           /* 对端IP地址,Big Endian */
    ULONG ulSrcIPAddress;           /* 本端IP地址,Big Endian */
    ULONG ulTTL;                    /* TTL */
...
typedef struct tagBFD6_PPI_S
{
    ...
    ULONG ulDstIPAddress[4];        /* 对端IP地址,网络序 */
    ULONG ulSrcIPAddress[4];        /* 本端IP地址,网络序 */
...
/* End of Added by w00207740, 2013/8/29 */
...
*  Description: 当外部添加、删除了一个IP地址时，通知BFD模块进行相应处理，比如：
*               一个会话的源地址被删除了，则该会话被停止；
*               若一个会话的目的地址被设置为了本地地址，则该会话被停止。
*               直接写消息和事件通知BFD任务处理;
*
*        Input: ulIpEvent  IP地址事件：添加、删除;0 表示增加1 表示删除
*               ulIpAddr   添加或删除的IP地址值；主机序；
...
*               ULONG ulDstIP: 接收报文的目的IP地址，网络序
*               ULONG ulSrcIP: 接收报文的源IP地址，网络序；
