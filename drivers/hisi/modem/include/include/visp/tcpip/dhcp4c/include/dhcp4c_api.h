...
/*Add by x57575 for Nodeb CR End*/
...
typedef struct tagDHCP4C_LEASE
{
    ...
    UCHAR pucChaddr[DHCP4C_CHADDR_LEN];   /* MAC地址 */
    UCHAR ucEnableFlag;      /* DHCP使能标志 */
    UCHAR ucPriority;        /* 地址优先级 */
    UCHAR ucPadding[2];
    ULONG ulInterfaceIPAddr; /* 接口IP地址，主机序 */
...
enum tagDHCP_ErrCode
{
    ...
    DHCP4C_CONFIGERR_CFGDHCPIP, /*15 配置DHCP IP地址失败*/
    ...
    DHCP4C_IP_ADDR_WRONG,       /*31 IP地址非法*/
...
*  Description: 配置DHCP的默认IP地址和子网掩码
*        Input: ULONG ulIfIndex: 接口索引号
*               ULONG ulIpAddr: 所添加的IP地址（主机序）
*               ULONG ulSubnetMask: 所添加的IP地址的子网掩码（主机序）
*       Output: 无
*       Return: 成功返回DHCP4C_SUCCESS,其他返回失败
*      Caution:
*------------------------------------------------------------------------------
...
*  Description: 清除DHCP的默认IP地址和子网掩码
*        Input: ULONG ulIfIndex: 接口索引号
*       Output: NA
*       Return: 成功返回DHCP4C_SUCCESS,其他返回失败
*      Caution:
...
*  Description: 获取DHCP的默认IP地址和子网掩码
*        Input: ULONG ulIfIndex: 接口索引
*       Output: ULONG *pulIpAddr: 默认IP地址（主机序）
...
*  Description: 显示DHCP的默认IP地址和子网掩码
*        Input: CHAR *pstIfName:接口名称
*       Output: 无
*       Return: VOID
*      Caution:
...
*       Return: DHCP4C_HA_IS_SMOOTHING:系统处于平滑或手动倒换阶段
                ...
                DHCP4C_CONFIGERR_CFGDHCPIP: 配置DHCP IP地址失败
...
*               DHCPC报文，但是在把从该DHCPC报文获取到的IP地址配置到接口前需要
