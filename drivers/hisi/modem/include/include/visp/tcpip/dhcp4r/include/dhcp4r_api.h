...
/* Added by likaikun 00213099,for DHCP relay request,2013/09/27 */
#define DHCP4R_AGENT_MAX_NUM        8
/* End of Added by likaikun 00213099,for DHCP relay request,2013/09/27 */
...
/*根据接口索引和Relay IP地址获取Relay请求报文源地址回调函数 
  传入参数: ulIfIndex:接口索引，用于支持多个接口使能DHCP Relay的情况
            ulRelayIP: 网关IP地址(主机序)，如果没有其它IP地址时，可以使用该地址做为报文的源地址
  返回值:   源地址(主机序)，如果返回的源地址为0或者指定的IP地址非本机IP，使用网关地址做为源地址
*/
...
/*根据接口索引和server IP地址获取Relay请求报文源地址回调函数 
