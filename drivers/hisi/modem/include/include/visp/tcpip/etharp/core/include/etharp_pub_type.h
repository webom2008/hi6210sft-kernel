...
/*Added by f54882, for A82D22647, 2008/01/17*/
/*ETHARP模块SID定义*/
...
/* ARP表项刷新标记 */
#define ETHARP_ARPUPDATE_BYFREEARP 0x00000001   /* ARP表项MAC因免费ARP而更新 */
#define ETHARP_ARPUPDATE_OUTIF     0x00000002   /* 静态ARP表项刷新出接口 */
...
{
    ULONG   ulIpAddr;                     /* 待应答的ARP请求中的目的IP地址 */
    struct tagARPTOREPLYINFO *pstNext;   /* 下一个待应答的ARP请求中的目的IP地址 */
}ARPTOREPLYINFO_S;
...
typedef struct tagARPRTENTRY
{
    ...
    /* Added by f54882, for A82D22687, 2008/1/18 */
    ...
    ULONG        ulArpUpdateFlag;          /* ARP表项有更新 */
...
typedef struct  tagMTSOFTCSUB
{
    ...
    ULONG                  ulLinkStatus;          /*描述接口的DOWN或UP状态*/
    ULONG                  ulNetProtocolStatus;  /*protocol link status of the interface except IP */
    /* Modified by f54882, for A82D22651, 2008/01/17 */
    ULONG           ulIP6LinkStatus;          /*描述接口的IPv6 DOWN或UP状态*/
    UCHAR           ucInterfaceID[8];          /* 接口ID */
...
typedef struct tagETHARP_MOD_INFO
{
    ...
    ULONG    ulBakMacSet; /*标识MAC地址的设置方式*/
    ...
    ULONG    ulArpMissNotNetTimeOut;  /*上报ARP MISS如果IP地址和接口不是同一网段,哑元的老化时间  */ 
    ULONG    ulChkPktSrcMac;          /*是否检查报文的源地址,1为检查,0为不检查,默认为不检查 */
    ULONG    ulFreeArpReplyUnicast;   /* 是否单播应答IP地址冲突的免费ARP报文 */
...
#define ETHARP_SCOPE_STATIC         1
#define ETHARP_SCOPE_DYNAMIC        2
#define ETHARP_SCOPE_ALL            3
#define ETHARP_SCOPE_THIS_INTERFACE 4
#define ETHARP_SCOPE_PER_INTERFACE  5
...
/* Add by z62474 for BC3D00621 2008-11-15 */
...
#define ETHARP_MAX_IP_CONFLICT_ITEM         8  /* 冲突IP地址最大数*/
#define ETHARP_MAX_IP_MAC_ITEM              4  /* 一个冲突IP对应Mac地址最大数*/
...
/* 使能VLANIF代理后，对同一个IP地址的最大待应答的ARP请求个数 */
#define ETHARP_MAX_COUNT_TOREPLY        64

/* IP地址冲突表项 */
typedef struct tagMacFlagItem
{
    UCHAR  ucMac[6];        /*与IP地址冲突对应的Mac地址*/
    UCHAR  ucFlag;          /*IP地址Mac地址确定的冲突项的冲突标志 ETHARP_STATE_CONFLICT, ETHARP_STATE_WAITFORREPLY */
    UCHAR  ucTimeCount;     /*计时器 单位s */
}ETHARP_MAC_Flag_Item_S;  
typedef struct tagIpConflictItem
{
    ULONG ulIPConflictType; /*冲突的IP地址类型:ETHIP(1);OMIP(2)*/
    ULONG ulIfIndex;        /*冲突的IfIndex*/    
    ULONG ulIpAddr;         /*冲突IP地址*/
    ULONG ulMacNum;         /*同一个冲突IP地址对应冲突Mac地址个数*/
    ETHARP_MAC_Flag_Item_S stMacFlagItem[ETHARP_MAX_IP_MAC_ITEM] ;/*一个IP地址可以检测4个与其冲突的Mac地址*/
    ULONG ulVrfIndex;
...
{
    UCHAR     aucDestAddr[6];/* Destination MAC address */
    UCHAR     aucSrcAddr[6]; /* Source MAC address */
