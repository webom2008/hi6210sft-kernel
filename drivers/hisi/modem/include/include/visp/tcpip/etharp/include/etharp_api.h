...
enum tagETHARPRetCode
{
    ...
    ARPSRCMAC_BROADCAST,            /*11,ARP源MAC地址为广播地址*/
    ARPSRCMAC_ALLZERO,              /*12,ARP源MAC地址为全零地址*/
    ...
    ETHARP_INVALID_IPADDR,          /*29,IP地址不合法*/
    ...
    ETHARP_ERROR_NOTSAMENETWITHVLANIF, /*53 IP地址和VLANIF不在同一网段内*/
    ...
    ETHARP_ERR_VIRTUAL_IP,          /* 61 IP地址是Backup路由器的虚IP地址 */
    ...
    ETHARP_GUARD_MAX_BLACKNUM_ONEIP,          /*88 同一个IP地址下的黑名单超出规格 */
    ...
    /* 2010-5-27,zKF23859,BC3D03074  */
    ETHARP_ERROR_VRF_NAMELONG=104,            /* 104, VRF名字长度大于31 */
    /* 2010-5-27,zKF23859 ,End */
...
#define ARP_TIMEOUT      1000      /*1000ms,为防止ARP恶意攻击,如果连续收到IP地址与本机冲突的ARP报文, 每隔1秒处理一次 */
...
{
    ULONG ulRtIpAddr;             /*用来保存IP地址*/
    UCHAR ucMacAddr[6];           /*用来保存MAC地址*/
    UCHAR ucPadding_1[2];
...
/* End of Added by z00208058, 2013/1/23 */
...
/* 是否是NAT IP地址回调函数数据结构定义,IP地址为网络序 */
...
typedef struct tagARPInfo
{
    ...
    ULONG     ulIpAddr;
    UCHAR     ucMac[6];
    /*Modify by Q62011 for BC3D00516 start ,08-10-23 */
    USHORT    usVID;
    /*Modify by Q62011 for BC3D00516 end ,08-10-23 */
    ULONG     ulArpType;
    /* Begin:VISP1.7C03 VRF wangchengyang , 2009-02-06 */
...
{
    ULONG ulIPConflictType; /* Conflicting IP address type: ETHIP (1); OMIP (2) */
    ULONG ulIfIndex;        /* Conflict IfIndex */
    ULONG ulIpAddr;         /* Conflicting IP address */
    ULONG ulMacNum;         /* IP address conflicts with an address corresponding to the number of conflicts Mac */
...
*       Author: wangchengyang
*  Description: 根据IP地址查询ARP表项
*        Input: ULONG ulIPAddr: 要查询的ARP表项的IP地址，主机序
...
*       Return: 0 成功/非0 错误码
*      Caution: DOPRA IP V300R001C10版本仅支持指定vrf、ip地址两个查询条件
*------------------------------------------------------------------------------
*  Modification History
*  DATE         NAME                    DESCRIPTION
*  ----------------------------------------------------------------------------
...
*  Description: 获取以太MAC地址
*        Input: ULONG ulIfIndex:接口索引
*       Output: UCHAR *pucMac:返回接口对应的MAC地址
*       Return: VOS_OK  操作成功
*               错误码  失败
*      Caution:
*------------------------------------------------------------------------------
...
*       Author: wuhailan
*  Description:
*        Input: ULONG ulIpAddr:IP地址(主机序)
*               UCHAR *pucMacAddr:MAC地址
...
*  Description: 配置指数退避算法发送免费ARP时同一IP地址发送的最大次数
...
*       Author: ZhengSenHuo/GeXianJun(104483/121208)
*  Description: 获取指数退避数法发送免费ARP时同一IP地址发送的最大次数
*                           和发送免费ARP时每秒发送的最大次数
*        Input:
*       Output: ULONG *pulTimes:   发送免费ARP时同一IP地址发送的最大次数
...
*       Author: wuhailan
*  Description: 设置是否处理IP地址为0的ARP请求报文开关
...
*  Description: 获取是否处理IP地址为0的ARP请求报文开关
*        Input:
...
* Date Created: 2008-10-28
*       Author: zhangchunyu(62474)
*  Description: 指定接口发送相应IP地址ARP请求,用户接口
*        Input: ULONG ulIfIndex: 接口索引
*               ULONG ulDstIpAddr: IP地址,网络序
*       Output:
...
*       Author: zhangchunyu(62474)
*      Purpose: 注册是否是NAT IP地址回调函数
*  Description: 如果接口使能NAT功能,支持接口回答外面ARP请求,即收到ARP请求判断是否是该接口NAT使用的IP地址,
*               如果是则回应本接口的MAC
