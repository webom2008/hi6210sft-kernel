...
/* ICMP_SRCADDR_ANY并不是协议规定的ICMP类型,这里只是为了攻击防范时便于处理,
   将其与ICMP的类型统一编码 */
#define     ICMP_SRCADDR_ANY          20    /* ICMP报文源地址为 0.0.0.0 */
...
typedef struct  tagICMPFlowStat_Echo
{
    ...
    ULONG ulRemoteIPAddr;       /* Peer IP address, host order */
...
{
    ULONG ulLocalIPAddr;        /* The end of the IP address, host sequence */
    ULONG ulRemoteIPAddr;       /* peer IP address, host sequence */
