...
/* End of Added by z00208058, 2012/12/6 */
...
extern ULONG TCPIP_HA_GetPhyInitByType(ULONG ulType, UCHAR * szIfName, ULONG *pfPhyInitial);
...
extern ULONG IF_CloseIfEntryByDepth(ULONG ulWaitListHandle);
extern ULONG IF_OpenIfEntryByDepth(ULONG *pulWaitListHandle);
extern ULONG IF_GetIfEntryByDepth(ULONG ulEntryWaitList, IFNET_S ** ppstIfConfig);
/*f47854 modify : 函数参数或者返回值类型与函数指针定义不匹配*/
extern VOID * IF_GetNextIfByDepth(VOID *pParam,VOID **pNewParam);
extern ULONG IF_OpenDeviceNodeTable (ULONG *pulWaitListHandle,ULONG ulFirstDim);
extern ULONG IF_GetDeviceNode (ULONG ulEntryWaitList, DEVLINK_S  *pstDevNode);
extern ULONG IF_CloseDeviceNodeTable( ULONG ulWaitListHandle );
