 ...
 */
#define IP_ADD       0x0100   /* 添加IP地址 */
#define IP_DEL       0x0200   /* 删除IP地址 */
#define IP_CHA       0x0400   /* 改变IP地址 */
...
#define IP_DELSEC     512   /* 删除从IP地址,通知Eth删除ARP表项 */
...
/* Added by f54882, for A82D21769, 2007/12/07 */
#define IP_BORROW_FLAG   0x08 /* 该标记表示不与借用地址进行冲突检测 */

#define DHCP4C_CFG_FLAG   0x100  /*标记DHCP4C可以获取相同的网段的IP地址*/
...
typedef struct tagIPCMDMSG
{
    ...
    ULONG   ipif_ulAddrFlag;    /* 接口IP地址的特性：地址协商、借用IP地址、有自己的IP地址、IFF_NEGOTIATE,*/
    ...
    IPIFADDR_S  ipif_stIpIfAddrList[MAXADDR_ON_ONEIF]; /* 接口的单播IP地址链：双向链表 */
    IPMULTI_S   ipif_stIPIfMultiList[1];    /*接口的多播IP地址链:  循环链表*/
}IP_IPCMDMSG_S;
...
/*
 * changed for permitting ip address overlap 2002-10-30 by mayun & yuyong
 * now use radix tree
 */
...
ULONG IP_Del_AllOfIfIpAddr( IPIFNET_S *pstIpIf );
...
VOID DealWithOthersBorrowMyIPaddr(IPIFNET_S *pstCurIf, ULONG ulCmd);
