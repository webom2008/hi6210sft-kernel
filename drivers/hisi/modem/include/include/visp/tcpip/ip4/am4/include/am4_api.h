...
enum enum_IP_AM4_ERR
{
    ...
    ERR_AM4_INVALIDMAC,                      /* 16 无效的MAC地址 */
    ...
    ERR_AM4_VRRP,                            /* 23 某些不合法的虚拟IP地址已被删除 */
...
* Date Created: 2009-02-02
*       Author: q62011
*  Description: 根据指定的IP地址查找全局IP地址链,判断该IP地址是否是本地某个接口的IP地址
...
/*****************************************************************************
 ...
 功能描述  : 判断IP地址是否是给定接口上的IP地址
 输入参数  : ULONG ulIfIndex  
             ULONG ulIpAddr   
 输出参数  : ULONG *pulFlag : 
            VOS_TRUE:IP地址在指定的接口上
            VOS_FALSE:IP地址不在接口上
 返 回 值  : 
 调用函数  : 
 被调函数  : 
...
*  Description: 获取接口IP地址的特性：地址协商、借用IP地址、有自己的IP地址
*        Input: ULONG ulIfIndex:    接口索引
*       Output: ULONG *pulAddrFlag: 接口IP地址特性
*       Return: 成功VOS_OK,其他返回失败
*      Caution: 
*------------------------------------------------------------------------------
*  Modification History
