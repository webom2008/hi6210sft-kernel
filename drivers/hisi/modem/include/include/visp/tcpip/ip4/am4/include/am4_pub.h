...
typedef struct tagIPIFADDR
{
    ...
    /*f47854 add begin:IP地址描述信息*/
    char ia_description[48];
    /*f47854 add end:IP地址描述信息*/
...
typedef struct tagIPIFNET
{
    ...
    ULONG   ipif_ulAddrFlag;    /* 接口IP地址的特性：地址协商、借用IP地址、有自己的IP地址 
                                   IFF_NEGOTIATE,*/
    ...
    IPIFADDR_S  ipif_stIpIfAddrList;    /* 接口的单播IP地址链：双向链表 */
    IPMULTI_LIST_S   *ipif_pMultiAddrList;   /* 接口的多播IP地址链 */
    ...
    ULONG      ipif_ulNODHCPFlag;       /* 接口地址优先级高于DHCP获得地址的优先级的标志位，即如果接口上有
                                           IP地址则DHCP学习的地址不会配置到接口上，2006-06-03 */
...
typedef struct tagIP_L3VPN_VRF
{
    ...
    IPIFADDR_S             * pVRFIPIfAddr;      /* IP Address List */   
...
/* is the ip address and mask invalid on the interface? */
extern ULONG IP_IsInvalidIPAddress( struct tagIFNET *pstIfNet, ULONG ulAddr, ULONG ulMask );

/* does the ip address and mask conflict with other configured address? */
...
/* get the main ip address of the interface, only OWN address */
...
/* search the longest match IP address of the interface */
extern ULONG IP_IF_MaxIpAddr( struct tagIFNET *pstIfNet, ULONG ulIpAddr );
/* search the longest match IP address on interface */
