...
/*add by x36530 for adjust the directory vipip 20041130*/
...
{
    ULONG ulSourceAddr;                               /* 接收到的多播数据包中的IP地址   IP address that is in the multicast packet recieved*/
...
typedef struct tagIPMREQ
{
    ...
    ULONG   imr_ulIndexIf;                          /* 接口的本地地址      local IP address of interface */
...
extern USHORT Ip_Fast_Cksum(USHORT * pusWord);/*add by y36299 for Head  Cache 2005/04/11*/
/* 多播  multicast */
...
extern USHORT IP_Get_IpId(VOID);/*added by l61496 2007-8-1*/
