...
typedef struct tagDHCP6C_ServerInfo
{
    ...
    IN6ADDR_S           stServerIPAddress;     /* server ip地址 */
...
*        Input: ULONG ulIfIndex:      接口索引<非0>
*               IN6ADDR_S *pstAddress:     用户期望的DNS Server IP地址数组，如果用户不清楚则设置为空指针，
*                                此时完全由DHCPv6 Server负责分配
*               ULONG *ulAddressCnt:   DNS Server IP地址个数，对于删除操作该值必须为1
...
*       Author: wujie 61195
*      Purpose: NA
*  Description: 获取用户配置的DNS Server IP地址列表
*        Input: ULONG ulIfIndex:      接口索引<非0>
*       Output: IN6ADDR_S **ppstAddress:    DNS Server IP地址数组，*ppstAddress指向的内存由VISP申请，调用者释放<非空>
