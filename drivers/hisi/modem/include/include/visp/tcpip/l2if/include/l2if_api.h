...
typedef enum enumL2IfErrCode
{
    ...
    L2IF_ERR_VLAN_SUPER,              /* 14 VLAN为SUPER VLAN                        */
    ...
    L2IF_ERR_VLAN_MAXSUPER,           /* 24 Super Vlan个数已达到1024                */
    L2IF_ERR_VLAN_MAXSUB,             /* 25 加入Super VLAN的Sub VLAN个数已达到1024  */
    L2IF_ERR_SUBVLAN_INOTHERSUPER,    /* 26 Sub Vlan已加入其他Super Vlan            */
    L2IF_ERR_VLAN_SUB,                /* 27 VLAN类型为Sub VLAN                      */
    L2IF_ERR_SUBVLAN_NOTINSUPER,      /* 28 Sub VLAN没有加入Super VLAN              */
    L2IF_ERR_VLAN_SUPERNOUP,          /* 29 UP sub vlan时候,super vlan还没有up      */
    
    L2IF_ERR_VP_NULLPOINT,            /* 30 VP输入参数指针为空                      */    
    L2IF_ERR_PORT_NULLPOINT,          /* 31 PORT输入参数指针为空                    */
    L2IF_ERR_PORT_GETIFFAIL,          /* 32 获取ifnet指针失败                       */
    L2IF_ERR_VLAN_NOT_SUPER,          /* 33 VLAN 不是super vlan                     */
    ...
    L2IF_ERR_VP_ISSUPER,              /* 43 super vlan不允许端口移除操作            */
...
{
    L2IF_MAC_LEARNING_DISABLE = 0,   /* MAC地址学习禁止 */
    L2IF_MAC_LEARNING_ENABLE  = 1    /* MAC地址学习使能 */
}L2IF_MACLEARN_E;
...
typedef struct tagTCPIP_L2IF_VLAN_INFO
{
    ...
    UCHAR  ucMacLearning;          /* MAC地址学习,缺省值MAC_LEARNING_ENABLE */
    UCHAR  ucBroadcast;            /* VLAN内广播报文的处理方式 */
    UCHAR  ucUnkownMulti;          /* VLAN内未知多播报文的处理方式，不可配置 */
    UCHAR  ucUnkownUniFrame;       /* VLAN内未知单播报文的处理方式 */
    UCHAR  ucReserved[1];
    USHORT usSuperVlan;            /* 对应的super-VLAN的VLAN ID,初始值VLANID_INVALID*/
    USHORT usSubCount;             /* 对于Super VLAN而言，包含的Sub VLAN的个数 */    
    USHORT usSubVlanID[L2IF_MAX_SUB_VLAN]; /* Super Vlan 下所有Sub Vlan ID列表 */
...
typedef struct tagTCPIP_L2IF_PORT_INFO
{
    ...
    UCHAR  ucMacLearning;            /* 端口的MAC地址学习使能标志 */
...
*
*******************************************************************************/
extern ULONG TCPIP_OpenVlanQinQTable(ULONG *pulWaitlist);
...
*
*******************************************************************************/
extern ULONG TCPIP_GetVlanQinQTable(ULONG ulWaitlist,ULONG ulTransNum, 
...
*
*******************************************************************************/
extern ULONG TCPIP_CloseVlanQinQTable(ULONG ulWaitlist);
...
* Date Created: 2008-11-20
*       Author: zhangchi(00142640)
*  Description: 根据MAC地址和VLAN ID删除MAC表项
*        Input: UCHAR *aucMacAddr: MAC地址
*               USHORT usVlanId:   VLAN ID
*       Output: NA
*       Return: 成功返回L2IF_OK，失败返回错误码
...
*  Description: 根据MAC地址和VLAN ID来获取表项
*        Input: UCHAR* aucMacAddr:MAC地址
*               USHORT usVlanid:VLAN ID
...
*
*******************************************************************************/
extern ULONG TCPIP_FDB_OpenWtLstObject(ULONG *pulWaitListHandle);
...
*
*******************************************************************************/
extern ULONG TCPIP_FDB_CloseWtLstObject(ULONG ulWaitListHandle);
...
*
*******************************************************************************/
extern ULONG TCPIP_FDB_GetWLStaticByIfIndexPlusVlan(ULONG ulWaitlist, ULONG ulIfIndex, USHORT usVlanId, 
...
*
*******************************************************************************/
extern ULONG TCPIP_FDB_GetWLBlackholeByVlan(ULONG ulWaitlist, USHORT usVlanId, 
    TCPIP_FDB_ENTRY_S* pstFdbEntry, ULONG *pulFdbNum);
