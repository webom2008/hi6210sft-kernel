...
typedef struct tagPoesHisMac
{
    ...
    UCHAR      aucHisMacAddr[POES_MAC_LEN]; /* 对端MAC地址 */
    USHORT     usSessionNum;/* 与对端MAC地址建立Session的数目 */
} POES_HIS_MAC_S;
...
typedef struct tagPoesSb
{
    ...
    POES_HIS_MAC_S *pstHisMac;      /* 对端MAC地址控制块指针 */
...
typedef struct tagPoesCb
{
    ...
    UCHAR                aucOurMacAddr[POES_MAC_LEN]; /* 本接口的MAC地址 */
