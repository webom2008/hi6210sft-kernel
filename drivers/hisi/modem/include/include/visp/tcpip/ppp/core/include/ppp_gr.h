...
enum enumPppGRERR
{
    ...
    GR_ERR_CHECK_IPCP_NOT_ADDR,                            /* 31 主机地址和底层获取的IP地址不相同 */
    GR_ERR_CHECK_IPCP_ADDR,                                /* 32 本端和对端的协商到的彼此IP地址不一致 */
...
{\
    (VOID)VOS_Mem_Zero(pstOption, sizeof(optionType));\
...
VOID PPP_GR_DealMsg(ULONG aulMsg[4]);
