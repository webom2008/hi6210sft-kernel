...
typedef enum enPppProErrorInfo
{
    ...
    PPP_CIPHERPASSWORD_WRONGLONG,       /* 40 密码过长 */
...
#define DHCP4C_DNSIP_NUM       6 /* dns server IP地址个数 */
#endif
...
#define PPP_IPCONFLICT                  0x0002  /* PPP的IP地址协商冲突告警 */
...
typedef struct tagPppAuthMsg
{
    ...
    UCHAR ucPwdLen;
    CHAR szResponse[LEN_256];/*根据用户名取密码时为密码，chap验证结果时为生成的response*/
    UCHAR ucResponseLen;
}PPPAUTHMSG_S; 
...
typedef struct tagPppIp6cpOption
{
    ...
    ULONG   neg_interfaceid :  1,   /* Negotiate IP Address? */
            neg_compprotocol:  1,
    ...
    ULONG   bReserve        :  29,  /* 为了以后扩展预留协商位 */
            nak_isok        :  1,   /* Use old (IP-Addresses) option? */
            neg_compprotocol:  1, 
            neg_interfaceid :  1;   /* Negotiate IP Address? */
#endif
...
typedef struct tagPPPNEGORESULT
{
    ...
    /* IPCP */
    ULONG  ouraddr;              /* 本端IP地址 */
    ULONG  hisaddr;              /* 对端IP地址 */
    ULONG  iphc_protocol;
    USHORT us_tcp_space;
    USHORT us_non_tcp_space;
    USHORT us_f_max_period;
...
typedef struct tagPppConfigInfo 
{
    ULONG bCallbackReq    : 1,
          ...
          bAuthRmtPap     : 1,
          bEnableVJComp   : 1,
          /* deleted by z43740 2006/05/08 */
          /*bEnableComp     : 1,*/
          /* Added by z43740 for PFC/ACFC 2006/05/08 */
          bEnablePComp    : 1,
          bEnableACComp   : 1,
          /* End of addition */
          bSetAsMp        : 1,
          ...
          bPppReorderWindow  : 1, /*Added for BYDD08873 by Z34981, 2004/08/30*/
          bMpDiscriminator:1,   /* SWFD10247 2005.06.11  discriminator negotiation,默认使能,class:1(本地随机分配6字节), */

          /*Added by z43740 for IPOS D00042,2005/12/29*/
          bNegoLCPEnable:  1,   
          bNegoNCPEnable:  1,
          /*End for IPOS D00042*/
    ...
    /* Added by z43740 for IPHC,2005/09/19 */
...
*  Description: 设置chap验证时的用户名和认证密码
*        Input: ULONG ulIfIndex:接口索引
*               ULONG ulSetYes:是否设置：0为否；1为是
*               ULONG usEncrypt:口令是否加密，1为设置加密，0为取消
*               UCHAR *pcP***W***:待设置的密码指针
...
*  Description: 设置PAP验证的用户名和口令
*        Input: ULONG ulIfIndex:接口索引
*               ULONG ulSetYes:是否设置的标志：1为使用，0为取消，其他非法；如果设置取消则后面三个参数不用
*               ULONG usEncrypt:口令是否加密，1为设置加密，0为取消
*               UCHAR *pcP***W***:口令内容（有效长度非加密口令1～16，或加密口令24）
...
*  Description: 设置/删除IPCP协商的DNS服务器IP地址，提供给对端
*        Input: ULONG ulIfIndex:接口索引
*               ULONG ulSetYes:设置标志，1配置，0 删除
*               ULONG ulNum_Pri:主DNS服务器IP地址
*               ULONG ulNum_Sec:从DNS服务器IP地址
*       Output: NA
*       Return: VOS_OK                      成功
*               PPP_CONFIG_IFCONFLICT       接口失败
*               PPP_WRONGPARA_ERROR         输入的参数错误
*               PPP_HA_IS_SMOOTHING         HA正在平滑处理或手动倒换处理
*               PPP_DNSIPADDRESS_INVALID    配置的主从DNS服务器IP地址无效
*               PPP_DNSIPADDRESS_PRI_IS_INVALID 配置的主DNS服务器IP地址无效
*               PPP_DNSIPADDRESS_SEC_IS_INVALID 配置的从DNS服务器IP地址无效
*               Other                       获取PPP信息块的错误码
*      Caution: 
*------------------------------------------------------------------------------
*  Modification History
...
*  Description: CHAP验证时根据用户名获取用户的口令
...
*               ULONG ulPriDns:主DNS服务器IP地址(主机字节序)
*               ULONG ulSecDns:次DNS服务器IP地址(主机字节序)
*       Output: NA
*       Return: VOS_OK:处理成功
*              其它  :处理失败
*      Caution: 
