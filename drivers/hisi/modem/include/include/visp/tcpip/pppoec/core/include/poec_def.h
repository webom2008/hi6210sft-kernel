...
#define PPPOE_STATE_PPP_UP      4   /*session阶段ppp协商已经通过，可以传输网络数据*/

/*Add for VISP3 by t45114, 2005/09/15*/
...
/*将IP地址ULONG型转换成SOCKADDR_S类型(网络或主机顺序不同类转换)*/
...
#define PPPOE_CLIENT_ERR_SOURCE_MAC           0x01000003  /*源MAC地址非法*/
 ...
 *-----------------------------------------------------------*/
/* Modified by f54882, for A82D22647, 2008/1/17 */
...
{\
    (stSessionInfo).ulEthIfIndex = ((TCPIP_HA_GetBoardType() == TCPIP_HA_AMB) ? \
                                    (pstSession)->pstEthIf->if_ulIfIndex : (ULONG)(pstSession)->pstEthIf);\
