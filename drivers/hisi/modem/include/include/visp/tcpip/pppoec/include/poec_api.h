...
typedef enum tagPOEC_EXEC_INFO_INDEX
{
    ...
    PPPOE_CLIENT_NP_MAC_INVALID,        /* 40.底层的会话信息中保存的MAC地址非法 */
    PPPOE_CLIENT_NP_MAC_DIFF,           /* 41.底层的会话信息中保存的本端MAC地和上层Eth接口的MAC地址不一致 */
...
typedef struct tagPPPOE_C_SESSION_S
{
    ...
    UCHAR   szOurMacAddr[PPPOE_MAC_LEN];  /*Client MAC地址 */
    UCHAR   szHisMacAddr[PPPOE_MAC_LEN];  /*Server MAC地址 */
