...
/*PTP  端口的状态*/
#define  TCPIP_PTP_PORT_STATE_MASTER    1
#define  TCPIP_PTP_PORT_STATE_SLAVE     2
#define  TCPIP_PTP_PORT_STATE_PASSIVE   3  /*BC中 系统没有选到源时候端口的状态*/
...
typedef struct TagTCPIP_PTP_PARENT_DS
{
    ...
    BOOL_T      bFrequencyTraceable;    /*是否跟踪到主时钟频率*/
    BOOL_T      bTimeScale;
...
typedef struct tagTCPIP_PTP_SLAVE_PPI
{
    ...
    TCPIP_PTP_PORT_ADDR_S stLocalAddr;      /*本IP地址*/
...
*       Return: 成功返回PTP_OK，失败返回错误码
*      Caution: 1.目前仅支持Overmac，基于接口配置不需要指定MAC地址
*               2.OC下，返回的端口号只能为1
*        Since: V300R002C10
*    Reference: NA
*------------------------------------------------------------------------------
...
*               3. pstPtpAddr仅为未来扩展预留，在OC下，同一个接口不同的IP地址可以配置PTP应用
*        Since: V300R002C10
*    Reference: NA
*------------------------------------------------------------------------------
*  Modification History
