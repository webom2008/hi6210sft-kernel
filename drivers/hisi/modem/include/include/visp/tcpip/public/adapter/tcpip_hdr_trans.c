...
{
#if((TCPIP_FOR_HERT == VRP_NO) && (TCPIP_FOR_BALONG == VRP_NO))
...
*  Description: 把ULONG的ip地址转换成点分十进制
*        Input: ULONG ulAddr:IP地址(必须为主机序)
*       Output: CHAR *szStr:点分十进制IP地址
*       Return: 点分十进制IP地址
*      Caution: 
*------------------------------------------------------------------------------
*  Modification History
...
VOID HDR_WriteBinaryFile(UCHAR* pHDRMsg, ULONG ulMsgLen, ULONG ulMsgType, ULONG ulScanId)
{
    ...
    {
        (VOID)sprintf(szFileName,"e:/tcpiphealth%d.file",ulScanId);
    }
    else
    {
        (VOID)sprintf(szFileName,"e:/tcpiplastword.file");
    }
...
ULONG HDR_GetBeginLineByType(FILE *fp,ULONG ulSubType,ULONG *pulSubLen,UCHAR *uszInfo)
{
    ...
    while (1)
    {
        ...
        (VOID)memcpy(ucSubTemp,(pcCurStr + strlen("BEGIN:")), pcNextStr - (pcCurStr + strlen("BEGIN:")));
        ...
        if (ulCfgSubType == ulSubType)
        {
            ...
            if (NULL != (pcNextStr = strstr(pcCurStr,",")))
            {
                (VOID)memcpy(ucSubTemp,pcCurStr, pcNextStr - pcCurStr);
            ...
            if (NULL != (pcCurStr = strstr(pcCurStr,"\"")))
            {
                ...
                {
                    (VOID)memcpy(uszInfo,(pcCurStr + 1), pcNextStr - (pcCurStr + 1));
...
VOID HDR_TranslateByCfg(HDR_MSG_HEAD_S *pstHdrComHead, FILE *fpHdrFile, FILE *fpCfgFile, ULONG ulScanId)
{
    ...
    {
        sprintf(szResualt,"tcpiphealthresualt%d.txt",ulScanId);
        ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet, "\r\n%30s","Print health info:");
    ...
    while (ulOffLen > 0)
    {
        ...
        ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%30s%s\r\n","SubStructInfo:",ucDesc);
        ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%30s%u\r\n","SubMsgType:",ulSubType);
            ...
            if (NULL != (pcCurStr = strstr(szLine,",")))
            {
                if (NULL != (pcNextStr = strstr(pcCurStr + 1,",")))
                {
                   (VOID)memcpy(szSubTemp,pcCurStr + 1,(pcNextStr - (pcCurStr + 1)));
                    ...
                    if (NULL != (pcNextStr = strstr(pcCurStr,",")))
                    {
                        (VOID)memcpy(szSubTemp,pcCurStr + 1,(pcNextStr - 1 - (pcCurStr + 1)));
                        ...
                        if (NULL != (pcNextStr = strstr(pcCurStr,"\"")))
                        {
                            (VOID)memcpy(ucDesc,pcCurStr,pcNextStr - pcCurStr);
                            ucDesc[pcNextStr - pcCurStr] = 0;
                            ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%30s:",ucDesc);
                        }
                        ...
                        if (NULL != (strstr(szSubTemp,"%d")))
                        {
                            for (ulLoop = 0;ulLoop < ulTypeLen;ulLoop++)
                            {
                                ...
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%d ",HDR_toHL(*((ULONG*)(pucSubValue) + 
                                    ulLoop)));
                                }
                                else if (HDR_TYPE_USHORT == ulFlag)
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%d ",HDR_toHS(*((USHORT*)(pucSubValue) + 
                                    ulLoop)));
                                }
                                else if (HDR_TYPE_BOOL == ulFlag)
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%d ",*((USHORT*)(pucSubValue) + ulLoop));
                                }
                                else
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%d ",*(pucSubValue + ulLoop));
                                }
                            }
                            ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"\r\n");
                        }
                        else if (NULL != (strstr(szSubTemp,"%u")))
                        {
                            for (ulLoop = 0;ulLoop < ulTypeLen;ulLoop++)
                            {
                                if (HDR_TYPE_ULONG == ulFlag)
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%u ",HDR_toHL(*((ULONG*)(pucSubValue) + 
                                    ulLoop)));
                                }
                                else if (HDR_TYPE_USHORT == ulFlag)
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%u ",HDR_toHS(*((USHORT*)(pucSubValue) + 
                                    ulLoop)));
                                }
                                else if (HDR_TYPE_BOOL == ulFlag)
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%u ",*((USHORT*)(pucSubValue) + ulLoop));
                                }
                                else
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%u ",*(pucSubValue + ulLoop));
                                }
                            }
                            ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"\r\n");
                        }
                        else if (NULL != (strstr(szSubTemp,"%lu")))
                        {
                            for (ulLoop = 0;ulLoop < ulTypeLen;ulLoop++)
                            {
                                if (HDR_TYPE_ULONG == ulFlag)
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%lu ",HDR_toHL(*((ULONG*)(pucSubValue) + 
                                    ulLoop)));
                                }
                                else if (HDR_TYPE_USHORT == ulFlag)
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%lu ",HDR_toHS(*((USHORT*)(pucSubValue) + 
                                    ulLoop)));
                                }
                                else if (HDR_TYPE_BOOL == ulFlag)
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%lu ",*((USHORT*)(pucSubValue) + ulLoop));
                                }
                                else
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%u ",*(pucSubValue + ulLoop));
                        ...
                        else if (NULL != (strstr(szSubTemp,"%x")))
                        {
                            for (ulLoop = 0;ulLoop < ulTypeLen;ulLoop++)
                            {
                                if (HDR_TYPE_ULONG == ulFlag)
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%#x ",HDR_toHL(*((ULONG*)(pucSubValue) + 
                                    ulLoop)));
                                }
                                else if (HDR_TYPE_USHORT == ulFlag)
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%#x ",HDR_toHS(*((USHORT*)(pucSubValue) + 
                                    ulLoop)));
                                }
                                else if (HDR_TYPE_BOOL == ulFlag)
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%#x ",*((USHORT*)(pucSubValue) + ulLoop));
                                }
                                else
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%#x ",*(pucSubValue + ulLoop));
                                }
                            }
                            ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"\r\n");
                        }
                        else if (NULL != (strstr(szSubTemp,"%c")))
                        {
                            ...
                            ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%s",szTemp);
                        ...
                        else if (NULL != (strstr(szSubTemp,"%s")))
                        {
                            ...
                            memcpy(szTemp,pucSubValue,ulTypeLen);
                            ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%s",szTemp);
                            ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"\r\n");
                        }
                        else if (NULL != (strstr(szSubTemp,"%mac")))
                        {
                            ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%02x-%02x-%02x-%02x-%02x-%02x",
                        ...
                        else if (NULL != (strstr(szSubTemp,"%ip")))
                        {
                            ...
                            ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%s",szIp);
                            ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"\r\n");
                        }
                        else if (NULL != (strstr(szSubTemp,"%ifname")))
                        {
                            ...
                            memcpy(uszName,pucSubValue,IF_MAX_INTERFACE_NAME_LEN + 1);
                                
                            pstNode = (HDR_IFNAMEANDINDEX_S*)malloc(sizeof(HDR_IFNAMEANDINDEX_S));
                            if (pstNode != NULL)
                            {
                                memset(pstNode,0,sizeof(HDR_IFNAMEANDINDEX_S));
                                memcpy(pstNode->szIfName,uszName,IF_MAX_INTERFACE_NAME_LEN + 1);
                            ...
                            ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%s",uszName);
                            ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"\r\n");
                        ...
                        else if (NULL != (strstr(szSubTemp,"%index")))
                        {
                            ...
                            for (ulLoop = 0; ulLoop < ulTypeLen; ulLoop++)
                            {
                                ...
                                while(pstNode != NULL)
                                {
                                    ...
                                    {
                                        ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%s ",pstNode->szIfName);
                                        ulFindFlag = 1;
                                        break;
                                    }
                                ...
                                if (0 == ulFindFlag)
                                {
                                    ulOutOffSet += sprintf(pcOutInfo + ulOutOffSet,"%#x ",HDR_toHL(ulIndex));
                                }
                            }
