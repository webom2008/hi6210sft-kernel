...
typedef struct tagMBufIpPacketInfo
{
    ...
    ULONG ulNextHopIpAddress;/*下一跳的IP地址，从路由表中获得*//*ethernet*/
...
typedef struct tagMBufIp6PacketInfo
{
    ...
    */
    ...
    struct {
        UCHAR ucaddr[16];
    }stNextHopIpAddr;       /*下一跳的IP地址，从路由表中获得*/
    /*End of Modified by z00208058/w00207740, 2012/5/8 */
    
    /* 链路层地址 */
    union
    {
        UCHAR szLinkEtherAddr[MBUF_MACADDRLEN]; /* MAC地址 */
    }unLinkAddr;
...
typedef struct tagMBUF_UserTagData/*this data type is public, can be used by user*/
{
    ...
    /*与物理层相关的信息，例如以太网的发送者的MAC地址，X.25的线路号等*/
...
extern ULONG VOS_Mem_Copy(VOID * pDestPtr,const VOID * pSrcPtr, SIZE_T ulCopyLen);
...
/*申请Mbuf头*/
/* Modify by z62474 for BC3D00880 2008-12-12 */
...
\
    ...
    if(NULL != pstMBufHead)\
    {\
        (VOID)VOS_Mem_Zero((VOID *)pstMBufHead, MBUF_MBUF_SIZE); \
...
\
    ...
    if((pstMBufDBD) != NULL)\
    {\
        (VOID)VOS_Mem_Zero((VOID *)pstMBufDBD, MBUF_DATA_BLOCK_DESC_SIZE); \
...
/* Modify by z62474 for BC3D00396 2008-09-20*/
...
{\
    ...
    {\
        ...
        \
        (VOID)VOS_Mem_Set( (VOID *) (& pstMBufM->stUserTagData), 0 , sizeof(MBUF_USERTAGDATA_S));\
...
/* Added by z43740 for receive arp packet notify,2009-01-03 */
