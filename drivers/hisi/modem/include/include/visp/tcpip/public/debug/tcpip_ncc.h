...
typedef enum enNCC_DiagnoseInfo
{
    ...
    NCC_INFO_PPP_NOCONFIGIP,                /* 接口上未配置IP地址导致IPCP协商不过 */
    NCC_INFO_PPP_IPCP_UPCHECK_GOTIP_ZERO,   /* 本端配置为协商获取地址,但是协商获得的IP地址为0 */
    NCC_INFO_PPP_IPCP_UPCHECK_GOTIP_INVALID,/* 本端配置为协商获取地址,但是协商获得的IP地址非法(无效IP) */
    NCC_INFO_PPP_IPCP_UPCHECK_HISIP_INVALID,/* 对端配置为协商获取地址,但是对端协商获得的IP地址非法(无效IP) */
    NCC_INFO_PPP_IPCP_IPCONFLICT_LOCALSAME, /* 本端配置为协商获取地址,但是协商获得的IP地址与本端其他IP相同(冲突) */
...
typedef enum enNCC_PPPIPCPDOT
{
    ...
    IPCP_IPNEG_NOIPADDR,            /* 本接口未配置IP地址导致不进行IPCP的协商,从而IPCP协商不过 */

    IPCP_IPCONFLICT_LOCALSAME,      /* IP地址协商过程中协商得来地址与本端其他IP冲突 */
    IPCP_UPCHECK_GOTIPCONFLIT_MODE, /* IP地址协商过程中对端给本端分配IP，但本端有自己IP导致冲突 */
    IPCP_UPCHECK_IPCONFLIT_PEERSAME,/* IP地址协商过程中协商得到的两端IP冲突 */
    IPCP_UPCHECK_HISIPCONFLIT_MODE, /* IP地址协商过程中本端给对端分配IP，但对端有自己IP导致冲突 */

    IPCP_UPCHECK_GOTIP_ZERO,        /* IP地址协商过程中协商得到的本端IP地址为0 */
    IPCP_UPCHECK_GOTIP_INVALID,     /* IP地址协商过程中协商得到的本端IP地址无效 */
    IPCP_UPCHECK_HISIP_INVALID,     /* IP地址协商过程中协商得到的对端IP地址无效 */
...
extern VOID NCC_TaskMain(ULONG ul1, ULONG ul2, ULONG ul3, ULONG ul4);
