...
typedef enum enTCPIP_NCC_DIAGNOSEINFO_TYPE
{
    ...
    NCC_INFO_RESULT_PPP_NOCONFIGIP,                 /* 133 接口上未配置IP地址导致IPCP协商不过 */
    NCC_INFO_RESULT_PPP_IPCPNEG_GOTIP_ZERO,         /* 134 本端配置为协商获取地址,但是协商获得的IP地址为0 */
    NCC_INFO_RESULT_PPP_IPCPNEG_GOTIP_INVALID,      /* 135 本端配置为协商获取地址,但是协商获得的IP地址非法(无效IP) */
    NCC_INFO_RESULT_PPP_IPCPNEG_HISIP_INVALID,      /* 136 对端配置为协商获取地址,但是对端协商获得的IP地址非法(无效IP)*/
    NCC_INFO_RESULT_PPP_IPCPNEG_IPCONFLICT_LOCALSAME,/* 137 本端配置为协商获取地址,但是协商获得的IP地址与本端其他IP相同
                                                            (冲突) */
