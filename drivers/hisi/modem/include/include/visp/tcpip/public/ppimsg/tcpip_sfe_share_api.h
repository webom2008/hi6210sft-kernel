...
typedef struct tagTCPIP_SFE_INTF
{
    ...
    UINT8    u8MacAddr[TCPIP_SFE_MAC_LEN];/* MAC地址 */
...
typedef struct tagTCPIP_SFE_ROUTE
{
    ...
    UINT32    u32DstIp;             /* 目的IP地址,主机字节序,用户必须添加 */
    ...
    UINT8     u8DstPrefixLength;    /* 目的IP地址掩码长度，用户必须添加 */
...
typedef struct tagTCPIP_SFE_BFD_SESS_INFO
{
    ...
    TCPIP_SFE_IPADDR_UN unDstIPAddr;     /* 对端IP地址,主机序 */
    TCPIP_SFE_IPADDR_UN unSrcIPAddr;     /* 本端IP地址,主机序 */
    UINT32 u32TTL;                       /* TTL */
    ...
    INT8 szCtlPkt[SFE_BFD_LEN_AUTH_NONE];/* 会话的BFD控制报文,网络序,值为用户配置值 */
