...
enum tagPPIOprType
{
    ...
    PPI_IF_SETMAC,              /* 13 设置TRUNK接口的mac地址 */
    ...
    PPI_VLAN_ADDSUB,                        /* 59 向指定的Super VLAN中添加Sub VLAN*/
    PPI_VLAN_REMOVESUB,                     /* 60 从指定的Super VLAN中删除指定的Sub VLAN*/
    PPI_VLAN_SET_TYPE,                      /* 61 设置VLAN类型，包括Super/Sub/Commom等*/
    ...
    PPI_VLAN_SET_MACLEARN_MODE,             /* 65 使能/禁止VLAN内的MAC地址学习*/
    ...
    PPI_MULTICMAC_ADD,                      /* 97 增加多播MAC地址 */
    PPI_MULTICMAC_DEL,                      /* 98 删除多播MAC地址 */

    PPI_TRUNK_ADDPORT,                      /* 99 TRUNK添加绑定端口 */
    PPI_TRUNK_REMOVEPORT,                   /* 100 TRUNK移除绑定端口 */

    PPI_AM4_ADD,                            /* 101 AM4添加IP地址 */
    PPI_AM4_DELETE,                         /* 102 AM4删除IP地址 */
...
enum tagPPIMsgType
{
    ...
    /* End of Added by z00208058/w00207740, 2012/5/2 */
...
enum tagPPIUpdateModuleType
{
    ...
    PPI_HOOKTYPE_AM4,            /*AM4 IP地址配置PPI信息下发模块号*/
...
enum tagPUBLICRetCode
{
    ...
    PUBLIC_ERR_TASKMONITOR_EXIST,       /*36,线程监控钩子函数已经存在*/
...
enum enumQueID
{
    ...
    /* QUE_LINK_DROP_INPUT,*//* delete by x36530 Trace to d09336 20050601*/ /* Added by z36377,20040928,For D01523. */

    QUE_PPPMUX_INPUT, /* Added by z43740 for PPPMUX, 2006/05/10 */
...
typedef struct tagPPI_RTMSG4
{
    ...
    /* End: VISP1.7C03 VRF qinyun , 2009-01-20 */
    UCHAR    ucDestPrefixLength;  /*目的IP地址掩码长度*/
...
{
    ULONG    ulDestination[4];       /*目的IP地址*/
    ULONG    ulNexthop[4];           /*下一跳*/
    ULONG    ulIfIndex;           /*出接口IFNET索引*/
    ULONG    ulATIndex;           /*virtual link index*/
    ULONG    ulSlot;              /*路由的槽号，暂不使用*/
    ULONG    ulFlags;             /*路由标记*/
    ULONG    ulDestPrefixLength;  /*目的IP地址掩码长度*/
    /* Begin:VISP1.7C03 VRF qinyun , 2009-01-20 */
...
{
    UCHAR aucMacAddr[MACADDRLEN];        /* MAC地址 */
    USHORT usVlanId;             /* MAC地址所属VLAN ID */
    ULONG ulIfIndex;            /* 接口索引 */
    USHORT usOutCeVlanId;    /*用户侧VLAN ID */
    UCHAR ucType;              /* MAC地址表项类型 */
...
typedef struct tagPPI_VRRP_UPDATEMAC
{
    ...
    UCHAR aucVirtualMacAddr[MACADDRLEN];      /* 虚MAC地址 */
    UCHAR ucReserved[2];                      /* 保留字段 */
}PPI_VRRP_UPDATEMAC_S;

/* VRRP PPI结构 用于SUPER VLAN添加移除SUB VLAN时通知SUB VLAN添删虚拟MAC的处理 */
...
{
    ULONG ulIfIndex;
    UCHAR aucMultiMacAddr[MACADDRLEN];  /* 多播MAC地址 */
    UCHAR aucMultiMacMask[MACADDRLEN];  /* MAC地址的掩码*/
}PPI_MULTIMAC_S;
...
typedef struct tagPPI_IPADDR_INFO
{
    ULONG   ulIfIndex;      /* 接口索引 */
    ULONG   ulVrfIndex;     /* VRF索引 */
    ULONG   ulIpAddr;       /* IP地址,网络序 */
    ULONG   ulSubnetMask;   /* IP地址掩码 */
    ULONG   ulFlag;         /* 主从IP地址标记,1 从IP标记, 2 主IP标记 */
}PPI_IPADDR_INFO_S;
...
typedef struct tagTCPIP_PPI_HOOK
{
    ...
    */
    ...
    /* Begin: Add by h54884 2008/04/23 VISP Support multi-instance project */
...
typedef struct tagNpPppIpcpOption
{
    ...
    /*IP地址*/
    ULONG   ulOurAddr;          /*本端IP地址，网络字节序*/
    ULONG   ulHisAddr;          /*对端IP地址，网络字节序*/
    /*IPHC协商结果*/
...
/* Add for V2R3C06, by z00171897/p00193127, at 2011-10-12 */
/* 新增1731 PM结构体 */
...
typedef struct tagPpi_1AGRmepPara
{
    ...
    UCHAR  aucMacAddr[MACADDRLEN];   /*远端MAC地址*/
...
typedef struct tagPPI_VlanIf
{
    ...
    UCHAR   aucMac[MACADDRLEN];  /* vlanif mac地址*/
    UCHAR   ucReserved[2];
}PPI_VLANIF_S;
...
/*End:Add by z62474 for 2007-09-10 ,增加OSI对NP的支持*/
    ...
    /* Begin: Add by h54884 2008/05/04 VISP Support multi-instance project */
...
typedef enum TCPIP_WARING_Etag
{
    ...
    WARNING_ETHARP_IP_CONFLICT,   /* 2 ETH链路上检测有IP地址与本机冲突 */
    ...
    WARNING_BFD_IPCHANGE,         /* 7 BFD使用的IP地址被用户更改 */
    ...
    WARNING_PPP_IPCONFLICT,       /* 11 PPP协商两端IP地址冲突 */
...
typedef struct tagETH_IPCONFLICT_PARAM
{
    ...
    ULONG ulIfIndex;
    CHAR  szIfName[IF_MAX_INTERFACE_NAME_LEN+1];
    ULONG ulIPAddr;     /* 网络序IP地址 */
    UCHAR szMacAddr[6]; /* 冲突者的MAC地址,6字节 */
    CHAR  ucReserved[2];
    ULONG ulVrfIndex;   /* VRF索引 */
    /* Add end */
...
typedef struct tagETH_SARPCONFLICT_PARAM
{
    ...
    ULONG     ulIPAddr;         /* 网络序IP地址 */
    UCHAR     szMacAddr[6];     /* 冲突者的静态MAC地址 */
    UCHAR     ucAlmSt;          /* 告警状态,1告警产生 0告警结束 */
    UCHAR     ucPadding;
}ETH_SARPCONFLICT_PARAM_S;

#define ETH_WARN_ETHIP_CONFLICT     1   /* 以太接口IP地址冲突类型 */
#define ETH_WARN_OMIP_CONFLICT      2   /* 特殊ARP代理的逻辑接口IP地址冲突类型，用于地址冲突告警时使用 */
#define ETH_WARN_ETHIP_CONFLICT_CLEAR_CONFIGURE     3   /*以太接口IP地址冲突消除告警（自己修改配置消除）*/
#define ETH_WARN_ETHIP_CONFLICT_CLEAR_DETECT        4   /*以太接口IP地址冲突消除告警（探测消除） */
#define ETH_WARN_OMIP_CONFLICT_CLEAR_CONFIGURE      5   /* ARP代理的逻辑接口IP地址冲突消除告警（自己修改配置消除） */
#define ETH_WARN_OMIP_CONFLICT_CLEAR_DETECT         6   /* ARP代理的逻辑接口IP地址冲突消除告警（探测消除） */
...
#define DHCP4C_WARN_IPCONFLICT     4 /* DHCP Client获取IP地址和网络内其它IP地址冲突 */
#define DHCP4C_WARN_BINDFAIL    5 /* DHCP Client获取IP地址绑定失败 */

typedef struct tagBFD_WARNING_PARAM
{
    /* Modified by q62011 for A82D13314,2007-04-04
       change description from 'BFD会话所在的接口索引'to'IP地址所在的接口索引'*/
    ULONG ulIfIndex;     /* IP地址所在的接口索引;或者为trunk成员端口索引 */
...
/*增加PPP协商IP地址冲突告警、ED描述符不一致告警、环回冲突告警及相应的告警消除,add by wuhailan,2008-04-11.*/
...
/* Add Begin: BC3D00903 2008-12-13 by z62474 */
...
typedef struct tagPPI_PPPOE_INFO
{
    ...
    UCHAR szOurMacAddr[MACADDRLEN]; /* 本端对应Ethernet接口的MAC地址 */
    UCHAR szHisMacAddr[MACADDRLEN]; /* 对端的MAC地址 */
...
*
*******************************************************************************/
extern VOID TCPIP_ShowIPAddrByVrf(CHAR *szVrfName);
/* End: VISP1.7C03 VRF qinyun , 2009-01-20 */
...
*       Author: wujie(61195)
*  Description: 设置日志、调试信息、显示信息是否打印到串口
*        Input: ULONG ulSwitch:
*               bit 0: 是否输出定长日志到串口 0 否  1 是
*               bit 1: 是否输出变长日志到串口 0 否  1 是
*               bit 2: 是否输出调试信息到串口 0 否  1 是
*               bit 3: 是否输出显示信息到串口 0 否  1 是
*       Output: NA
*       Return: VOID
*      Caution: NA
*------------------------------------------------------------------------------
...
/* 钩子函数原型，实现的功能是:在Master路由器上虚拟IP冲突检测，即检测传入的IP地址是否与指定接
   口上的某个Master备份组的虚IP冲突（即是否相等，相等则返回1，否则返回0）；入参IP地址是网络序 */
...
/* 钩子函数原型，实现的功能是:判断MAC地址是否与指定接口上某个备份组的虚MAC相同，是则返回1，否则返回0 */
...
/* 钩子函数原型，实现的功能是:判断IP地址是否虚IP地址，是则返回1，否则返回0；
   在入参aucMacAddr不为空时，需返回对应虚IP地址的虚MAC，否则无需返回，获取虚MAC成功也是返回1，否则返回0；入参IP地址是网络序 */
...
/* 钩子函数原型，实现的功能是:根据IP地址来取得与之网段匹配的虚IP对应的虚MAC，用于ARP代理的场景；进行报文发送/转发时，
   对于源IP不是本接口上的IP地址，但下一跳的IP地址与接口上的虚IP在同一网段时，报文的源MAC替换为虚MAC；成功则返回0，否则返回非0；
   入参IP地址是网络序 */
...
/* 钩子函数原型，实现的功能是:根据VRF索引和IP地址，判断指定的IP地址是否VRF内某个VRRP备份组的虚IP地址，是则返回1，否则返回0 */
...
{
    TCPIP_VRRP_DEBUG_IPCLFT      = 0x1,     /* 判断IP地址与虚IP是否冲突的调试开关 */
    TCPIP_VRRP_DEBUG_CMPVMAC     = 0x2,     /* 比较指定MAC是否虚MAC的调试开关 */
    TCPIP_VRRP_DEBUG_ISVIPINIF   = 0x4,     /* 判断IP是否指定接口的虚IP的调试开关 */
    TCPIP_VRRP_DEBUG_GETVMACBYIP = 0x8,    /* 根据IP地址获取接口上同网段虚IP对应的虚MAC的调试开关 */
    TCPIP_VRRP_DEBUG_ISVIPINVRF  = 0x10     /* 判断IP地址是否指定VRF内的虚IP的调试开关 */
}TCPIP_VRRP_DEBUG_E;
...
typedef ULONG  (*TCPIP_TaskExceptionHandlerFunc)(VOID);   /*任务监控异常处理函数*/
...
/* 注册需要监控的任务,
   ...
   pfHandler:被监控任务未被调度达到门限值的处理函数，NULL表示采用默认处理(复位进程)，
*/
...
typedef struct tagTCPIPTaskMonitorPARA
{
    ...
    TCPIP_RegMonitorTaskFunc   pfRegTaskFunc;      /*注册需要监控的任务*/
...
Description  : 提供给产品注册任务监控函数，任务监控机制由产品实现
Input        : pfHookFunc: 任务监控接口
Output       :
Return       : 0--成功，非0--失败
Create By    :
Modification :
