...
#define VRP_IP_BORROW          0x04    /* 主IP地址是借用的 */
...
#define VRP_SO_HEADCACHE        0x00400000      /*add by y36299 for HeadCache 2005/03/22*/
/* Add by w60786 for A82D24352 2008-03-18 */
...
{
    ULONG s_addr; /* IP address of the socket */
}vrp_in_addr;
...
typedef struct vrp_sockaddr_in
{
    ...
    struct vrp_in_addr   sin_addr;      /* 32-bit IP address */
...
typedef struct vrp_sockaddr_isis
{
    ...
    USHORT usMacLen;             /* MAC地址长度    */
    UCHAR  ucPadding_2[2];
    UCHAR  SrcMac[8];            /* 源MAC地址      */
    UCHAR  DstMac[8];            /* 目的MAC地址    */
}vrp_sockaddr_isis;
...
typedef struct vrp_ip_mreq
{
    ...
    ULONG                imr_ifindex;   /* local IP address of interface */
}vrp_ip_mreq;
...
{
    struct vrp_in_addr imr_mcastaddr;  /*  IP multicast address of group  */
    struct vrp_in_addr imr_msrcaddr;   /*  IP address of source for group */
    ULONG   imr_ifindex;    /*  local IP address of interface   */
}vrp_ip_mreq_source;
...
{
    ULONG sin_addr;            /* 32-bit IP address of the socket */
...
{
    ULONG ulsin_addr;            /* 32-bit IP address of the socket */
...
typedef struct tagTCPIP_HEADCACHE_ENTRY
{
    ...
    ULONG      ulSrcIP;               /* 源IP地址*/
    ULONG      ulDstIp;               /* 目的IP地址*/
    VRP_IN6ADDR_S  stSrcIP6;              /* IPv6 源IP地址   */
    VRP_IN6ADDR_S  stDstIP6;              /* IPv6 目的IP地址 */
    ULONG      ulIpPro;               /* 协议号*/
...
{
    struct vrp_sockaddr_in *pstLaddr; /* 32-bit Local IP address + local port*/
    struct vrp_sockaddr_in *pstFaddr; /* 32-bit Remote IP address + Remote port*/
...
*  Description: 设置远程主机名和IP地址
*        Input: CHAR * stHostName:指向主机名称的字符串
*               CHAR * stHostIP:要设置的IP地址，ASCII码存储形式，以'\0' 结尾
*       Output:
*       Return: VOS_OK  操作成功
...
*               ADD_HOST_ERR_INVALID_IP_ADDR   无效的IP地址(IP地址取值范围0.0.0.0 ~ 255.255.255.255，不能为空)
...
*               DEL_HOST_ERR_MNAY_IP_ADDR           预留参数（为了扩展一个主机名可以配置多个IP地址的情况）
