...
/* 用户配置MD5密码时用到的数据结构 md5P***W***中非密码成员长度和 */
#define PREFIXMD5LEN sizeof(ULONG)  /*IP address and mask*/
#define PREFIXMD5LEN_EX sizeof(ULONG) + sizeof(ULONG)  /*IP address and mask*/
...
{
    ULONG sin_addr;            /* socket的32位地址长度     32-bit IP address of the socket */
    CHAR  szMd5Password[TCPMD5PWDMAXLEN];  /* MD5密码(长度< 256位) MD5 P***W***(len < 256 bit) */
}SOCKMD5Pwd_S;

/* 设置MD5密码对应的数据结构 *//* C00-安全整改-l00103192 17  2014-1-26 */
...
typedef struct tagTCPMd5Pwd
{
    ...
    CHAR  *pszMd5Pwd;   /* MD5密码(长度< 256位)    MD5 P***W***(len < 256 bit) */
}TCPMd5Pwd_S;
...
*/
    ...
    TCPMd5PwdHead_S* pstTcpMd5PwdHead; /* 用于支持多peer MD5加密功能 */
    ...
    /*Added by x36317 for TCPCB Cache, 2005-3-25*/
...
ULONG TCPDoOptions(TCPCB_S         *pTCPCB,
                        ...
                        ULONG           *pTSVal,
                        ULONG           *pTSEchoReply,
                        /*Added by x36317 for VISPV1R7C03 Support Multi-Instance at 2009/2/4*/
                        ULONG           ulVrfindex
                        );
...
typedef struct tagTCP4MODINFO
{
    ULONG    ulModID;
    ULONG    ulSubModID;
    /*Added by x36317 for PreConfig, 2005-3-25*/
    ULONG    ulTCP4_CFG_NEED_CACHE;
    ULONG    ulTCP4_OUTPUT_OPTIMIZE; /*Added by x36317, 2005-9-26*/
} TCP4_MOD_INFO_S;
...
#define TCP_SETPASSWORD     8       /* 设置/收到TCP md5的验证密码  set/get tcp md5 authentication P***W*** */
/* TCP with MD5 authentication, zhangfeng, 2002,07,20 */
...
#define TCP_SOCKET_CANTSENDMORE         g_pstTCPSOCK4Vtbl->pfSO_CantSendMore /*Added by x36317 for SWFD03947, 2004-12-14*/
