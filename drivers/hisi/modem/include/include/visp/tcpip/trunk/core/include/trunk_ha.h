...
typedef struct tagLacpNegoBak
{
    ...
    UCHAR ucPartnerAdminSysMac[TRUNK_MACADDR_LEN]; /* 对端系统管理MAC地址 */
    UCHAR ucPartnerOperSysMac[TRUNK_MACADDR_LEN];  /* 对端系统运作MAC地址 */
}LACP_NEGO_BAK_S;
...
typedef struct tagTrunkPortNegoBak
{
    ...
    UCHAR ucOldMacAddress[TRUNK_MACADDR_LEN];       /* 成员端口加入Trunk前的MAC地址 */
...
typedef struct tagTrunkInfoBak
{
    ...
    UCHAR ucMacAddress[TRUNK_MACADDR_LEN];  /* Trunk接口MAC地址     */
...
{
    UCHAR   g_ucSysMacAddress[TRUNK_MACADDR_LEN];   /* Trunk接口MAC地址     */
...
/* 备份trunk动态获取mac地址情况下的接口索引,以保证主备一致 */
...
typedef struct tagTrunkPortMac
{
    ULONG  ulTrunkId;
    ULONG  ulPortIndex;
    UCHAR  ucBakMacAddress[TRUNK_MACADDR_LEN];       /* 主板成员端口的实际MAC地址 */
    UCHAR  Padding[2];    
}TRUNK_PORTMAC_S;
