...
typedef struct tagLacpNego
{
    ...
    UCHAR ucPartnerAdminSysMac[6]; /* 对端系统管理MAC地址 */
    UCHAR ucPartnerOperSysMac[6];  /* 对端系统运作MAC地址 */
...
typedef struct tagTrunkPortInfo
{
    ...
    UCHAR  ucOldMacAddress[6];                 /* 成员端口加入Trunk前的MAC地址 */
    UCHAR  ucBakMacAddress[6];              /* 保存主板成员端口的mac地址，用于主备一致 */
