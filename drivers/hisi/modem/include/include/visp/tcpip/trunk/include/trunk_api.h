...
enum enTrunkErrInfo
{
    ...
    TK_PORT_HAVE_IP,               /* 8、端口具有IP相关属性，如已经配置了IP地址，已经借用地址给其他接口 */
    ...
    TK_DYNAMIC_MAC_FIRST_PORT,     /* 32、动态MAC地址分配策略第一个端口必须最后一个退出  */
    ...
    TK_POLICY_IS_STATIC_GETMAC,    /* 60 产品该定制策略下TRUNK的MAC地址获取策略不能为固定MAC地址分配策略 */
...
/*End of Modified by w00207740, 2014/1/9 */
...
typedef struct tagTRUNK_INFO
{
    ...
    UCHAR ucMacAddress[6];                       /* Trunk的MAC地址 */
    UCHAR ucBfdTrunkMode;                        /* trunk或成员端口上建立单跳bfd的生效模式,默认trunk优先 */   
    UCHAR ucMacDynamic;                          /* Trunk mac地址获取模式是否为动态获取模式              */
...
/* End of Added by w00207740, 2013/9/10 */
...
#define   TRUNK_POLICY_PROTMAC_NOTREFRESH       1 /* 定制TRUNK组网优化功能策略(TRUNK成员端口MAC地址不变更,TRUNK MAC地址与当前UP的成员端口MAC地址一致） */
...
/* End of Added by w00207740, 2013/8/7 */
...
*       Return: TK_OK:  注册成功
*               TK_ERR: 注册失败
*      Caution: 产品实现时将分配的MAC地址Copy到pucMacAddr指向的内存，pucMacAddr指向的内存由VISP分配
*               由于各个Trunk接口都是直接与对端相连，因此Trunk的MAC地址是否相同并不影响报文收发。
*               如果产品为系统MAC和各个Trunk接口分别分配不同的MAC地址，那么在删除Trunk接口时要注意回收已分配的MAC地址
*------------------------------------------------------------------------------
*  Modification History
*  DATE         NAME             DESCRIPTION    
*  ----------------------------------------------------------------------------
...
*      Purpose: 获取TRUNK成员端口的原始MAC地址
*  Description: 
*        Input: ULONG ulIfIndex: 成员端口IFNET索引
*               UCHAR* pucMac:   成员端口原始的MAC地址，保证长度大于等于6字节
*       Output: UCHAR* pucMac:   成员端口原始的MAC地址
*       Return: VOS_OK           成功
*               其他             失败
*      Caution: 
*        Since: V2R3C05
...
*  默认不丢包，丢包对带来协商时间长达6秒，风险由产品自行组网保证
*******************************************************************************/
