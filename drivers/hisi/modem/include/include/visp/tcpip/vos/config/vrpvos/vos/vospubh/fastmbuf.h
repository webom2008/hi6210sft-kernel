...
{\
    ...
    /* Add Begin: by tianli29125, 2002/10/24, for D13632 */\
    /* if VRP_MODULE_MQC == VRP_YES */\
    MBUF_SET_MQC_CLASSNO(pstMBufTemp, 0);\
     /* modified by zhang zhifei, 32223, 2002/11/08, begin*/\
    /*(pstMBufTemp)->stUserTagData.stMplsPacketInfo.aulRd[0] = 0; */\
    /*(pstMBufTemp)->stUserTagData.stMplsPacketInfo.aulRd[1] = 0; */\
    (pstMBufTemp)->stUserTagData.stMplsPacketInfo.ulVrfIndex = 0;\
    (pstMBufTemp)->stUserTagData.ulAntiMultiFlag = 0 ;\
    /* modified by zhang zhifei, end */\
    ...
    {\
    	...
    	else if (MBUF_GET_TAG_NETWORK_TYPE(pstMBufTemp) == AF_INET6)\
    	{\
            MBUF_IP6_PACKET_INFO_S	*pstIp6Info;\
    	    \
    	    pstIp6Info = MBUF_GET_TAG_NETWORK_IP6_PTR(pstMBufTemp);\
        	pstIp6Info->ulLoopTimes = 0; \
        	pstIp6Info->ulIpPktType = 0; \
    	}\
    	MBUF_SET_MQC_CLASSNO(pstMBufTemp, 0);\
        VOS_DBGASSERT((pstMBufTemp)->stDataBlockDescriptor.pstNextDataBlockDescriptor == NULL);\
    	VOS_DBGASSERT((pstMBufTemp)->ulTotalDataLength == (pstMBufTemp)->stDataBlockDescriptor.ulDataLength);\
    ...
    {\
    	...
    	(pstMBufTemp)->stDataBlockDescriptor.ulType = MBUF_TYPE_DATA;\
    	(pstMBufTemp)->stDataBlockDescriptor.ulOption = 0;\
    	/*VOS_Mem_Set( (VOID *) (& pstMBufTemp->stUserTagData), 0 , sizeof(MBUF_USERTAGDATA_S));*/\
    	(pstMBufTemp)->stUserTagData.stCommonInfo.ulFlag = 0;\
    	(pstMBufTemp)->stUserTagData.ulAntiMultiFlag = 0 ;\
    	pstIpInfo = MBUF_GET_TAG_NETWORK_IP_PTR(pstMBufTemp);\
    	pstIpInfo->ulLoopTimes = 0; \
    	pstIpInfo->ulIpPktType = 0; \
    	/* Add Begin: by tianli29125, 2002/10/24, for QoS transplant */\
        /* if VRP_MODULE_MQC == VRP_YES */\
        MBUF_SET_MQC_CLASSNO(pstMBufTemp, 0);\
        /* modified by zhang zhifei, 32223, 2002/11/08, begin*/\
        /*(pstMBufTemp)->stUserTagData.stMplsPacketInfo.aulRd[0] = 0; \
...
#if 0
#define FASTMBUF_MBUF_CACHE_NUM4096 	256			/* Fast MBUF缓冲个数,2的整数倍 */

#define FASTMBUF_MBUF_CACHE_MASK4096	(FASTMBUF_MBUF_CACHE_NUM4096 - 1)
											/* Fast MBUF缓冲个数的掩码 */
#define FASTMBUF_PREALLOC_MBUF_NUM4096	   256		/* 预先申请的MBUF个数,小于FASTMBUF_MBUF_CACHE_NUM */
#else
