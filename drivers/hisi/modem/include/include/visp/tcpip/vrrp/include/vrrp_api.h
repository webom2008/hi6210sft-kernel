...
typedef enum enumVrrpErrCode
{
    ...
    VRRP_ERR_INVALID_IPADDR,            /* 11 IP地址非法 */
    VRRP_ERR_NOTSAMENET,                /* 12 虚IP与接口所有IP不在同一网段 */
    VRRP_ERR_IP_IN_OTHER_STANDBY,       /* 13 虚IP地址已经配置在另外的备份组里 */
    VRRP_ERR_IP_CONFLIC,                /* 14 虚IP与其他接口IP地址冲突 */
    VRRP_ERROR_ADDRESS_TOO_MANY,        /* 15 备份组中配置的虚IP地址达到最大值 */
...
typedef struct tagTCPIP_VRRP_STATISTICS
{
    ...
    ULONG ulErrorVirtualIpList;/* 虚拟IP地址列表匹配错误的次数 */
...
typedef struct tagTCPIP_VRRP_CONFIGINFO
{
    ...
    UCHAR ucAuthenticationKey[VRRP_AUTHENTICATION_KEY_LENGTH];  /* 认证密码 */
...
typedef struct tagTCPIP_VRRP_TRACKBFD
{
    ...
    UCHAR ucVrrpBfdState;       /* 被监控BDF 会话的状态*/
    UCHAR ucReserve[3];
...
typedef struct tagTCPIP_VRRP_TRACK3AH
{
    ...
    LONG  lReduce;              /* 下降的优先级数额 */
    UCHAR ucVrrp3ahState;       /* 被监控3ah的状态*/
    UCHAR ucReserve[3];
