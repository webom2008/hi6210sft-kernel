...
/**
    @brief          This function reallocates memory of ulNewSize bytes long and
                    returns new pointer

    @param[in]      pOldMemPtr - Pointer to previously allocated memory block.
    @param[in]      ulNewSize - new size in bytes.
    @retval         Pointer to reallocated memory block   On Success
                    NULL   On Failure
*/
IPSILINKDLL void *ipsi_realloc(void *pOldMemPtr, size_t ulNewSize);
