...
* 输出参数  : 无
*
* 返 回 值  : VOS_INT32 VOS_OK:成功 BSP_ERROR:失败
*****************************************************************************/
...
VOS_UINT32 BSP_SOCP_CoderSetDestChanAttr(VOS_UINT32                 u32DestChanID,
                                                        SOCP_CODER_DEST_CHAN_STRU *pDestAttr)
{
    VOS_UINT32             start;
    VOS_ULONG              ulStartTmp;
    VOS_UINT32             end;
...
VOS_VOID *SocpMemRemap(unsigned long phys_addr, size_t size)
{
    ...
    vaddr = (u8*)vmap(pages, npages, VM_MAP, pgprot_noncached(PAGE_KERNEL));
...
VOS_INT32 SocpAcpuInit(VOS_VOID)
{
    ...
    uBaseAddr = (VOS_UINT_PTR)DRV_GET_IP_BASE_ADDR(BSP_IP_TYPE_SOCP);
    ...
    /* 挂中断 */
    if (VOS_OK != DRV_VICINT_CONNECT((VOIDFUNCPTR *)s32IntLevel, (VOIDFUNCPTR)SocpAppInthandler, 0))
    {
        SOCP_LOG_ERR("SocpAcpuInit: connect app core int failed", 0, 0);
        return BSP_ERROR;
    }

    if (VOS_OK != DRV_VICINT_ENABLE(s32IntLevel))
    {
...
VOS_INT32 SocpCcpuInit(VOS_VOID)
{
    ...
    g_strSocpStat.uBaseAddr = (VOS_UINT_PTR)DRV_GET_IP_BASE_ADDR(BSP_IP_TYPE_SOCP);
    ...
    if (VOS_OK != BSP_INT_Enable(s32IntLevel))
    {
