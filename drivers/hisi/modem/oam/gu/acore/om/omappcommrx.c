...
VOS_UINT GU_OamAcpu_ICCError_CB(VOS_UINT ulChanID, VOS_UINT ulEvent, VOS_VOID* pParam)
{
    ...
    else if(UDI_ICC_GUOM4 == (UDI_HANDLE)ulChanID)
    {
...
{
    OM_ICC_UDI_CTRL_STRU                astACPUICCCtrlTable[OM_ICC_CHANNEL_BUTT];
    VOS_INT32                           i;
    ...
    {
        astACPUICCCtrlTable[i].pstICCAttr = (ICC_CHAN_ATTR_S*)VOS_MemAlloc(ACPU_PID_OM,
                                                                           STATIC_MEM_PT,
                                                                           sizeof(ICC_CHAN_ATTR_S));
        if(VOS_NULL_PTR == astACPUICCCtrlTable[i].pstICCAttr)
        {
    ...
    astACPUICCCtrlTable[OM_OM_ICC_CHANNEL].enICCId                    = UDI_ICC_GUOM0;

    astACPUICCCtrlTable[OM_OSA_MSG_ICC_CHANNEL].enICCId               = UDI_ICC_GUOM4;
    astACPUICCCtrlTable[OM_OSA_MSG_ICC_CHANNEL].pstICCAttr->read_cb   = V_ICC_OSAMsg_CB;

    for(i=0; i<OM_ICC_CHANNEL_BUTT; i++)
    {
        astACPUICCCtrlTable[i].pstICCAttr->u32Priority    = OM_ICC_CHANNEL_PRIORITY;
        astACPUICCCtrlTable[i].pstICCAttr->u32TimeOut     = OM_ICC_HANDSHAKE_TIME_MAX;

        if( i == OM_OSA_MSG_ICC_CHANNEL)
        {
            astACPUICCCtrlTable[i].pstICCAttr->u32FIFOInSize  = OSA_ICC_BUFFER_SIZE;
            astACPUICCCtrlTable[i].pstICCAttr->u32FIFOOutSize = OSA_ICC_BUFFER_SIZE;
        }
        else
        {
            astACPUICCCtrlTable[i].pstICCAttr->u32FIFOInSize  = OM_ICC_BUFFER_SIZE;
            astACPUICCCtrlTable[i].pstICCAttr->u32FIFOOutSize = OM_ICC_BUFFER_SIZE;
        }

        astACPUICCCtrlTable[i].pstICCAttr->enChanMode     = ICC_CHAN_MODE_PACKET;
        astACPUICCCtrlTable[i].pstICCAttr->event_cb       = GU_OamAcpu_ICCError_CB;
        astACPUICCCtrlTable[i].pstICCAttr->write_cb       = VOS_NULL_PTR;

        if(VOS_ERROR == DRV_ICC_OPEN(astACPUICCCtrlTable[i].enICCId, astACPUICCCtrlTable[i].pstICCAttr))
        {
            /* 打开失败时记录当前ICC通道信息 */
...
VOS_VOID OM_AcpuLogShowToFile(VOS_BOOL bIsSendMsg)
{
    ...
    DRV_FILE_WRITE((VOS_VOID*)&ulTemp, sizeof(VOS_CHAR), sizeof(VOS_UINT32), fp);
#ifdef SCM_SNCHECK
    DRV_FILE_WRITE((VOS_VOID*)&g_ulSnErrCount , sizeof(VOS_CHAR), sizeof(g_ulSnErrCount), fp);
    DRV_FILE_WRITE((VOS_VOID*)&ulTemp, sizeof(VOS_CHAR), sizeof(VOS_UINT32), fp);
#endif
