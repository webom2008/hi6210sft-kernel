                           ...
                           VOS_UINT32 aulArgs[VOS_TARG_NUM] )
{
    ...
    *pulTaskID = iTid;
    ...
    /* set name */
#if defined(FEATURE_KERNEL_LINUX_3_10)
    ulNameLen = VOS_MAX_LENGTH_TASK_NAME<TASK_COMM_LEN ? VOS_MAX_LENGTH_TASK_NAME : TASK_COMM_LEN;
    for ( i=0; i<ulNameLen-1; i++ )
    {
        tsk->comm[i] = vos_TaskCtrlBlk[iTid].Name[i];
    }
    tsk->comm[ulNameLen-1] = '\0';
#endif
