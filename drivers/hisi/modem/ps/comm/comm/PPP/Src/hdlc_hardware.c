...
VOS_VOID PPP_HDLC_HARD_PeriphClkOpen(VOS_VOID)
{
    ...
    PPP_HDLC_WRITE_32REG(SOC_SCTRL_SC_PERIPH_CLKEN0_ADDR(g_ulHdlcScCtrlBaseAddr), ulValue);
...
VOS_VOID PPP_HDLC_HARD_PeriphClkClose(VOS_VOID)
{
    ...
    PPP_HDLC_WRITE_32REG(SOC_SCTRL_C_PERIPH_CLKDIS0_ADDR(g_ulHdlcScCtrlBaseAddr), ulValue);
...
VOS_UINT32 PPP_HDLC_HARD_InitBuf(VOS_VOID)
{
    ...
    g_ulPppVirtAddr = (VOS_UINT32)VOS_UnCacheMemAlloc(g_ulPppTotalBufLen, (VOS_UINT32 *)&g_ulPppPhyAddr);
    if (!g_ulPppVirtAddr)
    {
        PPP_MNTN_LOG(PS_PID_APP_PPP, 0, PS_PRINT_ERROR,
                      "PPP_HDLC_HARD_InitBuf, ERROR, VOS_UnCacheMemAlloc Fail\r\n");
        return VOS_ERR;
    }

    memset(g_ulPppVirtAddr, 0, g_ulPppTotalBufLen);
...
VOS_UINT32 PPP_HDLC_HARD_Init(VOS_VOID)
{
    ...
    /* 获取HDLC基地址 */
    ulBaseAddr              = (VOS_UINT_PTR)DRV_GET_IP_BASE_ADDR(BSP_IP_TYPE_HDLC);
...
#else
    ulBaseAddr              = (VOS_UINT_PTR)DRV_GET_IP_BASE_ADDR(BSP_IP_TYPE_SYSCTRL);
...
VOS_UINT32 PPP_HDLC_NODE_PHY_TO_VIRT(unsigned int ulPAddr)
{
    ...
    {
        PPP_MNTN_LOG1(PS_PID_APP_PPP, 0, PS_PRINT_WARNING,
                      "PPP_HDLC_NODE_PHY_TO_VIRT, WARNING, invalid ulPAddr 0x%x\r\n",
                      ulPAddr);
...
VOS_UINT32 PPP_HDLC_NODE_VIRT_TO_PHY(unsigned int ulVAddr)
{
    ...
    {
        PPP_MNTN_LOG1(PS_PID_APP_PPP, 0, PS_PRINT_WARNING,
                      "PPP_HDLC_NODE_VIRT_TO_PHY, WARNING, invalid ulVAddr 0x%x\r\n",
                      ulVAddr);
