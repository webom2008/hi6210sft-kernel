...
BSP_S32 BSP_ICC_Init(void)
{
    ...
    printk(KERN_ERR "BSP_ICC_Init: enter,time stamp = 0x%x!\n", DRV_GET_SLICE());
    ...
    if (g_pstIccCtrlChan->bInit[ICC_TAGET])
    {
        ...
        printk(KERN_ERR "BSP_ICC_Init: leave,time stamp=0x%x,cnt=%d, connet=%d!\n", \
                                   DRV_GET_SLICE(), cnt, g_pstIccCtrlChan->bConnect);
        return BSP_OK;
    }
    else
    {
...
BSP_S32 BSP_ICC_Open(BSP_U32 u32ChanId, ICC_CHAN_ATTR_S *pChanAttr)
{
    ...
    pSmChan->openBeginTime[ICC_HOST] = DRV_GET_SLICE();
    printk(KERN_ERR "BSP_ICC_Open channel id is %d,enter at 0x%x(time)!\n", \
...
/*Modify_for_c_reset, l00212112,20130511, ends*/
    ...
    if(ICC_CHAN_STATE_OPENED == pChan->prxSubChanInfo->u32SubChanState)
    {
        ...
        pSmChan->openEndTime[ICC_HOST] = DRV_GET_SLICE();
        printk(KERN_ERR "BSP_ICC_Open channel id is %d,leave at 0x%x(time)!\n", \
