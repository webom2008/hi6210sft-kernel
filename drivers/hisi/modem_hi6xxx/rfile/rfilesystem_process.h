...
struct rfile_ops {
    ...
    int   (*bsp_fclose)(FILE* stream);
    int   (*bsp_fread)( void *ptr, unsigned int count, FILE *stream );
    int   (*bsp_fwrite)( void *ptr, unsigned int count, FILE *stream );
    int   (*bsp_fseek)( FILE *stream, long offset, int whence );
    int   (*bsp_remove)( const char *pathname );
    int   (*bsp_ftell)( FILE *stream );
