...
void platform_cpu_power_down(int cpu)
{
    ...
    do{
        tmp = readl((unsigned int)&(acpu_core_sc->acpu_sc_mtcmos_timer_state)) & 0x1;
     } while (0x0 != tmp);
...
void platform_cluster_power_down(int clusterId)
{
	...
	switch(clusterId)
		...
		case ACPU_PD_ID_CLUSTER1:
			if(1 == (readl(SOC_ACPU_SCTRL_ACPU_SC_A53_CLUSTER_ISO_STA_ADDR(acpu_sctrl_base_addr)) & BIT(SOC_ACPU_SCTRL_ACPU_SC_A53_CLUSTER_ISO_STA_pw_iso_a53_1_sta_START)))
			{
