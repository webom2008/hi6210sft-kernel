...
int buslow_minfreq_handle(unsigned int req_value)
{
	...
	if (busdfs == 0){
        ...
        /*get dfs switch flag*/
        if(RET_OK != pwrctrl_is_func_on(PWC_SWITCH_BLOWDFS))
        {
            return RET_ERR;
        }
        ...
        {
            reg_addr = HISI_VA_ADDRESS(SOC_AO_SCTRL_SC_MCU_VOTE2DIS_ADDR(SOC_AO_SCTRL_BASE_ADDR));
        ...
        {
            reg_addr = HISI_VA_ADDRESS(SOC_AO_SCTRL_SC_MCU_VOTE2EN_ADDR(SOC_AO_SCTRL_BASE_ADDR));
...
int bus_minfreq_handle(unsigned int req_value)
{
    ...
    {
        reg_addr = HISI_VA_ADDRESS(SOC_AO_SCTRL_SC_MCU_VOTE1DIS_ADDR(SOC_AO_SCTRL_BASE_ADDR));
    ...
    {
        reg_addr = HISI_VA_ADDRESS(SOC_AO_SCTRL_SC_MCU_VOTE1EN_ADDR(SOC_AO_SCTRL_BASE_ADDR));
...
{
    return BSP_IPC_IntSend(IPC_CORE_MCU, IPC_MCU_INT_SRC_ACPU_DFS);
}
...
int pwrctrl_acpu_freq_limit_max(unsigned int req_value, unsigned int req_flag)
{
    ...
    if(RET_OK != (pwrctrl_is_func_on(PWC_SWITCH_AGENERAL) | pwrctrl_is_func_on(PWC_SWITCH_ADFS)))
    {
        return -1;
    }
...
int pwrctrl_dfs_init(void)
{
    ...
    reg_addr = HISI_VA_ADDRESS(SOC_PERI_SCTRL_SC_RESERVED8_ADDR(SOC_PERI_SCTRL_BASE_ADDR));
