...
int rdr_nv_init(void)
{
	...
	while (1) {
		//ret = -ENXIO;//hisi_nve_direct_access_for_rdr(&nv);
...
static void stack_queue(u32 field, u32 *buf, u32 byte, struct task_struct *task)
{
	...
	for (i = 0; i < RDR_DUMP_STACK_DEEP; i++)
		qbuf[i + byte / sizeof(u64)] = rdr_dump_stack[i];
...
void dump_task_switch_hook(const void *old_tcb, void *new_tcb)
{
	...
	int magic = 0;//((struct task_struct *)new_tcb)->dump_magic;*/
	struct hisi_ringbuffer_s *q;
...
/*del by xiehongliang for seattle*/
/*	if (1 == g_dump_cfg.dump_cfg.bits.int_switch)
...
/*#define HI_SRAM_MEM_ADDR	hisi_reserved_debug_phymem
#define DDR_MEM_ADDR		hisi_reserved_debug_phymem
#define DDR_SHARED_MEM_ADDR	hisi_reserved_debug_phymem
#define DDR_SOCP_ADDR		hisi_reserved_debug_phymem
#define DDR_HIFI_ADDR		hisi_reserved_debug_phymem
#define DDR_MEM_SIZE		0
#define DDR_SHARED_MEM_SIZE	0
#define DDR_HIFI_SIZE		0
#define DDR_SOCP_SIZE		0
*/
