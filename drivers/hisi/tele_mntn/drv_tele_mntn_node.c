...
static void tele_mntn_wake_up_acore_irq(unsigned int param)
{
...
static int __init proc_tele_mntn_node_init(void)
{
    ...
    ret = BSP_IPC_IntConnect(IPC_ACPU_INI_SRC_MCU_TELE_MNTN_NOTIFY, tele_mntn_wake_up_acore_irq, 0);
    if (BSP_OK != ret){
        printk(KERN_ERR "failed to connect IPC irq handle (%d)\n", IPC_ACPU_INI_SRC_MCU_TELE_MNTN_NOTIFY);
    }
    else {
        BSP_IPC_IntEnable(IPC_ACPU_INI_SRC_MCU_TELE_MNTN_NOTIFY);
    }

    printk(KERN_ERR"proc_tele_mntn_node_init success\n");
    return 0;
