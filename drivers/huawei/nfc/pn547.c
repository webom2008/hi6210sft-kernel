		...
		size_t count, loff_t *offset)
{
	...
	{
		ret = (size_t)i2c_master_recv(pn547_dev->client, tmp, count);
		if (ret == count) {
			isSuccess = true;
			break;
		} else {
		...
		size_t count, loff_t *offset)
{
	...
	for (retry = 0; retry < NFC_TRY_NUM; retry++)
	{
		ret = (size_t)i2c_master_send(pn547_dev->client, tmp, count);
		if (ret == count) {
			isSuccess = true;
			break;
		} else {
			    ...
			    unsigned int cmd, unsigned long arg)
{
	...
	case PN547_SET_PWR:
		if (arg == 2) {
			...
			ret = regulator_bulk_enable(1, &(pn547_dev->ven_felica));
			if (ret < 0) {
				pr_err("%s: regulator_enable failed, ret:%d\n", __func__, ret);
				return ret;
			}
			...
			msleep(20);
			ret = regulator_bulk_disable(1, &(pn547_dev->ven_felica));
			msleep(1);
			ret = regulator_bulk_disable(1, &(pn547_dev->ven_felica));
			if (ret < 0) {
				pr_err("%s: regulator_disable failed, ret:%d\n", __func__, ret);
                                                                ...
                                                                void *unused)
{
	...
	retval = regulator_bulk_enable(1, &(nfcdev->ven_felica));
...
static int check_pn547(struct i2c_client *client, struct  pn547_dev *pdev)
{
	int ret = -1;
	int count = 0;
	const char host_to_pn547[4] = {0x20, 0x00, 0x01, 0x01};
...
static int pn547_get_resource(struct  pn547_dev *pdev,  struct i2c_client *client)
{
	...
	pinctrl_select_state(pdev->pctrl, pdev->pins_default);
		...
		const struct i2c_device_id *id)
{
	...
	regulator_bulk_disable(1, &(pn547_dev->ven_felica));
...
err_check_pn547:
    if(pn547_dev->nfc_clk)
	{
		...
		pinctrl_select_state(pn547_dev->pctrl, pn547_dev->pins_idle);
	}
...
#if 0
	pinctrl_select_state(pn547_dev->pctrl, pn547_dev->pins_default);
