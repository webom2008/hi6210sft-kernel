...
extern unsigned int easywake_position[6];
...
#if defined (CONFIG_HUAWEI_DSM)
	LDO17_status = hi6xxx_pmic_reg_read(LDO17_PHYS_ADDR);
	LSW50_status = hi6xxx_pmic_reg_read(LSW50_PHYS_ADDR);
...
{
	int error;
	error = ts_glove_mode_store(dev, attr, buf, count);
	TS_LOG_INFO("ts_sensitivity_store done\n");
	return error;
/*	unsigned int value;
	int error;
...
{
	struct ts_easy_wakeup_info *info = &g_ts_data.easy_wakeup_info;
	ssize_t ret;

	TS_LOG_INFO("ts_easy_wakeup_gesture_show_called\n");
	ret = snprintf(buf, sizeof(info->easy_wakeup_gesture)*2, "0x%04X\n",
			info->easy_wakeup_gesture+info->palm_cover_flag*2048);
	return ret;
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	info->easy_wakeup_gesture = (u16)value&0x07ff;
	info->palm_cover_flag = (u16)(value&0x0800)/2048;
...
static ssize_t ts_position_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
static ssize_t ts_position_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	TS_LOG_INFO("ts_position_store\n");
	return 0;
}
		...
		struct device_attribute *attr, char *buf, size_t size)
{
	...
	ret =ts_position_show(dev, NULL, buf);
out:
	return ret;
}

static ssize_t ts_easy_wakeup_position_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	ssize_t ret = 0;
	if (dev == NULL) {
	...
	ret =ts_position_store(dev, NULL, buf, size);
out:
	return ret;
}
...
static DEVICE_ATTR(easy_wakeup_control, (S_IRUSR | S_IWUSR),NULL,ts_easy_wakeup_control_store);
static DEVICE_ATTR(easy_wakeup_position, (S_IRUSR | S_IWUSR),ts_easy_wakeup_position_show,ts_easy_wakeup_position_store);
	...
	struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	...
	case TS_BEFORE_SUSPEND: 	/*do something before suspend*/
		...
		if(g_ts_data.easy_wakeup_info.easy_wakeup_gesture == 0){
			disable_irq(irq_id);
		}
#else
		disable_irq(irq_id);
...
static int ts_chip_init(void)
{
	...
	if (dev->ops->chip_init){
		#ifdef HUAWEI_EASY_WAKEUP
		error = dev->ops->chip_init(&g_ts_data);
		#else
			       ...
			       struct kobj_attribute *attr, char *buf)
{
      if (get_lcd_type() == JDI_1902B_LCD) {
	      return sprintf(buf,
		__stringify(EV_KEY) ":" __stringify(KEY_BACK)   ":270:2013:250:160"
		":" __stringify(EV_KEY) ":" __stringify(KEY_HOMEPAGE)   ":540:2013:250:160"
		":" __stringify(EV_KEY) ":" __stringify(KEY_MENU) ":810:2013:250:160"
		"\n");		
		
      	} else {
             return sprintf(buf,
		__stringify(EV_KEY) ":" __stringify(KEY_BACK)   ":142:1342:150:100"
		":" __stringify(EV_KEY) ":" __stringify(KEY_HOMEPAGE)   ":360:1342:150:100"
		":" __stringify(EV_KEY) ":" __stringify(KEY_MENU) ":578:1342:150:100"
		"\n");
      	}
}
