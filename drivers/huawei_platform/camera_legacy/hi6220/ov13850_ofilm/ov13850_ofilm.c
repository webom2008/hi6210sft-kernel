...
static vcm_info_s vcm_dw9714a_param = {
    #include "vcm_dw9714_ofilm.h"
};
static effect_params effect_ov13850_ofilm = {
#include "effect_ov13850_ofilm_cherry.h"
};
...
static framesize_s ov13850_ofilm_framesizes[] = {
//2080x1560, 30fps
	{0, 0, 2080, 1560, 4736, 1786, 30, 30, 0x214, 0x1bb, 0x100, VIEW_FULL, RESOLUTION_4_3, false, false, {ov13850_ofilm_framesize_quater, ARRAY_SIZE(ov13850_ofilm_framesize_quater)}, CLK_500M},
//4160x3120, 10fps
...
//4160x3120, 13fps
    {0, 0, 4160, 3120, 5704, 3328, 13, 13, 0x1B0, 0x168, 0xD0, VIEW_FULL, RESOLUTION_4_3, false, false, {ov13850_ofilm_framesize_fullsize_nonzsl, ARRAY_SIZE(ov13850_ofilm_framesize_fullsize_nonzsl)}, CLK_500M}
};
...
static int ov13850_ofilm_enum_framesizes(struct v4l2_frmsizeenum *framesizes)
{
	...
	u32 this_max_index = ARRAY_SIZE(ov13850_ofilm_framesizes) - 1;
...
{
	int max_index = ARRAY_SIZE(ov13850_ofilm_framesizes) - 1;
				 ...
				 struct v4l2_frmsize_discrete *fs, int flag, camera_setting_view_type view_type,bool zsl_preview)
{
	...
	size = ARRAY_SIZE(ov13850_ofilm_framesizes);

	if (VIEW_FULL == view_type) {
		for (i = 0; i < ARRAY_SIZE(ov13850_ofilm_framesizes); i++) {
	...
	if (i >= ARRAY_SIZE(ov13850_ofilm_framesizes)) {
...
static int ov13850_ofilm_init_reg(void)
{
	...
	print_debug("Enter Function:%s  , initsize=%d",
		    __func__, sizeof(ov13850_ofilm_init_regs));
	...
	size = ARRAY_SIZE(ov13850_ofilm_init_regs);
...
static int ov13850_ofilm_framesize_switch(camera_state state)
{
	...
	if (next_frmsize_index >= ARRAY_SIZE(ov13850_ofilm_framesizes)){
...
static int ov13850_ofilm_check_sensor(void)
{
	...
	ov13850_ofilm_sensor.vcm = &vcm_dw9714a_param;
       sensor->effect = &effect_ov13850_ofilm;

	ret = k3_socgpio_get_id_value(M_CAMERA_ID_PIN);

	if(ret < 0) {
		print_error("%s  ov13850_ofilm_sensor fail to get gpio value!!!so return -1!\n",__func__);
		return -1;
	}

	if(ret != MODULE_OFILM){
		print_info("%s this moudle is not ov13850_ofilm moudle",__func__);
		return -1;
	}
...
static void ov13850_ofilm_set_default(void)
{
	...
	ov13850_ofilm_sensor.frmsize_list = ov13850_ofilm_framesizes;
