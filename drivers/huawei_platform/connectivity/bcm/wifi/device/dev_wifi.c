	...
	if (on) {
		...
		msleep(100);
                ...
                if (!(IS_ERR(wifi_host->pctrl)||IS_ERR(wifi_host->pins_normal))){
                    ...
                    if(ret)
                        pr_err("%s: set NORMAL mode failed, ret:%d\n", __func__, ret);
                }
#endif
...
{
    pr_info("%s: len:%d, val=%s.\n", __func__, len, val);
    if ( val == NULL || len <= 0 ) {
        pr_err("%s:val is null or len is invalid.\n", __func__);
        return -1;
    }
...
int hi_sdio_detectcard_to_core(int val)
{
    ...
    pr_err("dw_mci_sdio_card_detect %d, sdio_host=0x%x", val, (unsigned int)sdio_host);
...
int  wifi_power_probe(struct platform_device *pdev)
{
	...
	{
		pr_err("%s: iomux_lookup_block failed, and the value of wifi_host->pctrl is %lx\n", __func__,(unsigned long)wifi_host->pctrl);
		ret = -1;
	...
	{
		pr_err("%s: pinctrl_lookup_state default failed, and the value of wifi_host->pins_normal is%lx\n", __func__,(unsigned long)wifi_host->pins_normal);
		ret = -1;
		goto err_pinctrl_get;
	}
	...
	{
		pr_err("%s: pinctrl_lookup_state idle failed, and the value of wifi_host->pins_idle is %lx\n", __func__,(unsigned long)wifi_host->pins_idle);
		ret = -1;
		goto err_pinctrl_get;
	}
	...
	ret = pinctrl_select_state(wifi_host->pctrl, wifi_host->pins_normal);
	if (ret <0) {
		pr_err("%s: pinctrl_select_state failed.\n", __func__);
		goto err_pinctrl_get;
	}
#endif
	...
	pr_err("%s: wifi_host->wifi_wakeup_irq %d\n", __func__, wifi_host->wifi_wakeup_irq);
	if (wifi_host->wifi_wakeup_irq<0) {
		ret = -1;
		goto err_gpio_get;
	}
	...
	int vio_enable;
	if(!udp){
		pr_err("%s: not udp board, set vio.\n", __func__);
		...
		pr_err("%s: udp board doesn't have vio, do nothing.\n", __func__);
	}
#endif
	...
	if ( ret < 0 || nvpath == NULL) {
		pr_err("%s: get nvrame_name failed, ret:%d.\n", __func__, ret);
	}
	else {
		pr_err("%s:  nvrame_name :%s.\n", __func__, nvpath);
