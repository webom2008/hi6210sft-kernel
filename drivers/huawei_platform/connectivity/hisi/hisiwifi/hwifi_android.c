...
int32 hwifi_android_priv_cmd(struct hi110x_device* hi110x_dev, struct ifreq *ifr, int cmd)
{
    ...
    wiphy = hi110x_dev->netdev->ieee80211_ptr->wiphy;
    ...
    switch (i)
    {
        ...
        case ANDROID_WIFI_CMD_SETROAMMODE:
            roam_switch = *(command + strlen(g_android_hwifi_priv_cmd[i]) + 1) - '0';
            HWIFI_INFO("ANDROID_WIFI_CMD_SETROAMMODE roam_switch=%d", roam_switch);

            #ifdef __ROAM__
            ret = hwifi_set_roam_switch(hi110x_dev->cfg, roam_switch);
            if(SUCC == ret)
            {
                snprintf(command, 3, "OK");
                written_number = strlen(command);
