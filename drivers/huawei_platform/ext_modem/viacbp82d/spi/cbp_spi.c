...
static int S_gpio_via_requst_to_send = 0;

/*----------------------data_ack functions-------------------*/
static struct cbp_wait_event *cbp_data_ack = NULL;
extern long long ack_us[10];
extern unsigned int ack_index;
extern unsigned int ack_init_status;
...
static int data_ack_wait_event(struct cbp_wait_event *pdata_ack)
{
	...
	unsigned int failed_times = 0;
       ...
       int Rts = oem_gpio_get_value(S_gpio_via_requst_to_send);
	...
	do {
		...
		if (0 == cbp_power_state) {
			...
			hwlog_err("%s: wait CBP response data ack timeout in 2s. data ack = %d,  last data ack = %d\n", __func__, oem_gpio_get_value(cbp_data_ack->wait_gpio), cbp_data.saved_last_data_ack_gpio_level);
                     hwlog_err("%s: Old AP wake CP= %d,  CP ready = %d\n", __func__, tx_wake, tx_ready);
                     hwlog_err("%s: AP wake CP = %d,  CP ready = %d\n", __func__, oem_gpio_get_value(spi_tx_handle.gpio_wake), oem_gpio_get_value(spi_tx_handle.gpio_ready));
                     hwlog_err("%s: Old CP wake AP = %d,  AP ready = %d\n", __func__, rx_wake, rx_ready);
                     hwlog_err("%s: CP wake AP = %d,  AP ready = %d\n", __func__, oem_gpio_get_value(spi_rx_handle.gpio_wake), oem_gpio_get_value(spi_rx_handle.gpio_ready));
                     hwlog_err("%s: Rts = %d, old Rts = %d\n", __func__, oem_gpio_get_value(S_gpio_via_requst_to_send), Rts);
			failed_times++;
...
static int cbp_probe(struct platform_device *pdev)
{
	...
	plat->gpio_via_requst_to_send = of_get_named_gpio(np, "via_requst_to_send", 0);
        S_gpio_via_requst_to_send = plat->gpio_via_requst_to_send;
