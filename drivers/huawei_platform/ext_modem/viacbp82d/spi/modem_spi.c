...
int rnic_send_packet(struct sk_buff *pstData, unsigned char ucPDNId);
...
static void spi_tx_rx_printk(const void *buf, unsigned char type)
{
	...
	if(type == 1){
		spi_print_buff_pos += snprintf(&spi_print_buff[spi_print_buff_pos], (1024 - spi_print_buff_pos), \
			"write %d to channel %d tx cnt %d >>", count, *(print_buf+1), spi_tx_cnt);
	}
	else{
		spi_print_buff_pos += snprintf(&spi_print_buff[spi_print_buff_pos], (1024 - spi_print_buff_pos), \
			"read %d from channel %d rx cnt %d<<", count, *(print_buf+1), spi_rx_cnt);
	}
 ...
 */
		...
		spi_is_current_tx = 1;
                ...
                do {
                    ...
                    if (-1 == ret_wait_ack) {
                        hwlog_err("%s: spi_write_port_work has wait data ack timeout. data_lenth = %d, todo = %d, write_len = %d\n", __func__, count, todo, write_len);
                        if (retry_times++ < MAX_SEND_RETRY) {
...
int rnic_send_packet(struct sk_buff *pstData, unsigned char ucPDNId)
{
    ...
    hwlog_info("RNIC__ %s: port%d, packge size = %d\n", __func__, port->index, rnic_frame.list_size);
...
static void spi_read_port_work(struct work_struct *work)
{
	...
	do{
		...
		if (TRANS_STATUS_WAITING_INIT_SEGMENT == modem->spi_rx_state) {
			...
			msg_buf_pos = 0;
			total_len= (((unsigned int)(modem->msg->head.tranHi & 0x0F) << 8) |
			...
			if (total_len < 24) {
				memcpy(spi_rx_current_packet_dump, modem->msg, total_len);
			} else {
				memcpy(spi_rx_current_packet_dump, modem->msg, 24);
			}
			spi_is_current_tx = 0;
			...
			memset(spi_rx_current_packet_dump, 0, sizeof(spi_rx_current_packet_dump));
			if (remain_packet_len< 24)  {
				memcpy(spi_rx_current_packet_dump, via_spi_follow_buf, remain_packet_len);
			} else {
				memcpy(spi_rx_current_packet_dump, via_spi_follow_buf, 24);
			}
			spi_is_current_tx = 0;
		...
		for (i = 0; i < MAX_SEND_RETRY; i ++) { //wait 12s wait for VIA watchdog
			ret_read_wait_ack = modem->cbp_data->data_ack_wait_event(modem->cbp_data->cbp_data_ack);
			if (-1 == ret_read_wait_ack) {
				hwlog_err("%s %d: channel %d has wait data ack timeout. xfer.len = %d\n", \
					__func__, __LINE__, modem->msg->head.chanInfo, xfer.len);
				spi_tx_rx_packets_dump();
				if ((xfer.len > VIA_SPI_PACKET_END_DUMMY_SIZE) || (modem->spi_rx_state != TRANS_STATUS_FINISHED)) {
					err_ack = 1;
				}
			} else if (-2 == ret_read_wait_ack) {
				hwlog_err("%s %d: VIA reset has happened.\n", __func__, __LINE__);
				spi_tx_rx_packets_dump();
				goto out;
			} else if (0 == ret_read_wait_ack){
				break;
			} else {
	...
	/* if extra data are found after spi_sync without VIA ack,
	* those garbidge will stop AP from reading
	* later data. So just read and dump them.
	*/
	if (1 == err_ack) {
		do {
			xfer.len = VIA_SPI_PACKET_END_DUMMY_SIZE;
			spi_message_init(&m);
			spi_message_add_tail(&xfer, &m);
			//READ remain valid data until dummy data read from SPI
			status = spi_sync(modem->spi, &m);
			if (status) {
				hwlog_err("%s %d: sync error: status=%d.\n", __func__, __LINE__, status);
			}

			if ((0xFF == via_spi_follow_buf[0]) && (0xFF == via_spi_follow_buf[1]) && \
				(0xFF == via_spi_follow_buf[2]) && (0xFF == via_spi_follow_buf[3])) {
				break;
			} else {
				hwlog_err("%s %d: packet tailer is invalid, chan=%d end following:0x%x-0x%x-0x%x-0x%x\n", \
					__func__, __LINE__, modem->msg->head.chanInfo, via_spi_follow_buf[0], \
					via_spi_follow_buf[1], via_spi_follow_buf[2], via_spi_follow_buf[3]);
			}
			tailer_retry++;
		}while (tailer_retry < VIA_SPI_PACKET_MAX_LENGTH_IN_WORD);
	}
