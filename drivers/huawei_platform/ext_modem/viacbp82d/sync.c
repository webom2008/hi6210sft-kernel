...
//    daemonize(name);
    ...
    while(1){
        ...
        if(AP_RX_EVENT_RESET == id){
            ...
            dsp = rx->table + index;
            if(dsp->handle){
                hwlog_info("Rx(%s): process event(%d) in state(%s).\n", rx->cfg.name, id, dsp->name);
                dsp->handle(rx, id);
                hwlog_info("Rx(%s): go into state(%s).\n", rx->cfg.name, rx->table[atomic_read(&rx->state)].name);
            }
        }
...
{
    struct asc_rx_handle *rx = (struct asc_rx_handle *)data;//linux native
...
int asc_tx_user_count(const char *path)
{
    ...
    name = strchr(path, '.');
    if (name){
        memcpy(hname, path, min((int)(name - path), ASC_NAME_LEN - 1));
        name++;
    }else {
...
int asc_tx_auto_ready(const char *name, int sync)
{
    ...
    struct asc_user *user;
    struct asc_tx_handle *tx;
	long timeout =0;//linux native for sleep_on return value
