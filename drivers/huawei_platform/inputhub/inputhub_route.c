...
static int shb_reboot_notifier(struct notifier_block *nb, unsigned int foo, void *bar)
{
...
#ifdef CONFIG_IOM3_RECOVERY
static int shb_recovery_notifier(struct notifier_block *nb, unsigned int foo, void *bar)
{
	/* prevent access the emmc now: */
	hwlog_info("%s (%ld) +\n", __func__, foo);
	mutex_lock(&mutex_write_cmd);
