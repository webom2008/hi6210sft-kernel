...
static int detect_i2c_device(struct device_node *dn, char *device_name)
{
	...
	hwlog_err("%s:read i2c bus_number (%d)or slave address(0x%x) or chip_id_register(0x%x) from dts success\n",
			device_name, i2c_bus_num, i2c_address, register_add);
...
static int init_sensors_cfg_data_from_dts(void)
{
	...
	is_invensense_exit = check_invensense_exist();
       hwlog_info("%s, start check property, is_invensense_exit=%d\n", __func__, is_invensense_exit);
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	uint64_t val = 0;
	int ret=0;
	uint8_t bus_num = 0, i2c_address = 0, reg_add = 0, len = 0, rw = 0, buf_temp[10] = {0};
