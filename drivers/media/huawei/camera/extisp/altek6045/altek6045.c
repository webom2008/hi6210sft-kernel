...
static char miniisptest_index=0;
...
static ssize_t altel6045_miniisptest_show(struct device_driver *drv,
								char *buf);
static ssize_t altel6045_miniisptest_store(struct device_driver *drv,
								const char *buf, size_t count);
static DRIVER_ATTR(miniisp_test, 0664, altel6045_miniisptest_show, altel6045_miniisptest_store);

extern void misp_start_preview(void);
void misp_getsysteminfo(u8 * buf);
u8 miniisp_sysinfo[100]={0};
												  ...
												  const char *buf, size_t count)
{
	...
	if (isdigit(*pos))
		miniisptest_index=*pos-'0';
	else
		cam_err("%s please enter digit number 0-9!", __func__);

	if(miniisptest_index == 0){
		altek6045_power_off(NULL);
	}
	else	if(miniisptest_index == 1){
		altek6045_power_on(NULL);
	}
	else	if(miniisptest_index == 2){
		altek6045_load_firmware(NULL);
	}
	else	if(miniisptest_index == 3){
		misp_start_preview();
	}
	else	if(miniisptest_index == 4){
		memset(miniisp_sysinfo,0,sizeof(miniisp_sysinfo));
		misp_getsysteminfo((u8 *)miniisp_sysinfo);
		cam_info("%s miniisp_sysinfo=%s", __func__,miniisp_sysinfo);
	}else{
		cam_err("%s miniisp test function %d is not ready!", __func__, miniisptest_index);
	}

	cam_info("%s exit, miniisptest_index=%d", __func__,miniisptest_index);
												...
												char *buf)
{
	...
	ret = snprintf(offset, PAGE_SIZE, "[current miniisptest=%d] ", miniisptest_index);
	offset += ret;
...
altek6045_init_module(void)
{
	...
	/* NOTE: use driver attribute */
	if (ret == 0) {
		if (driver_create_file(&s_altek6045_driver.driver, &driver_attr_miniisp_test))
			cam_warn("%s create driver attr failed", __func__);
	}
