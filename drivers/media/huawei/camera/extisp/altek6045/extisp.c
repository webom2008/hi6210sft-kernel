...
#define MINI_ISP_BOOT_CODE		("/system/miniisp/miniBoot.bin")
#define MINI_ISP_BASIC_CODE		("/system/miniisp/TBM_Latour.bin")
...
int misp_exec_cmd(u32 opcode, u8 *param)
{
	...
	if (!devdata) {
		cam_notice("%s no driver data", __func__);
		return -ENODEV;
	}
	...
	if (set_flag == 1) {
		...
		if ((cmd == ISPCMD_CAMERA_PREVIEWSTREAMONOFF) &&
			...
			if (devdata->state != MINI_ISP_STATE_SYNC) {
				cam_notice("%s wait sync signal timeout", __func__);
				err = -EAGAIN;
				goto out;
			}
		}
		...
		if (devdata->state != MINI_ISP_STATE_READY) {
			cam_notice("%s wait ready signal timeout", __func__);
			err = -EAGAIN;
			goto out;
		}
...
int camera_agent_streamonoff(int sensor_index, u8 state)
{
	...
	cam_notice("Enter Function:%s=[%s]", __func__, state?"on":"off");
	...
	if (errorcode) {
		cam_notice("%s fail, error code = %d", __func__, errorcode);
		return -1;
	}
	return 0;
}

int camera_agent_set_sensormode(int sensor_index,u8 size_index,u8 hv_flip)
{
	...
	cam_notice("Enter Function:%s mode=%d hv_flip = %x", __func__, size_index,hv_flip);
	cmd = misp_construct_opcode(OpCode,SET_CMD,sizeof(buf));
...
void misp_start_preview(void)
{
	int errorcode;
	cam_notice("%s enter", __func__);
	errorcode = camera_agent_set_sensormode(0,99,0);
	if (errorcode == ERR_SUCCESS) {
		cam_notice("%s: camera_agent_set_sensormode success", __func__);
		
	}else{
		cam_err("%s: camera_agent_set_sensormode failed", __func__);
		return ;
	}
	msleep(1);
	errorcode = camera_agent_streamonoff(0,1);
	if (errorcode == ERR_SUCCESS) {
		cam_notice("%s: camera_agent_streamonoff success", __func__);		
	}else{
		cam_err("%s: camera_agent_streamonoff failed", __func__);
		return ;
	}
	cam_notice("%s exit", __func__);

}

void misp_getsysteminfo(u8 * buf)
{
	...
	if (err){
		cam_notice("%s mutex_lock_interruptible   error ", __func__);

		goto out;
		}

	err = misp_send_cmd(drv_data, ISPCMD_CAMERA_GET_SYSTEMINFORMATION, NULL, 0);
	if (err){
		cam_notice("%s misp_send_cmd   error ", __func__);
		goto out;
		}
	
	err = misp_poll_status(drv_data);
	if (err){
		cam_notice("%s misp_poll_status   error ", __func__);
		goto out;
		}
	
	err = misp_recv_data(drv_data, buf,47);
	if (err){
		cam_notice("%s misp_recv_data   error ", __func__);
		goto out;
		}
	...
	/* dump last error code */
      if (!err) {
            ...
            if(!dsm_client_ocuppy(drv_data->client_extisp)){
                 ...
                 cam_info("[I/DSM] %s dsm_client_notify \n", drv_data->client_extisp->client_name);
	      }
    }
}
...
static void hi6630_spidev0_cs_set(u32 control)
{
	...
	mini_isp_spi_cs_gpio = drv_data->plat_data.spi_cs_gpio;
...
int extisp_get_dt_data(struct device *dev, struct misp_plat_data *plat_data)
{
	...
	cam_notice("%s spi_cs_gpio is %d", __func__,plat_data->spi_cs_gpio);
	of_property_read_u32(np, "mini_isp,reset_gpio",
		&plat_data->reset_gpio);
	cam_notice("%s reset_gpio is %d", __func__,plat_data->reset_gpio);
