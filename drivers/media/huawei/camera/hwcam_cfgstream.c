...
typedef struct _tag_hwcam_cfgstream_mount_graphic_buf_req
{
	...
	struct ion_handle *handle[HWCAM_GRAPHIC_BUF_INFO_LENGTH];
} hwcam_cfgstream_mount_graphic_buf_req_t;

static struct ion_client *p_client;
...
#else
	if (!p_client) {
		printk("p_client NULL!\n");
	}
	for (; i < mbr->buf->num_fds; i++) {
		mbr->buf->data[i] = ion_share_dma_buf_fd(p_client, mbr->handle[i]);
		ion_free(p_client, mbr->handle[i]);
		mbr->handle[i] = NULL;
	}
#endif
...
#if 1
	if (!p_client) {
		p_client = hisi_ion_client_create("hwcam_cfgstream");
		if (!p_client) {
			printk("create ion client failed!\n");
		}
	}
#endif
...
#else
		mbr.handle[i] = ion_import_dma_buf(p_client, buf->data[i]);
		if (IS_ERR_OR_NULL(mbr.handle[i])) {
			printk("impoet dma buf failed!\n");
			goto fail_to_fget;
		}
#endif
    }
