...
static void meta_data_handle(struct work_struct *work)
{
	...
	if (g_gain == 0x80) {
		SETREG16(MANUAL_AWB_GAIN_B(0), b_gain);
		SETREG16(MANUAL_AWB_GAIN_GB(0), g_gain);
		SETREG16(MANUAL_AWB_GAIN_GR(0), g_gain);
		SETREG16(MANUAL_AWB_GAIN_R(0), r_gain);
	}
	expo_line = isp_hw->meta_data.ae_info.expo_line << 4;
	gain = (isp_hw->meta_data.ae_info.gain << 4)/100;
	SETREG32(REG_FW_AECAGC_MANUAL_EXPO(0), expo_line);
	SETREG16(REG_FW_AECAGC_MANUAL_GAIN(0), gain);
	/* PIPE1 */
	...
	if (g_gain == 0x80) {
		SETREG16(MANUAL_AWB_GAIN_B(1), b_gain);
		SETREG16(MANUAL_AWB_GAIN_GB(1), g_gain);
		SETREG16(MANUAL_AWB_GAIN_GR(1), g_gain);
		SETREG16(MANUAL_AWB_GAIN_R(1), r_gain);
	}
	expo_line = isp_hw->meta_data.ext_info.ext_expo_line << 4;
	gain = (isp_hw->meta_data.ext_info.ext_gain << 4)/100;
	SETREG32(REG_FW_AECAGC_MANUAL_EXPO(1), expo_line);
	SETREG16(REG_FW_AECAGC_MANUAL_GAIN(1), gain);
    ...
    do_gettimeofday(&buf->info.timestamp);
	hwisp_stream_intf_buf_done(&stream_vc->intf, buf);

#if 0
	cam_info("%s frame_index=%u", __func__ , isp_hw->meta_data.common_info.frame_index);
	cam_info("%s result_BV=%u avg_raw_lum=%u expo_time=%u expo_line=%u iso=%u gain=%u", __func__,
	...
	cam_info("%s b_gain=%u g_gain=%u r_gain=%u", __func__,
		isp_hw->meta_data.awb_info.b_gain, isp_hw->meta_data.awb_info.g_gain,
		isp_hw->meta_data.awb_info.r_gain);
	append_meta_file("/data/k3_camera/meta.raw", data_src, META_DATA_RAW_SIZE);
#endif
}
...
int hw_isp_init(struct device *pdev)
{
	...
	meta_vir_addr = (unsigned long)dma_alloc_coherent(pdev, META_DATA_RAW_SIZE, &meta_phy_addr, GFP_KERNEL);
	if (meta_vir_addr && meta_phy_addr) {
		 isp_hw->meta_viraddr = meta_vir_addr;
		 isp_hw->meta_phyaddr = meta_phy_addr;
		 cam_info("%s meta_viraddr=0x%x meta_phyaddr=0x%x size=%u", __func__,
...
int hw_isp_deinit(struct device *pdev)
{
	...
	if (isp_hw->meta_viraddr) {
		dma_free_coherent(pdev, META_DATA_RAW_SIZE,
			(void *)isp_hw->meta_viraddr, isp_hw->meta_phyaddr);
...
{
	u32 meta_phy_addr = isp_hw_data.meta_phyaddr;
	isp_hw_data_t *isp_hw = &isp_hw_data;

	if (META_CTRL_ENABLE== ctrl) {
		cam_info("%s: enable", __func__);
		//ROI smode enable
		SETREG8(REG_MAC_BASE(ISP_MAC0) + ROI_ENABLE_OFFSET,\
			(GETREG8(REG_MAC_BASE(ISP_MAC0) + ROI_ENABLE_OFFSET)) | SHIFT_BIT_0);

		SETREG16(REG_MAC_BASE(ISP_MAC0) + ROI_MEM_SIZE_OFFSET, META_DATA_RAW_SIZE);
		//mac write address
		SETREG32(REG_MAC_BASE(ISP_MAC0) + MAC_WRITE_ADDRESS_OFFSET, meta_phy_addr);

		//idi write size, half of mac size for imx135
		SETREG16(VC_IDI_SIZE, META_DATA_RAW_SIZE >> 1);
		//idi write line
		SETREG8(VC_IDI_LINES, 1);
		//mipi selection, only mipi0 supportted now
		SETREG8(VC_IDI_MIPI_INDEX, 0);
		//channel id
		SETREG8(VC_IDI_CHANNEL_ID, 0 << 2);
		//idi enable
		SETREG8(VC_IDI_ENABLE, 0x2c);

		memset((void *)isp_hw->meta_viraddr, 0x55, META_DATA_RAW_SIZE);
		isp_hw->meta_running = true;
	} else {
		cam_info("%s: disable", __func__);
		//idi disbale
		SETREG8(VC_IDI_ENABLE, 0x0);
		isp_hw->meta_running = false;
	}
}
...
void ispv3_hardware_set_ready(int port)
{
	...
	if ((port % 2) == 0)
		SETREG8(REG_PORT_READY(port), GETREG8(REG_PORT_READY(port)) | 0x1);
	else
		SETREG8(REG_PORT_READY(port), GETREG8(REG_PORT_READY(port)) | 0x4);
}
...
void ispv3_hardware_clear_ready(int port)
{
	...
	if ((port % 2) == 0)
		SETREG8(REG_PORT_READY(port), GETREG8(REG_PORT_READY(port)) & 0xfe);
	else
		SETREG8(REG_PORT_READY(port), GETREG8(REG_PORT_READY(port)) | 0xfb);
}
...
int ispv3_hardware_update_addr(isp_port_e port, hwisp_buf_t *buf)
{
	...
	if(buf->info.port.use_phy_memory == 0){
		...
		MMU_SETREG32(REG_PORT_MMU_CTL_ADDR(port), 0x2);
		...
		SETREG32(REG_PORT_MAC_TRANSID_ADDR(port), MMU_MAC_TRANSID(port % 2, mac_transid[0]));

		MMU_SETREG32(REG_PORT_MMU_AW0_ADDR(port), buf->info.y_addr_iommu);
		SETREG32(REG_PORT_WRITE_ADDR(port), buf->info.y_addr_iommu);
		...
		MMU_SETREG32(REG_PORT_MMU_RDY_ADDR(port), 0x1);
            ...
            SETREG32(REG_PORT_WRITE_ADDR(port), buf->info.y_addr_phy);
            SETREG32(REG_PORT_WRITE_ADDR_U(port), buf->info.u_addr_phy);
            SETREG32(REG_PORT_WRITE_ADDR_V(port), buf->info.u_addr_phy);
        }
...
static void isp_update_output_addr(int index, hwisp_stream_buf_info_t *output_buf)
{
	...
	SETREG32(REG_PORT_MAC_TRANSID_ADDR(port), MMU_MAC_TRANSID(port % 2, mac_transid[1]));
	...
	SETREG32(ISP_MAC_OUTPUT_ADDR(index), output_buf->y_addr_iommu);
	SETREG32(ISP_MAC_OUTPUT_U_ADDR(index), output_buf->u_addr_iommu);
...
static void ispv3_update_input_addr(hwisp_stream_buf_info_t *input_buf)
{
	...
	SETREG32(ISP_MAC_INPUT_ADDR, input_buf->y_addr_iommu);
...
static void dump_process_raw_cmdset_regs(void)
{
	...
	cam_info("ISP_INPUT_TYPE 0x%0x", MY_GETREG16(ISP_INPUT_TYPE));
	cam_info("ISP_INPUT_WIDTH 0x%0x", MY_GETREG16(ISP_INPUT_WIDTH));
	cam_info("ISP_INPUT_HEIGHT 0x%0x", MY_GETREG16(ISP_INPUT_HEIGHT));

	cam_info("ISP_ZOOMIN_RATIO 0x%0x \n", MY_GETREG16(ISP_ZOOMIN_RATIO));

	cam_info("ISP_OUTPORT_SELECTION 0x%0x", MY_GETREG16(ISP_OUTPORT_SELECTION));
	cam_info("ISP_OUT_TYPE 0x%0x", MY_GETREG16(ISP_OUT_TYPE(0)));
	cam_info("ISP_OUT_WIDTH 0x%0x", MY_GETREG16(ISP_OUT_WIDTH(0)));
	cam_info("ISP_OUT_HEIGHT 0x%0x", MY_GETREG16(ISP_OUT_HEIGHT(0)));
	cam_info("ISP_OUT_MEMORY_WIDTH 0x%0x", MY_GETREG16(ISP_OUT_MEM_WIDTH(0)));

	cam_info("ISP_READBACK_PORT_SELECTION 0x%0x", MY_GETREG16(ISP_READBACK_PORT_SELECTION));
	cam_info("ISP_READBACK_WIDTH 0x%0x", MY_GETREG16(ISP_READBACK_WIDTH));
	cam_info("ISP_READBACK_HEIGHT 0x%0x", MY_GETREG16(ISP_READBACK_HEIGHT));
	cam_info("ISP_READBACK_MEM_WIDTH 0x%0x", MY_GETREG16(ISP_READBACK_MEM_WIDTH));

	cam_info("ISP_MAC_OUTPUT_ADDR_1 0x%0x", MY_GETREG32(ISP_MAC_OUTPUT_ADDR(0)));
	cam_info("ISP_MAC_OUTPUT_U_ADDR_1 0x%0x", MY_GETREG32(ISP_MAC_OUTPUT_U_ADDR(0)));
	cam_info("ISP_MAC_OUTPUT_V_ADDR_1 0x%0x", MY_GETREG32(ISP_MAC_OUTPUT_V_ADDR(0)));

	cam_info("ISP_MAC_INPUT_ADDR 0x%0x", MY_GETREG32(ISP_MAC_INPUT_ADDR));

	cam_info("COMMAND_REG1 0x%0x \n", GETREG8(COMMAND_REG1));
}

static void isp_save_raw_info(raw_buffer_info_t *raw_info, int flow)
{
	GETREG32(ISP_EXPOSURE_OUTPUT_ADDR(flow), raw_info->exposure);
	GETREG16(ISP_GAIN_OUTPUT_ADDR(flow), raw_info->gain);
	GETREG16(ISP_MEAN_Y_OUTPUT_ADDR_ADDR(flow), raw_info->mean_y);
	GETREG16(ISP_RAW_STRETCH_LOW_LEVEL_OUTPUT_ADDR(flow), raw_info->raw_stretch_low_level);
	GETREG16(ISP_RAW_STRETCH_GAIN_OUTPUT_ADDR(flow), raw_info->raw_stretch_gain);
	GETREG16(ISP_BGAIN_OUTPUT_ADDR(flow), raw_info->b_gain);
	GETREG16(ISP_GGAIN_OUTPUT_ADDR(flow), raw_info->g_gain);
	GETREG16(ISP_RGAIN_OUTPUT_ADDR(flow), raw_info->r_gain);
	GETREG16(ISP_B_OFFSET_OUTPUT_ADDR(flow), raw_info->b_offset);
	GETREG16(ISP_G_OFFSET_OUTPUT_ADDR(flow), raw_info->g_offset);
	GETREG16(ISP_R_OFFSET_OUTPUT_ADDR(flow), raw_info->r_offset);
	GETREG16(ISP_LENS_ONLINE_BA_OUTPUT_ADDR(flow), raw_info->lens_online_ba);
	GETREG16(ISP_LENS_ONLINE_BR_OUTPUT_ADDR(flow), raw_info->lens_online_br);
	GETREG16(ISP_LENS_PROFILE_OUTPUT_ADDR(flow), raw_info->lens_profile);
}
...
static void isp_restore_raw_info(raw_buffer_info_t *raw_info, int flow)
{
	...
	SETREG32(ISP_EXPOSURE_INPUT_ADDR(flow), raw_info->exposure);
	SETREG16(ISP_GAIN_INPUT_ADDR(flow), raw_info->gain);
	SETREG16(ISP_MEAN_Y_INPUT_ADDR_ADDR(flow), raw_info->mean_y);
	SETREG16(ISP_RAW_STRETCH_LOW_LEVEL_INPUT_ADDR(flow), raw_info->raw_stretch_low_level);
	SETREG16(ISP_RAW_STRETCH_GAIN_INPUT_ADDR(flow), raw_info->raw_stretch_gain);
	SETREG16(ISP_BGAIN_INPUT_ADDR(flow), raw_info->b_gain);
	SETREG16(ISP_GGAIN_INPUT_ADDR(flow), raw_info->g_gain);
	SETREG16(ISP_RGAIN_INPUT_ADDR(flow), raw_info->r_gain);
	SETREG16(ISP_B_OFFSET_INPUT_ADDR(flow), raw_info->b_offset);
	SETREG16(ISP_G_OFFSET_INPUT_ADDR(flow), raw_info->g_offset);
	SETREG16(ISP_R_OFFSET_INPUT_ADDR(flow), raw_info->r_offset);
	SETREG16(ISP_LENS_ONLINE_BA_INPUT_ADDR(flow), raw_info->lens_online_ba);
	SETREG16(ISP_LENS_ONLINE_BR_INPUT_ADDR(flow), raw_info->lens_online_br);
	SETREG16(ISP_LENS_PROFILE_INPUT_ADDR(flow), raw_info->lens_profile);
...
int isp_reprocess_cmdset(struct reprocess_param_list *reprocess_params)
{
	...
	if (reprocess_params->output_fmt == V4L2_PIX_FMT_NV21) {
		SETREG8(REG_ISP_TOP7(pipeline), REG_UV_ORDER_V_AHEAD);
	} else if (reprocess_params->output_fmt == V4L2_PIX_FMT_NV12) {
		SETREG8(REG_ISP_TOP7(pipeline), REG_UV_ORDER_U_AHEAD);
	} else if (reprocess_params->output_fmt == V4L2_PIX_FMT_YUYV) {
		SETREG8(REG_ISP_TOP7(pipeline), REG_UV_ORDER_U_AHEAD);
	}

	SETREG16(ISP_ZOOMIN_RATIO, reprocess_params->zoom_ratio);

	CMD_SET_ISP_IN_FMT_SIZE(in_fmt, in_width, in_height);

	/* setup output attributes: configure write port */
	SETREG16(ISP_OUTPORT_SELECTION, 1 << reprocess_params->output_buf_info.port.id);
	...
	SETREG16(ISP_READBACK_PORT_SELECTION, 1 << (reprocess_params->input_buf_info.port.id - ISP_PORT_READ0));
	...
	SETREG8(COMMAND_REG1, pipeline);

	SETREG8(COMMAND_REG2, 1);

	/* REG7[0]:1-enable pingpang buffer; REG7[1]:1-enable single port ready */
	SETREG8(COMMAND_REG7, 0x02);

	dump_process_raw_cmdset_regs();
	/* send out cmdset */
	SETREG8(COMMAND_REG0, CMD_PROCESS_RAW);
...
static void  hw_isr_do_tasklet(unsigned long data)
{
	...
	//printk("int status mac1_s0 = %x mac2_s0 = %x mac3_s0 = %x\n",isp_hw_data.irq_val.mac1_irq_status_s0,isp_hw_data.irq_val.mac2_irq_status_s0,isp_hw_data.irq_val.mac3_irq_status_s0);
	//printk("int status mac1_s1 = %x mac2_s1 = %x mac3_s1 = %x\n",isp_hw_data.irq_val.mac1_irq_status_s1,isp_hw_data.irq_val.mac2_irq_status_s1,isp_hw_data.irq_val.mac3_irq_status_s1);

	//mac1
       /*done*/
	if (isp_hw_data.irq_val.mac1_irq_status_s0 & INT_ISP_MAC1_DONE0) {
		isp_write_done_handler(ISP_PORT_WRITE0);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac1_irq_status_s0 &= ~INT_ISP_MAC1_DONE0;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);

		if (isp_hw_data.meta_running)
			schedule_work(&isp_hw_data.meta_data_work);
	}

	/*start*/
	if (isp_hw_data.irq_val.mac1_irq_status_s0 & INT_ISP_MAC1_START0) {
		isp_write_start_handler(ISP_PORT_WRITE0);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac1_irq_status_s0 &= ~INT_ISP_MAC1_START0;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	/* drop */
	if (isp_hw_data.irq_val.mac1_irq_status_s0 & INT_ISP_MAC1_DROP0) {
		isp_write_drop_handler(ISP_PORT_WRITE0);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac1_irq_status_s0 &= ~INT_ISP_MAC1_DROP0;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	/* FIFO overflow */
	if (isp_hw_data.irq_val.mac1_irq_status_s0 & INT_ISP_MAC1_OVERFLOW0) {
		cam_info("mac1 overflow irq");
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac1_irq_status_s0 &= ~INT_ISP_MAC1_OVERFLOW0;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

 	 /*done*/
	if (isp_hw_data.irq_val.mac1_irq_status_s1 & INT_ISP_MAC1_DONE1) {
		isp_write_done_handler(ISP_PORT_WRITE1);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac1_irq_status_s1 &= ~INT_ISP_MAC1_DONE1;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	if (isp_hw_data.irq_val.mac1_irq_status_s1 & INT_ISP_MAC1_START1) {
		isp_write_start_handler(ISP_PORT_WRITE1);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac1_irq_status_s1 &= ~INT_ISP_MAC1_START1;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	/* drop */
	if (isp_hw_data.irq_val.mac1_irq_status_s1 & INT_ISP_MAC1_DROP1) {
		isp_write_drop_handler(ISP_PORT_WRITE1);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac1_irq_status_s1 &= ~INT_ISP_MAC1_DROP1;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	//mac2
     	/*done*/
	if (isp_hw_data.irq_val.mac2_irq_status_s0 & INT_ISP_MAC2_DONE0) {
		isp_write_done_handler(ISP_PORT_WRITE2);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac2_irq_status_s0 &= ~INT_ISP_MAC2_DONE0;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	/*start*/
	if (isp_hw_data.irq_val.mac2_irq_status_s0 & INT_ISP_MAC2_START0) {
		isp_write_start_handler(ISP_PORT_WRITE2);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac2_irq_status_s0 &= ~INT_ISP_MAC2_START0;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	/* drop */
	if (isp_hw_data.irq_val.mac2_irq_status_s0 & INT_ISP_MAC2_DROP0) {
		isp_write_drop_handler(ISP_PORT_WRITE2);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac2_irq_status_s0 &= ~INT_ISP_MAC2_DROP0;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	/* FIFO overflow */
	if (isp_hw_data.irq_val.mac2_irq_status_s0 & INT_ISP_MAC2_OVERFLOW0) {
		cam_info("mac2 overflow irq");
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac2_irq_status_s0 &= ~INT_ISP_MAC2_OVERFLOW0;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

  	 /*done*/
	if (isp_hw_data.irq_val.mac2_irq_status_s1 & INT_ISP_MAC2_DONE1) {
		isp_write_done_handler(ISP_PORT_WRITE3);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac2_irq_status_s1 &= ~INT_ISP_MAC2_DONE1;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	/* mac1 start1 */
	if (isp_hw_data.irq_val.mac2_irq_status_s1 & INT_ISP_MAC2_START1) {
		isp_write_start_handler(ISP_PORT_WRITE3);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac2_irq_status_s1 &= ~INT_ISP_MAC2_START1;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	/* drop */
	if (isp_hw_data.irq_val.mac2_irq_status_s1 & INT_ISP_MAC2_DROP1) {
		isp_write_drop_handler(ISP_PORT_WRITE3);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac2_irq_status_s1 &= ~INT_ISP_MAC2_DROP1;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	//mac3
	if (isp_hw_data.irq_val.mac3_irq_status_s0 & INT_ISP_MAC3_DONE0) {
		cam_info("mac3 done irq");
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac3_irq_status_s0 &= ~INT_ISP_MAC3_DONE0;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	/*start*/
	if (isp_hw_data.irq_val.mac3_irq_status_s0 & INT_ISP_MAC3_START0) {
		cam_info("mac3 start irq");
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac3_irq_status_s0 &= ~INT_ISP_MAC3_DROP0;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	/* drop */
	if (isp_hw_data.irq_val.mac3_irq_status_s0 & INT_ISP_MAC3_DROP0) {
	...
	/* FIFO overflow */
	if (isp_hw_data.irq_val.mac3_irq_status_s0 & INT_ISP_MAC3_OVERFLOW0) {
		cam_info("mac3 overflow irq");
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac3_irq_status_s0 &= ~INT_ISP_MAC3_OVERFLOW0;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

 	 /*done*/
	if (isp_hw_data.irq_val.mac3_irq_status_s1 & INT_ISP_MAC3_DONE1) {
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac3_irq_status_s1 &= ~INT_ISP_MAC3_DONE1;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	if (isp_hw_data.irq_val.mac3_irq_status_s1 & INT_ISP_MAC3_START1) {
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac3_irq_status_s1 &= ~INT_ISP_MAC3_START1;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	/* drop */
	if (isp_hw_data.irq_val.mac3_irq_status_s1 & INT_ISP_MAC3_DROP1) {
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac3_irq_status_s1 &= ~INT_ISP_MAC3_DROP1;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}


	/* PIPELINE0 EOF */
	if(isp_hw_data.irq_val.irq_status_4 & ISP_PIPE0_EOF_IDI) {
		ovisp23_notify_eof(0);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.irq_status_4 &= ~ISP_PIPE0_EOF_IDI;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	/* PIPELINE1 EOF */
	if(isp_hw_data.irq_val.irq_status_3 & ISP_PIPE1_EOF_IDI) {
		ovisp23_notify_eof(1);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.irq_status_3 &= ~ISP_PIPE1_EOF_IDI;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	/* PIPELINE0 SOF */
	if(isp_hw_data.irq_val.irq_status_4 & ISP_PIPE0_SOF_IDI) {
		ovisp23_notify_sof(0);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.irq_status_4 &= ~ISP_PIPE0_SOF_IDI;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	/* PIPELINE1 SOF */
	if(isp_hw_data.irq_val.irq_status_3 & ISP_PIPE1_SOF_IDI) {
		ovisp23_notify_sof(1);
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.irq_status_3 &= ~ISP_PIPE1_SOF_IDI;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}

	if(isp_hw_data.irq_val.mac3_irq_status_h & INT_ISP_MAC3_DONE1) {
		cam_info("mac3 read done irq");
		spin_lock_irqsave(&isp_hw_data.irq_status_lock, lock_flags);
		isp_hw_data.irq_val.mac3_irq_status_h &= ~INT_ISP_MAC3_DONE1;
		spin_unlock_irqrestore(&isp_hw_data.irq_status_lock, lock_flags);
	}
