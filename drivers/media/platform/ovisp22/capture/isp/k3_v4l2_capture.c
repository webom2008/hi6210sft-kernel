...
static struct dentry *debugfs_sensor_otp;//otp status interface
...
static ssize_t k3_sensors_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    /*Fix the issue that the PRI/SEC sensor's name cannot be stored in the sysfs
      if the other one is open fail*/
    if( (NULL != *pri_sensor) || (NULL != *sec_sensor)){
        if(NULL != *pri_sensor){
            snprintf(buf, BUF_LEN, "back sensor:[%s]; ", (*pri_sensor)->info.name);
        }else{
            print_error("%s: NULL pri_sensor", __FUNCTION__);
            snprintf(buf, BUF_LEN, "back sensor:[NULL]; ");
        }

        if(NULL != *sec_sensor){
            snprintf(temp_buf, BUF_LEN,  "front sensor:[%s]\n", (*sec_sensor)->info.name);
        }else{
            print_error("%s: NULL sec_sensor", __FUNCTION__);
            snprintf(temp_buf, BUF_LEN, "front sensor:[NULL]\n");
        }

        strncat(buf, temp_buf, strlen(temp_buf)+1);
...
/*
 **************************************************************************
 * FunctionName: sensor_otp_debug_read;
 ...
 **************************************************************************
 */
static ssize_t sensor_otp_debug_read(struct file *filp, char __user *buffer,
	size_t count, loff_t *ppos)
{
	char buf[10]={'\0'};
	int len =0;
	...
	print_info("otp check result:%s", buf);
...
//interface to get otp status
static const struct file_operations debugfs_sensor_otp_fops = {
	//.open = sensor_otp_debug_open,
	//.write = sensor_otp_debug_write,
	.read = sensor_otp_debug_read,
};
...
int camera_init(void)
{
	...
	debugfs_isp_reg = debugfs_create_file("isp_reg",
											...
											(void *) "isp_reg",
											&debugfs_isp_reg_fops);
	debugfs_sensor_otp = debugfs_create_file("sensor_otp", 0444, debugfs_camera_dir,
										 NULL, &debugfs_sensor_otp_fops);
