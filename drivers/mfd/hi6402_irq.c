...
static inline void hi6402_irq_pll_mode_cfg(struct hi6402_irq *irq, enum hi6402_pll_status status)
{
	...
	case HI6402_PLL_LOW_FREQ:
		...
		hi6402_irq_write(irq, HI6402_PLL_LPF_PI, 0x29);
	...
	case HI6402_PLL_HIGH_FREQ:
		...
		hi6402_irq_write(irq, HI6402_PLL_FCW_17_10, 0x70);
		hi6402_irq_write(irq, HI6402_PLL_FCW_9_2, 0xA3);
		hi6402_reg_write_bits(irq, HI6402_PLL_FCW_1_0,
				0x3<<HI6402_PLL_FCW_1_0_BIT, 0x3<<HI6402_PLL_FCW_1_0_BIT);
		hi6402_irq_write(irq, HI6402_PLL_PDIV_AD, 0xDB);
		hi6402_irq_write(irq, HI6402_PLL_LPF_PI, 0x69);
		hi6402_irq_write(irq, HI6402_PLL_DCO_INIT, 0x80);
		hi6402_irq_write(irq, HI6402_PLL_PHE_SH, 0x00);
		hi6402_reg_clr_bit(irq, HI6402_PLL_CLKTEST_EN, HI6402_PLL_CLKTEST_EN_BIT);
		hi6402_reg_clr_bit(irq, HI6402_PLL_PHE_THR_SEL, HI6402_PLL_PHE_THR_SEL_BIT);
		break;
	default:
		pr_err("no cmd %d.\n", status);
		break;
...
void hi6402_irq_mad_enable_work(struct hi6402_irq *irq, bool enable)
{
	int ret = 0;
	...
	if (enable) {
		...
		/* CP div 256 */
		hi6402_reg_write_bits(irq, HI6402_CP_CLK_DIV_REG, 0x07, 0x07);
		hi6402_reg_clr_bit(irq, HI6402_POP_IRQ_CLK_PD_REG,0 );
		/* PLL cfg */
		hi6402_irq_pll_mode_cfg(irq, HI6402_PLL_LOW_FREQ);
		hi6402_irq_write(irq, HI6402_ANA_REG69, 0x00);
		hi6402_irq_write(irq, HI6402_PLL_TRK_EN, 0x14);
		udelay(10);
		...
		hi6402_reg_set_bit(irq, HI6402_PLL_RSTN_REFDIV, HI6402_PLL_RSTN_REFDIV_BIT);
		...
		hi6402_reg_set_bit(irq, HI6402_PLL_CLK_SEL_REG, HI6402_PLL_CLK_SEL_REG_BIT);
		/* CP div 2 */
		hi6402_reg_write_bits(irq, HI6402_CP_CLK_DIV_REG, 0x00, 0x07);
		hi6402_reg_clr_bit(irq, HI6402_POP_IRQ_CLK_PD_REG,0 );
...
void hi6402_irq_pll_enable_work(struct hi6402_irq *irq, bool enable)
{
	int ret = 0;
...
static void hi6402_irq_set_pll_mode(struct hi6402_irq *irq)
{
	BUG_ON(NULL == irq);
	...
	switch(irq->pll_status) {
		case HI6402_PLL_PD:
			if (irq->ref_pll)
				hi6402_irq_pll_enable_work(irq, true);
			else if (irq->mad_need_pll)
				hi6402_irq_mad_enable_work(irq, true);
			else
				/* todo */;
			break;
		case HI6402_PLL_HIGH_FREQ:
			if (0 == irq->ref_pll) {
				hi6402_irq_pll_enable_work(irq, false);
				if(irq->mad_need_pll)
					hi6402_irq_mad_enable_work(irq, true);
			}
			break;
		case HI6402_PLL_LOW_FREQ:
			if (0 == irq->mad_need_pll || irq->ref_pll)
				hi6402_irq_mad_enable_work(irq, false);
			if (irq->ref_pll)
				hi6402_irq_pll_enable_work(irq, true);
			break;
		default:
...
void hi6402_irq_mad_pll_enable(struct hi6402_irq *irq, bool enable)
{
	BUG_ON(NULL == irq);
...
EXPORT_SYMBOL(hi6402_irq_mad_pll_enable);

void hi6402_irq_pll_enable(struct hi6402_irq *irq, bool enable)
{
	BUG_ON(NULL == irq);
	mutex_lock(&irq->pll_mutex);
	if(enable) {
...
EXPORT_SYMBOL(hi6402_irq_pll_enable);

void hi6402_irq_work_func(struct work_struct *work)
{
	struct hi6402_irq *hi6402_irq = 
		container_of(work, struct hi6402_irq, hi6402_irq_delay_work.work);
	...
	for (i = HI6402_REG_IRQ_0; i <= HI6402_REG_IRQ_2; i++) {
		...
		spin_lock_irqsave(&hi6402_irq->lock, flags);
		/* handle each irq */
		if (pending)
			for_each_set_bit(offset, &pending, HI6402_IRQ_BITS)
					generic_handle_irq(hi6402_irq->irqs[offset + irqs_offset]);
		spin_unlock_irqrestore(&hi6402_irq->lock, flags);
	}

	enable_irq(hi6402_irq->irq);

	wake_lock_timeout(&hi6402_irq->wake_lock, 3000);
...
static irqreturn_t hi6402_irq_handler(int irq, void *data)
{
	...
	disable_irq_nosync(hi6402_irq->irq);
	queue_delayed_work(hi6402_irq->hi6402_irq_delay_wq,
			&hi6402_irq->hi6402_irq_delay_work,
			msecs_to_jiffies(0));
...
{
	struct hi6402_irq *hi6402_irq = irq_data_get_irq_chip_data(d);
	unsigned int id = irqd_to_hwirq(d);
	unsigned int mask_offset = 0;

	/* get reg addr offset */
	mask_offset = HI6402_REG_IRQM_0 + (id >> 3);

	hi6402_reg_set_bit(hi6402_irq, mask_offset, (id & 0x07));
}
...
static void hi6402_irq_unmask(struct irq_data *d)
{
	...
	/* get reg addr offset */
	irq_offset = HI6402_REG_IRQ_0 + (id >> 3);
	mask_offset = HI6402_REG_IRQM_0 + (id >> 3);

	/* clear irq */
	hi6402_irq_write(hi6402_irq, irq_offset, 1 << (id & 0x07));

	hi6402_reg_clr_bit(hi6402_irq, mask_offset, (id & 0x07));
}
			...
			irq_hw_number_t hw)
{
	...
	irq_set_chip_and_handler_name(virq, &hi6402_irq_chip,
				      handle_simple_irq, "hi6402_irq");
...
static int hi6402_rr_read(struct file *file, char __user *user_buf,
...
static int hi6402_rr_write(struct file *file, const char __user *user_buf, size_t count, loff_t *ppos)
{
	char *kn_buf = NULL;
...
static int hi6402_irq_probe(struct platform_device *pdev)
{
	...
	irq->hi6402_irq_delay_wq = create_singlethread_workqueue("hi6402_irq_delay_wq");
	if (!(irq->hi6402_irq_delay_wq)) {
		pr_err("%s(%u) : workqueue create failed", __FUNCTION__,__LINE__);
		ret = -ENOMEM;
		goto irq_delay_wq_err;
	}
	INIT_DELAYED_WORK(&irq->hi6402_irq_delay_work, hi6402_irq_work_func);

	irq->irq = gpio_to_irq(irq->gpio);

	irq->domain = irq_domain_add_simple(np, HI6402_MAX_IRQS, 0,
	...
	ret = request_irq(irq->irq, hi6402_irq_handler,
	...
	if (!debugfs_create_file("rr", 0644, debug_dir, NULL, &hi6402_rr_fops))
		pr_err("hi6402: Failed to create hi6402 debugfs file\n");
...
module_platform_driver(hi6402_irq_driver);
