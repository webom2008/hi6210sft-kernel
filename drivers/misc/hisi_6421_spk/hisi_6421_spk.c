...
{
	BUG_ON(NULL == g_pdata);
	readl_relaxed(g_pdata->reg_base_addr + reg);
}
#endif
static void hisi_6421_spk_reg_write(unsigned int reg, unsigned int value)
{
	BUG_ON(NULL == g_pdata);
	writel_relaxed(value, g_pdata->reg_base_addr + reg);
}
...
static long hisi_6421_spk_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case HISI_6421_SPK_ENABLE:
		...
		/*
		 * 1. config dead time as 7us
		 * 2. sleep 7us ~ 10us
		 * 3. enable ClassD
		 */
		hisi_6421_spk_reg_write(HISI_6421_SPK_CRTL_3, 2);
		usleep_range(7, 10);
		hisi_6421_spk_reg_write(HISI_6421_SPK_CRTL_1, 1);
		break;
	case HISI_6421_SPK_DISABLE:
		hisi_6421_spk_reg_write(HISI_6421_SPK_CRTL_1, 0);
