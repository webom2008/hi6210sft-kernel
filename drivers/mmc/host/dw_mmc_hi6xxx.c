...
#define SET_SD_MMC_CARD_FLAG    (1)
#define RESET_SD_MMC_CARD_FLAG  (0)
        ...
        case MMC_POWER_UP:
            ...
            */
            ...
            if(MMC_SD == id)
            {
                ...
                if (host->card_sd_mmc == SET_SD_MMC_CARD_FLAG){
                    ret = dw_mci_hi6xxx_3_3v_signal_voltage_switch(host->slot[0]);
                    if (ret)
                        dev_err(host->dev, "card_sd_mmc regulator_enable 3.0V failed !\n");
		            ...
		            *(volatile u32 *) (priv->ioc_off+0x20) = 0;
                    ...
                    /*配置sd管脚配置*/
		            *(volatile u32 *) (priv->ioc_off+0x80c) = 0x54;
...
#if defined (CONFIG_HUAWEI_DSM)
 void dw_mci_dsm_dump(struct dw_mci  *host)
{
	if (dclient == NULL){
	...
	if(!dsm_client_ocuppy(dclient)){
		...
		dsm_client_notify(dclient, DSM_SDIO_RW_ERROR_NO);
       }else
...
static int dw_mci_hi6xxx_priv_init(struct dw_mci *host)
{
    ...
    priv->old_timing = -1;
    host->priv = priv;
    host->card_sd_mmc = RESET_SD_MMC_CARD_FLAG;
    host->sd_in_resume = 0;
...
static int dw_mci_hi6xxx_parse_dt(struct dw_mci *host)
{
    ...
    printk(KERN_ERR "######board-mmc-bus-clk is %x \n",priv->dw_mmc_bus_clk);
    ...
    printk(KERN_ERR "######dw_voltage_switch_gpio is %x \n",priv->dw_voltage_switch_gpio);
...
static void dw_mci_hi6xxx_tuning_fail_excprocess(struct dw_mci *host)
{
    ...
    printk(KERN_ERR "before  mshci_tuning_fail_excprocess: INTMASK=%x, TMOUT=%x, RINTSTS=%x, TCBCNT=%x, TBBCNT=%x, IDSTS=%x, BUFADDR=%x \n",_intmask_flags,_timeout,_rintsts,_tcbcnt,_tbbcnt,_idsts,_bufaddr);
	...
	//mci_writel(host, INTMASK, intmask_flags);
    ...
    printk(KERN_ERR "after mshci_tuning_fail_excprocess: CTR=%x, INTMASK=%x, RINTSTS=%x, TCBCNT=%x, TBBCNT=%x, IDSTS=%x, BUFADDR=%x， CLKENA=%x \n",retval,intmask_flags,_rintsts,_tcbcnt,_tbbcnt,_idsts,_bufaddr,_clkena);
}
...
int dw_mci_hi6xxx_mmc_es_tuning(struct dw_mci_slot *slot)
{
    ...
    printk(KERN_ERR "mshci_execute_tuning: get into tuning \n");
    ...
    printk(KERN_ERR "mshci_execute_tuning: the is_tuning_all_phased is %x,%x \n",is_tuning_all_phased,host->saved_tuning_phase);
...
retry:
    ...
    if (is_tuning_all_phased)
    {
        ...
        {
            printk(KERN_ERR "mshci_execute_tuning: the %d times \n",i);
            ...
            printk("mshci_execute_tuning: ctrl is the %x  \n",mshci_ctrl);
    ...
    printk(KERN_ERR "tuning_point_len is %d,best_tuning_point_len is %d,start_point is %d, best_start_point is %d \n",tuning_point_len,best_tuning_point_len,start_point,best_start_point);
...
retry:
    ...
    if (is_tuning_all_phased)
    {
        ...
        for (i = 0;i <= MMC_MAX_TUNING_CS_VALUE; i++)
        {
            ...
            printk(KERN_ERR "mshci_execute_tuning: the %d times, odd_even is %d , tuning_phase is %d \n",i,odd_even,tuning_phase);
    ...
    printk(KERN_ERR "dw_mci_hi6xxx_tuning: before ######## our best tuning is %d \n",best_tuning_sample_point);
    ...
    printk(KERN_ERR "dw_mci_hi6xxx_tuning: after ######## our best tuning is %d \n",best_tuning_sample_point);
...
int dw_mci_hi6xxx_sd_tuning(struct dw_mci_slot *slot)
{
    ...
    printk(KERN_ERR "dw_mci_hi6xxx_sd_tuning: get into tuning \n");

    /*如果saved_tuning_phase 不等于INVALID_TUNING_PHASE，不turning */
    is_tuning_all_phased = !(mmc->card && (INVALID_TUNING_PHASE != host->saved_tuning_phase));

    printk(KERN_ERR "dw_mci_hi6xxx_sd_tuning: the is_tuning_all_phased is %x,%x \n",is_tuning_all_phased,host->saved_tuning_phase);
...
retry:
    ...
    if (is_tuning_all_phased)
    {
        ...
        for (i = 0;i <= MMC_MAX_TUNING_VALUE; i++)
        {
            printk(KERN_ERR "dw_mci_hi6xxx_sd_tuning: the %d times \n",i);
            ...
            printk("dw_mci_hi6xxx_sd_tuning: ctrl is the %x  \n",mshci_ctrl);
...
static void dw_mci_hi6xxx_hw_reset(struct dw_mci *host)
{
	unsigned int ctype_flags;
	unsigned int uhs_flags;
	unsigned int fifoth;
    ...
    /*配置寄存器CTRL bit25为1-使能IDMA；bit4为1-使能MMC1中断*/
	mci_writel(host, CTRL, 0x2000010);

    /*配置MMC1内部寄存器，重新SD接口时钟*/
	host->clock_to_restore = host->bus_hz;
...
static const struct dw_mci_drv_data hi6xxx_drv_data = {
	...
	.hw_reset           = dw_mci_hi6xxx_hw_reset,
...
static int dw_mci_hi6xxx_suspend(struct device *dev)
{
    ...
    if ((host->hw_mmc_id == 1) && (slot->mmc->card && mmc_card_mmc(slot->mmc->card))){
        host->card_sd_mmc = SET_SD_MMC_CARD_FLAG;
        dev_info(host->dev, "%s: SD mmc card in\n", __func__);
    }
