...
static struct bq2419x_high_temp_cp japan_temp_cp[] ={
...
static void bq2419x_get_boardid_japan_charge_parameter(struct bq2419x_device_info *di)
{
    ...
    if(ret)
        dev_err(di->dev, " bq2419x get japan charge parameter from dts fail \n");
...
#if defined(CHIP_BB_HI6210) /*V8´ò×®*/
...
static int is_aux_batt_exist(struct bq2419x_device_info *di)
{
    ...
    int aux_batt_exist = 0;
     if (!(of_property_read_u32(di->dev->of_node, AUX_BATT, &aux_batt_exist))){
        return 0;
    }
...
static void bq2419x_otg_int_work(struct work_struct *work) {
    ...
    if (di->otg_int_work_cnt == 2){
        dev_info(di->dev, "OTG_INT reg09=0x%x first, then=0x%x\n", init_read_reg, read_reg);
    }
    else{
        dev_info(di->dev, "OTG reg09=0x%x first, then=0x%x\n", init_read_reg, read_reg);
    }
    ...
    if (di->otg_int_work_cnt){
        di->otg_int_work_cnt --;
        schedule_delayed_work(&di->otg_int_work, msecs_to_jiffies(1000));
    }
}
...
static struct bq2419x_device_info *g_di;
...
MODULE_DEVICE_TABLE(of, bq2419x_charger_match_table);
