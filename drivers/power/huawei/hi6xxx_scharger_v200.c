...
STATIC void hi6521_monitor_battery_ntc_japan_charging(struct hi6521_device_info *di)
{
    ...
    di->chg_en = (di->chg_en & di->factory_flag) | di->not_stop_chrg_flag;
...
STATIC void hi6521_monitor_battery_ntc_charging(struct hi6521_device_info *di)
{
   ...
   if((CHARGER_TYPE_NON_STANDARD == di->event))
    ...
    hwlog_info("ntc_charging batt_temp:%d, batt_volt:%d, iin:%d, ichg:%d, chg_en:%d, status0:0x%x\n", \
        batt_temp, di->battery_voltage, di->cin_limit, di->chg_fast_ichg, di->chg_en, read_reg);
    if((read_reg & CHG_STATUS0_STAT_CHAEGE_DONE) == CHG_STATUS0_STAT_CHAEGE_DONE)
        return;
...
STATIC void hi6521_irq_work_handle(struct work_struct *work)
{
    ...
    if (irq_status[3] & CHG_IRQ3_LCD_LDO_OCP){
        ...
        hi6521_config_lcd_ldo_enable(di);
        hi6521_config_lcd_boost_enable(di);
        hwlog_err("SCHARGER ERROR:CHG_IRQ3_LDO_OCP,reg[0x0A]:0x%x.\n",irq_status[3]);
...
STATIC ssize_t hi6521_set_cin_limit(struct device *dev,
                  struct device_attribute *attr,
                  const char *buf, size_t count)
{
    long val;
...
STATIC ssize_t hi6521_show_cin_limit(struct device *dev,
                  struct device_attribute *attr,
                  char *buf)
{
    int val;
...
STATIC ssize_t hi6521_set_fast_current(struct device *dev,
                  struct device_attribute *attr,
                  const char *buf, size_t count)
{
    long val;
...
STATIC ssize_t hi6521_show_fast_current(struct device *dev,
                  struct device_attribute *attr,
                  char *buf)
{
    int val;
...
*/
static ssize_t hi6521_show_not_limit_chrg_flag(struct device *dev,
                  struct device_attribute *attr,
                  char *buf)
{
    unsigned long val;
...
static ssize_t hi6521_set_not_limit_chrg_flag(struct device *dev,
                  struct device_attribute *attr,
                  const char *buf, size_t count)
{
    long val;
...
STATIC ssize_t hi6521_set_temp_current_iin(struct device *dev,
                  struct device_attribute *attr,
                  const char *buf, size_t count)
{
    long val;
...
STATIC ssize_t hi6521_show_temp_current_iin(struct device *dev,
                  struct device_attribute *attr,
                  char *buf)
{
    unsigned int val;
...
STATIC ssize_t hi6521_set_temp_current_ichg(struct device *dev,
                  struct device_attribute *attr,
                  const char *buf, size_t count)
{
    long val;
...
STATIC ssize_t hi6521_show_temp_current_ichg(struct device *dev,
                  struct device_attribute *attr,
                  char *buf)
{
    unsigned int val;
...
STATIC DEVICE_ATTR(cin_limit, (S_IWUSR | S_IRUGO),
                hi6521_show_cin_limit,
                hi6521_set_cin_limit);
STATIC DEVICE_ATTR(fast_voltage, (S_IWUSR | S_IRUGO),
                hi6521_show_fast_voltage,
                hi6521_set_fast_voltage);
STATIC DEVICE_ATTR(fast_current, (S_IWUSR | S_IRUGO),
                hi6521_show_fast_current,
                hi6521_set_fast_current);
...
STATIC DEVICE_ATTR(temp_current_iin, (S_IWUSR | S_IRUGO),
                hi6521_show_temp_current_iin,
                hi6521_set_temp_current_iin);
STATIC DEVICE_ATTR(temp_current_ichg, (S_IWUSR | S_IRUGO),
                hi6521_show_temp_current_ichg,
                hi6521_set_temp_current_ichg);
static DEVICE_ATTR(not_limit_chrg_flag, (S_IWUSR | S_IRUGO),
                hi6521_show_not_limit_chrg_flag,
                hi6521_set_not_limit_chrg_flag);
