...
static void set_sctrl_memory(unsigned int repair_disable_val)
{
	...
	int memory_repair_flag = 0;

	memory_repair_flag = hisi_sctrl_regu_read(SCTRL_MEMORY_REPAIR_FLAG);
	if (!(memory_repair_flag & 0x01)) {
		return;
	}
	...
	while (1) {
		...
		if (0 == ret) {
			...
			if (MEMORY_REPAIR_DELAY_TIME <= udelay_flag) {
				pr_err("%s: [%d]memory repair donot complete!\n", __func__, repair_disable_val);
				BUG_ON(1);
				break;
			}
		}
	...
	if (0 != hisi_sctrl_regu_read(0x220)) {
		pr_err("%s: [%d]memory repair err_flag:0x%x\n", __func__, repair_disable_val, hisi_sctrl_regu_read(0x220));
	}
