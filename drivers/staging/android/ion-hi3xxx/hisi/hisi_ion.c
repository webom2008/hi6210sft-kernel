...
static void hisi_ion_get_heap(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(hisi_ion_heaps); i++) {
		...
		printk(KERN_INFO "[ION]heap: %s, id: %d, type: %d, base: %ldM, size: %dM\n",
			hisi_ion_heaps[i].name, hisi_ion_heaps[i].id, hisi_ion_heaps[i].type,
			hisi_ion_heaps[i].base >> 20, hisi_ion_heaps[i].size >> 20);
	}
}
...
#ifdef CONFIG_COMPAT
	idev = ion_device_create(compat_hisi_ion_custom_ioctl);
#else
