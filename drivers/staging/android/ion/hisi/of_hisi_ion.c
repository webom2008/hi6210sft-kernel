...
static struct hisi_ion_name_id_table name_id_table[] __initdata = {
    ...
    {"gralloc-iommu", ION_GRALLOC_HEAP_ID},
...
static bool iommu_enable = false;
static unsigned long long system_type = FULL_SYSTEM;
...
EXPORT_SYMBOL(hisi_ion_get_media_mode);
...
static int __init of_hisi_ion_setup_platform_data(struct platform_device *dev)
{
	...
	for_each_child_of_node(node, np) {
        ...
        hisi_ion_platform_heap[index].cached = 0x1;
				            ...
				            unsigned long arg)
{
	...
	case ION_HISI_CUSTOM_GET_KERN_VA:
	{
		...
		pr_info("handle is %d\n",data.handle_id);
		handle = hisi_ion_handle_get_by_id(client,data.handle_id);
        kern_va = (unsigned long)ion_map_kernel(client, handle);
        if ((IS_ERR((void *)kern_va))|| (!kern_va)) {
            /*
            * just for handle ref -1
            */
            pr_err("in ION_HISI_CUSTOM_GET_KERN_VA ion_map_kernel is failed \n");
            ion_free(client, handle);
	...
	case ION_HISI_CUSTOM_GET_MEDIA_HEAP_MODE:
	case ION_HISI_CUSTOM_ISSUPPORT_IOMMU: {
		...
		data.is_support_iommu = hisi_ion_get_media_mode();
		pr_info("iommu_is_init is %#x\n",(unsigned int)data.is_support_iommu);
...
static int __init hisi_ion_probe(struct platform_device *pdev)
{
    ...
    if (true == iommu_enable){
        iommu_init(iommu_build_params());
    }
...
static int __init hisi_ion_no_iommu_probe(struct platform_device *pdev)
{
    return hisi_ion_probe(pdev);
}
...
static int hisi_ion_no_iommu_remove(struct platform_device *pdev)
{
    int ret = hisi_ion_remove(pdev);
...
static struct of_device_id hisi_ion_no_iommu_match_table[] = {
	{.compatible = "hisi,hi6xxx-ion-no-iommu"},
	{},
};

static struct platform_driver hisi_ion_no_iommu_driver = {
	.probe = hisi_ion_no_iommu_probe,
	.remove = hisi_ion_no_iommu_remove,
	.driver = {
		.name = "hi6xxx-ion-no-iommu",
		.of_match_table = hisi_ion_no_iommu_match_table,
	},
};

static int __init early_parse_system_type_cmdline(char *str)
{
	char *endp = NULL;
	pr_err(" *********** in %s str\n",__func__);
    if (str && *str)
    	system_type = simple_strtoull(str,&endp,0);
	return 0;
}
early_param("trial_version", early_parse_system_type_cmdline);
...
static int __init hisi_ion_iommu_enable(char *str)
{
    iommu_enable = true;
    return 0;
}
early_param("iommu_enable", hisi_ion_iommu_enable);
...
static int __init hisi_ion_init(void)
{
    ...
    if (true == iommu_enable){
        ...
        ret = platform_driver_register(&hisi_ion_no_iommu_driver);
    }
    return ret;
}
...
static void __exit hisi_ion_exit(void)
{
    if (true == iommu_enable){
        ...
        platform_driver_unregister(&hisi_ion_no_iommu_driver);
    }
    return;
}
