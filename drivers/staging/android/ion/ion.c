...
#define ION_FLUSH_ALL_CPUS_CACHES_SZ   (256<<10)  /*256k*/
/**
...
struct ion_buffer * ion_get_buffer_by_handle(struct ion_handle *handle)
{
	return (handle->buffer);
}
EXPORT_SYMBOL(ion_get_buffer_by_handle);
...
struct ion_handle *hisi_ion_handle_get_by_id(struct ion_client *client,
						int id)
{
	return ion_handle_get_by_id(client,id);
}
EXPORT_SYMBOL(hisi_ion_handle_get_by_id);
...
/*pr_err("%s: len %zu align %zu heap_id_mask %u flags %x\n", __func__,
	...
	len = PAGE_ALIGN(len);
	iommu_enable = hisi_ion_get_media_mode();
	...
	plist_for_each_entry(heap, &dev->heaps, node) {
		...
		if (!((1 << heap->id) & heap_id_mask)){
                        if(iommu_enable && (heap_id_mask == ION_HEAP(ION_OVERLAY_HEAP_ID)) && 
                                        heap->id == ION_GRALLOC_HEAP_ID){
	...
	if (buffer == NULL){
	    pr_err("in %s buffer is null",__func__);
...
extern hi6xxx_ion_flush_cache_all();

static void ion_flush_all_cpus_caches(void)
{
	hi6xxx_ion_flush_cache_all();
...
int ion_sync_for_device(struct ion_client *client, int fd)
{
	...
	if(buffer->size > ION_FLUSH_ALL_CPUS_CACHES_SZ){
