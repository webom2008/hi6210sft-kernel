...
#define _KERNEL_ADDR_32_    0xC0000000
#define _KERNEL_ADDR_64_    0xFFFFFFFF80000000UL
...
static unsigned long device_file_cnt = 1;
...
#ifdef CONFIG_ARM64
static noinline int invoke_smc_cmd(u64 cmd_addr)
{
...
#else
static unsigned int invoke_smc_cmd(unsigned int cmd_addr)
{
    ...
    if(operation){
        for(i = 0; i < 4; i++){
            ...
            if((param_type == TEEC_MEMREF_TEMP_INPUT) ||
                ...
                if((param_type == TEEC_MEMREF_TEMP_INPUT) ||
                    (param_type  == TEEC_MEMREF_TEMP_INOUT)){
                    /* buffer is in kernel space(kernel driver call kernel TEE API) */
                    if(virt_addr_valid(client_param->memref.buffer)){
            ...
            /* register buffer */
                ...
                trans_paramtype_to_tee[i] = param_type - 8;
            }
            /* value */
    ...
    if(tee_ret != 0){
        while(tee_ret == TEEC_PENDING2 || tee_ret == TEEC_PENDING){
            if(tee_ret == TEEC_PENDING2){
                ...
                if(wq){
                    TCDEBUG("before wait event \n");
                    if(wait_event_interruptible(wq->send_cmd_wq,
                        wq->send_wait_flag)) {
                        ret = -ERESTARTSYS;
                        TCERR("wait event error\n");
                        goto error;
                    }
                    wq->send_wait_flag = 0;
                }
        ...
        TC_NS_Operation *operation, TC_NS_DEV_File *dev_file)
{
    ...
    for (index = 0; index < 4; index++) {
        ...
        if((param_type == TEEC_MEMREF_TEMP_INPUT) ||
            ...
            (param_type == TEEC_MEMREF_PARTIAL_INOUT)){
            /* TEEC_MEMREF_PARTIAL_INPUT - TEE_PARAM_TYPE_MEMREF_INPUT = 8 */
            trans_paramtype_to_tee[index] = param_type - 8;
        } else if((param_type == TEEC_VALUE_INPUT)||
            (param_type == TEEC_VALUE_OUTPUT)||
            (param_type == TEEC_VALUE_INOUT)){
            /* TEEC_VALUE_INPUT equal to TEE_PARAM_TYPE_VALUE_INPUT */
...
static int TC_NS_alloc_exception_mem(void)
{
    TC_NS_SMC_CMD smc_cmd = {0};
    unsigned int smc_cmd_phys;
    int ret;
...
static int TC_NS_store_exception_info(void)
{
    ...
    if(IS_ERR(filep)){
        TCDEBUG("Failed to filp_open exception_log.log, filep=0x%lx\n", (unsigned long)filep);
        return -1;
    }

    TCDEBUG("Succeed to filp_open exception_log.log, filep=0x%lx\n", (unsigned long)filep);
...
static long TC_NS_ClientIoctl(struct file *file, unsigned cmd,
    ...
    unsigned int dev_file_id = (unsigned long)file->private_data;
...
long TC_NS_ClientIoctl_from_kernel(unsigned int dev_id, unsigned cmd,
    ...
    int ret = TEEC_ERROR_GENERIC;
    void *argp = (void __user *) arg;
    unsigned int dev_file_id = dev_id;
...
static int TC_NS_ClientClose(struct inode *inode, struct file *file){
    ...
    list_for_each_entry_safe(dev, dev_temp, &tc_ns_dev_list.dev_file_list, head){
        if(dev->dev_file_id == (unsigned long)file->private_data){
...
static int TC_NS_ClientMalloc(struct file *filp, struct vm_area_struct *vma){
    ...
    mutex_lock(&tc_ns_dev_list.dev_lock);
    list_for_each_entry(dev_file, &tc_ns_dev_list.dev_file_list, head){
        if(dev_file->dev_file_id == (unsigned long)filp->private_data){
            find_dev = 1;
            dev_file->shared_mem_cnt++;
...
#ifdef CONFIG_COMPAT
int TC_NS_compat_ClientIoctl(struct file *flie, unsigned cmd, unsigned long arg)
{
    long ret = -ENOIOCTLCMD;
    arg = (unsigned long) compat_ptr(arg);
    ret = TC_NS_ClientIoctl(flie, cmd, arg);
    return ret;
}
#endif
...
#ifdef CONFIG_COMPAT
        .compat_ioctl = TC_NS_compat_ClientIoctl,
#endif
...
void smc_thread_work(void)
{
    ...
    while(1){
        ...
        rmb();
        ret = invoke_smc_cmd(p_smc_data->cmd_addr);
        wmb();
...
static __init int TC_NS_ClientInit(void)
{
    ...
    if (ret) {
        ...
        TCDEBUG("get sec_s_buring regulator success!\n");
    }
