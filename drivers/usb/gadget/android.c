...
static void android_enable(struct android_dev *dev)
{
    ...
    if (WARN_ON(!dev->disable_depth))
        return;
    printk(KERN_INFO "android device enable depth: %d\n", dev->disable_depth);
    if (--dev->disable_depth == 0) {
        printk(KERN_INFO "usb_add_config +\n");
        usb_add_config(cdev, &android_config_driver,
                    android_bind_config);
        printk(KERN_INFO "usb_gadget_connect +\n");
...
static void android_disable(struct android_dev *dev)
{
    ...
    printk(KERN_INFO "%s +\n",__func__);
    printk(KERN_INFO "android device disable depth: %d\n", dev->disable_depth);
    if (dev->disable_depth++ == 0) {
        gser_num = 0;
        rndis_num = 0;
        printk(KERN_INFO "usb_gadget_disconnect +\n");
        usb_gadget_disconnect(cdev->gadget);
        /* Cancel pending control requests */
        printk(KERN_INFO "usb_ep_dequeue +\n");
        usb_ep_dequeue(cdev->gadget->ep0, cdev->req);
        printk(KERN_INFO "usb_remove_config +\n");
                ...
                const char *buff, size_t size)
{
    ...
    printk("%s %d dev->enabled is %d\n", __FUNCTION__,__LINE__,dev->enabled);
    if (enabled && !dev->enabled) {
        printk("%s %d android_enable \n", __FUNCTION__,__LINE__);
        /*
