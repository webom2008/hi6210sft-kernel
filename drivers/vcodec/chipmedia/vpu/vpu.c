...
static int s_vpu_setirq_timeout = 0;
	...
	case VDI_IOCTL_WAIT_INTERRUPT:
		{
			...
			if (signal_pending(current)) {
				vpu_loge("###### error signal_pending failed\n");
	...
	case VDI_IOCTL_GET_INSTANCE_POOL:
		{
            ...
            else
			{
				...
				else
                {
		            ...
		            s_ion_instance_pool_handle = ion_alloc(s_ion_instance_pool_client, IOMMU_PAGE_ALIGN(s_instance_pool.size), 0, ION_HEAP(ION_GRALLOC_HEAP_ID), 0);
		            ...
		            if (0 != ion_phys(s_ion_instance_pool_client, s_ion_instance_pool_handle, (ion_phys_addr_t *) &s_instance_pool.phys_addr, &mm_size))
		            {
		            ...
		            s_instance_pool.size = (unsigned int)mm_size;
    ...
    case VDI_IOCTL_RESERVED_MEMORY_ALLOC:
        {
            ...
            s_ion_reserved_mem_handle = ion_alloc(s_ion_reserved_mem_client, IOMMU_PAGE_ALIGN(s_video_memory.size), 0, ION_HEAP(ION_GRALLOC_HEAP_ID), 0);
            ...
            if (0 != ion_phys(s_ion_reserved_mem_client, s_ion_reserved_mem_handle, (ion_phys_addr_t *) &s_video_memory.phys_addr, &mm_size))
            {
                vpu_loge("fail to  ion_phys().\n");
                ret = -EFAULT;
                goto ERROR_RESERVED_MEMORY_ALLOC;
            ...
            s_video_memory.size = (unsigned int)mm_size;
...
static int vpu_map_to_physical_memory(struct file *fp, struct vm_area_struct *vm)
{
    ...
    if (1 == s_vpu_systemMMU_support)
    {
        ...
        if(vm->vm_pgoff == (s_instance_pool.phys_addr>>PAGE_SHIFT))        
        {
            ...
            {
                vm->vm_page_prot = pgprot_noncached(vm->vm_page_prot);
                ret = iommu_remap_pfn_range(s_ion_instance_pool_handle,vm, 0, 0, 0, PROT_READ | PROT_WRITE);
                if (ret != 0)
                {
        ...
        else 
        {
	        ...
	        {
	            vm->vm_page_prot = pgprot_noncached(vm->vm_page_prot);
	            ret = iommu_remap_pfn_range(s_ion_reserved_mem_handle,vm, 0, 0, 0, PROT_READ | PROT_WRITE);
	            if (ret != 0)
	            {
	                vpu_loge("iommu_remap_pfn_range err.\n");
	                ret = -EAGAIN;
	            }
	        }
        } 
...
static int vpu_probe(struct platform_device *pdev)
{
    ...
    if (ION_IOMMU_MODE == hisi_ion_get_media_mode())
    {
        s_vpu_systemMMU_support = 1;
    }
    else
    {
        s_vpu_systemMMU_support = 0;
    }
...
static void vpu_dump_power_reg(int power)
{
    ...
    if (1 == power) {
        reg_value = phy_reg_readl(SOC_MEDIA_SCTRL_BASE_ADDR, SOC_MEDIA_SCTRL_SC_MEDIA_SUBSYS_CTRL5_ADDR(0), 0, 31);
        vpu_logi("SOC_MEDIA_SCTRL_SC_MEDIA_SUBSYS_CTRL5_ADDR = 0x%0x,  VPU, BIT[8].\n", reg_value);
    }
}
#endif
