    ...
    case JDI_IOCTL_RESERVED_MEMORY_ALLOC:
        {
            ...
            s_ion_handle = ion_alloc(s_ion_client,
                                     IOMMU_PAGE_ALIGN(s_jpeg_memory.size),
                                     IOMMU_PAGE_SIZE,
                                     ION_HEAP(ION_GRALLOC_HEAP_ID),
                                     0);
            ...
            if (0 != ion_phys(s_ion_client,
                                s_ion_handle,
                            (ion_phys_addr_t *) &s_jpeg_memory.phys_addr,
                            &mm_size))
            {
            ...
            s_jpeg_memory.size = (unsigned int)mm_size;
            s_jpeg_memory.base = 0;
...
STATIC int jpu_release(struct inode *inode, struct file *filp)
{
    ...
    if (s_jpu_instance_pool.size != 0)
    {
        ...
        memset(&s_jpu_instance_pool, 0x00, sizeof(jpudrv_buffer_t));
    }
...
STATIC int jpu_map_to_physical_memory(struct file *fp, struct vm_area_struct *vm)
{
    ...
    if (1 == s_jpu_systemMMU_support)
    {
        ...
        {
            ret = iommu_remap_pfn_range(s_ion_handle, vm, 0, 0, 0, PROT_READ | PROT_WRITE);
            if (ret != 0)
            {
                jpu_loge("iommu_remap_pfn_range err.");
            }
        }
    }
    else
...
ERROR_PROVE_DEVICE:
    ...
    if (1 != s_jpu_systemMMU_support && s_jpeg_memory.size)
    {
...
STATIC int jpu_remove(struct platform_device *pdev)
{
    ...
    if (s_jpu_instance_pool.size)
    {
        jpu_free_dma_buffer(&s_jpu_instance_pool);
        s_jpu_instance_pool.base = 0;
    }

    if (s_jpeg_memory.size)
    {
        video_iounmap((void *)s_jpeg_memory.base);
        memset(&s_jpeg_memory, 0x00, sizeof(jpudrv_buffer_t));
        jmem_exit(&s_jmem);
