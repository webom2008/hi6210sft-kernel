...
static int dpe_on(struct platform_device *pdev)
{
	...
	if (hisifd->index != PRIMARY_PANEL_IDX) {
		if (hisifd_list[PRIMARY_PANEL_IDX] &&
			(hisifd_list[PRIMARY_PANEL_IDX]->panel_info.vsync_ctrl_type & VSYNC_CTRL_CLK_OFF)) {
			HISI_FB_DEBUG("fb%d, pdp clk enable!\n", hisifd->index);
			dpe_clk_enable(hisifd_list[PRIMARY_PANEL_IDX]);
		}
	}
...
static int dpe_off(struct platform_device *pdev)
{
	...
	if (hisifd->index != PRIMARY_PANEL_IDX) {
		if (hisifd_list[PRIMARY_PANEL_IDX] &&
			(hisifd_list[PRIMARY_PANEL_IDX]->panel_info.vsync_ctrl_type & VSYNC_CTRL_CLK_OFF)) {
			HISI_FB_DEBUG("fb%d, pdp clk disable!\n", hisifd->index);
			dpe_clk_disable(hisifd_list[PRIMARY_PANEL_IDX]);
		}
	}
