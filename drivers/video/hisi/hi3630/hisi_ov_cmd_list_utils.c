...
int cmdlist_add_nop_list(struct hisi_fb_data_type *hisifd, struct list_head *cmdlist_head, int is_first)
{
	dss_cmdlist_node_t * cmdlist_node = NULL;
	int ret = 0;
	int id = 0;
...
int cmdlist_config_start(struct hisi_fb_data_type *hisifd, uint32_t wbe_chn)
{
	...
	cmdlist_add_nop_list(hisifd, &hisifd->offline_cmdlist_head[wbe_chn], TRUE);
	cmdlist_add_nop_list(hisifd, &hisifd->offline_cmdlist_head[wbe_chn], FALSE);
