...
int hisi_dss_scp_config(struct hisi_fb_data_type *hisifd, dss_layer_t *layer, bool first_block)
{
	dss_scp_t *scp = NULL;
	...
	if (is_mipi_cmd_panel(hisifd)) {
		if (hisifd->vactive0_start_flag == 0) {
			ret = wait_event_interruptible_timeout(hisifd->vactive0_start_wq,
				hisifd->vactive0_start_flag, HZ / 5);
			if (ret <= 0) {
		...
		ret = wait_event_interruptible_timeout(hisifd->vactive0_start_wq,
			(prev_vactive0_start != hisifd->vactive0_start_flag), HZ / 5);
		if (ret <= 0) {
