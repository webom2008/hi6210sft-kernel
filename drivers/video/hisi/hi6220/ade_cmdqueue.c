...
static u32 *getCtranYuv2rgbCscArray(int yuv_info, int *len)
{
    switch (yuv_info) {
...
static u32 *getCtranYuv2rgbChdcArray(int yuv_info, int *len)
{
    switch (yuv_info) {
        case ADE_YUV_601_WIDE:
...
int ade_cmdq_get_cmdfile_mem(struct ion_client *ade_ion_client, struct  cmdfile_buffer  *cf)
{
    ...
    if (ion_phys(ade_ion_client, cf->cmd_ion_handle, (ion_phys_addr_t *)(&cf->paddr), &cf->buff_size) < 0) {
        balongfb_loge("ion_alloc alloc handle error \n");
        cf->paddr = 0xdeadbeaf;
        return -EINVAL;
    }
