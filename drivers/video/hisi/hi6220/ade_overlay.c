...
void ade_overlay_commit_set_disp_ch(struct ade_compose_data_type *ade_pri_data, struct overlay_compose_info *pcomp_info, struct cmdfile_buffer *cf_buff)
{
    ...
    BUG_ON((pcomp_info == NULL) || (cf_buff == NULL));
        ...
        struct cmdfile_buffer *cf_buff)
{
    ...
    if (wait_event_interruptible_timeout(ade_pri_data->wait_res_cmpl, ade_pri_data->res_switch_cmpl, HZ) <= 0) {
...
STATIC void ade_set_medianoc_for_dfs(struct balong_fb_data_type *balongfd)
{
    u32  tmp1 = 0;
    u32  tmp2 = 0;
    BUG_ON(balongfd == NULL);
...
#define BACK_UP_ADE_TOP_REG_CNT     (14)
    ...
    set_TOP_ADE_EN(ade_base, ADE_TRUE);
    if (wait_event_interruptible_timeout(balongfd->ade_pri_data->wait_res_cmpl, balongfd->ade_pri_data->res_switch_cmpl, HZ) <= 0) {
