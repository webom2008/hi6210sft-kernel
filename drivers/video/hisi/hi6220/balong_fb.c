...
STATIC int balongfb_sbl_set_value(struct fb_info *info, unsigned long *argp)
{
    ...
    if (balongfd->panel_info.sbl_enable) {
        ...
        balongfd->sbl_lsensor_value =  val & 0xffff;
        balongfd->sbl_enable = (val >> 16) & 0xf;

        if (!balongfd->panel_power_on) {
            /*sbl first set, but panel is off, can't set sbl, save sbl value and set it in the panel on*/
            up(&balong_fb_blank_sem);
            return 0;
        }
        /*
           ade_sbl_ctrl_set(balongfd);
           */
...
EXPORT_SYMBOL_GPL(g_lcd_name_buf);
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    snprintf(buf,PAGE_SIZE,"%s %d x %d\n",g_lcd_name_buf,balongfd->panel_info.xres, balongfd->panel_info.yres);

    ret = strlen(buf) + 1;
    return ret;

}
static DEVICE_ATTR(lcd_info, S_IRUGO, lcd_info_show, NULL);

static struct attribute *lcd_fb_attrs[] = {
    &dev_attr_lcd_info.attr,
    NULL,
};

static struct attribute_group lcd_fb_attr_group = {

    .name = "lcd_dev",
    .attrs = lcd_fb_attrs,
};
STATIC int lcd_fb_sysfs_create(struct platform_device *pdev)

{
    int ret = 0;

    ret = sysfs_create_group(&pdev->dev.kobj, &lcd_fb_attr_group);
    if (ret) {
...
static struct attribute *balong_fb_attrs[] = {
    NULL
};
...
static struct attribute_group balong_fb_attr_group = {
    .attrs = balong_fb_attrs,
};

STATIC int balong_fb_sysfs_create(struct platform_device *pdev)
{
    int ret = 0;

    ret = sysfs_create_group(&pdev->dev.kobj, &balong_fb_attr_group);
    if (ret) {
        balongfb_loge("create sysfs file failed, error=%d!\n", ret);
        return ret;
    }
...
STATIC void balong_fb_sysfs_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &balong_fb_attr_group);
}
#if 0
STATIC void frame_end_workqueue(struct work_struct *ws)
{
    ...
    balongfd = container_of(ws, struct balong_fb_data_type, frame_end_work);
    BUG_ON(balongfd == NULL);
    pdata = (struct balong_fb_panel_data *)balongfd->pdev->dev.platform_data;
    ...
    if (balongfd->bl_level != balongfd->bl_level_cmd) {
        balongfb_logi_backlight_debugfs("set backlight balongfd->bl_level_cmd = %d \n", balongfd->bl_level_cmd);
...
#endif
#if 0
STATIC int init_frame_end_workqueue(struct balong_fb_data_type *balongfd)
{
    BUG_ON(balongfd == NULL);

    balongfd->frame_end_wq = create_singlethread_workqueue("frame_end_workqueue");
    if (!balongfd->frame_end_wq) {
        balongfb_loge("failed to create frame end workqueue!\n");
        return -1;
    }

    INIT_WORK(&balongfd->frame_end_work, frame_end_workqueue);

    return 0;
}

STATIC void video_idle_workqueue(struct work_struct *ws)
{
    struct balong_fb_data_type *balongfd = NULL;
    int ret;

    balongfd = container_of(ws, struct balong_fb_data_type, video_idle_work);
    BUG_ON(balongfd == NULL);
    ...
    ret = ade_fb_start_video_idle(balongfd);
    if (ret != 0) {
        balongfb_loge("balongfb enter video idle fail \n");
    }
...
STATIC int init_video_idle_workqueue(struct balong_fb_data_type *balongfd)
{
    BUG_ON(balongfd == NULL);

    balongfd->video_idle_wq = create_singlethread_workqueue("video_idle_workqueue");
    if (!balongfd->video_idle_wq) {
        balongfb_loge("failed to create video lcd idle workqueue \n");
        return -1;
    }

    INIT_WORK(&balongfd->video_idle_work, video_idle_workqueue);
...
STATIC int balong_fb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case FB_FRC_SET:
            ...
            break;
        case FB_SBL_SET_VALUE:
            ret = balongfb_sbl_set_value(info, argp);
            break;
#if PARTIAL_UPDATE
...
STATIC int balong_fb_probe(struct platform_device *pdev)
{
    ...
    /* fb sysfs create */
    balong_fb_sysfs_create(pdev);

    /*add lcd info*/
    lcd_fb_sysfs_create(pdev);
