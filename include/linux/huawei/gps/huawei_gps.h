...
#ifdef CONFIG_BCMGPS
extern int set_gps_ref_clk_enable_bcm433x(bool enable);
#endif
...
static inline int set_gps_ref_clk_enable(bool enable)
{
    ...
    if (isMyConnectivityChip(CHIP_TYPE_HI110X )) {
		...
		#ifdef CONFIG_BCMGPS
        printk(KERN_INFO "cfg ps type is matched with bcm433x, continue\n");
		ret = set_gps_ref_clk_enable_bcm433x(enable);
		#endif
    }
#else 
