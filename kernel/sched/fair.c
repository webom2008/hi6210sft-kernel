...
#ifdef CONFIG_ARCH_HI6XXX
extern unsigned int hmp_slower_domain_min_load;
extern int hmp_min_load_ctrl_flag;

unsigned int hmp_up_threshold = 1020;
unsigned int hmp_down_threshold = 768;
unsigned int hmp_up_minload_threshold = 85;/*85%*/
unsigned int hmp_down_minload_threshold = 20;/*20%*/
...
static int hmp_attr_init(void)
{
...
#endif
#ifdef CONFIG_ARCH_HI6XXX
	hmp_attr_add("hmp_up_minload_threshold",
		&hmp_up_minload_threshold,
		NULL,
		hmp_theshold_from_sysfs,
		NULL,
		0);
	hmp_attr_add("hmp_down_minload_threshold",
		&hmp_down_minload_threshold,
		NULL,
		hmp_theshold_from_sysfs,
		NULL,
		0);
	hmp_attr_add("hmp_up_cpufreq_threshold",
		...
		NULL,
		0);
	hmp_attr_add("hmp_min_load_ctrl_flag",
		&hmp_min_load_ctrl_flag,
		NULL,
		hmp_up_freq_from_sysfs,
		NULL,
		0);
...
#ifdef CONFIG_ARCH_HI6XXX
	sprintf(desc, "~~O,%d,%s", hmp_slower_domain_min_load,p->comm);
	trace_sched_hmp_offload_abort(cpu, 1, desc);

	/*cpu min_load > 20% 205*/
	if((hmp_min_load_ctrl_flag != 0)&&(hmp_slower_domain_min_load > hmp_down_minload_threshold)){
		return NR_CPUS;
	}
#else
...
#if defined(CONFIG_ARCH_HI3XXX) || defined(CONFIG_ARCH_HI6XXX)
	if (p->mm && (sd_flag & SD_BALANCE_FORK) && task_fork_on_bigcluster) {
#else
...
#ifdef CONFIG_ARCH_HI6XXX
	/*cur freq >= 960M*/
	if((hmp_min_load_ctrl_flag != 0)&&(*cur_freq < hmp_up_cpufreq_threshold))
	{
	    return 0;
	}

	/*cpu min_load > 85% 870*/
	if ((hmp_min_load_ctrl_flag != 0)&&(hmp_slower_domain_min_load < hmp_up_minload_threshold)) {
		return 0;
	}
#endif
...
#ifdef CONFIG_ARCH_HI6XXX
	sprintf(desc, "~~Up %d, %d, %s", hmp_slower_domain_min_load, *cur_freq, p->comm);
	trace_sched_hmp_offload_abort(cpu, 1, desc);
...
#ifdef CONFIG_ARCH_HI6XXX
	sprintf(desc, "~~Down %d,%d,%s", hmp_slower_domain_min_load, (int)se->avg.load_avg_ratio, p->comm);
	trace_sched_hmp_offload_abort(cpu, 1, desc);
...
static void hmp_force_up_migration(int this_cpu)
{
	...
	for_each_online_cpu(cpu) {
		...
		p = task_of(curr);
#ifdef CONFIG_ARCH_HI6XXX
	sprintf(desc, "~~Force %d,%d,%s", (int)p->se.avg.load_avg_ratio, hmp_slower_domain_min_load, p->comm);
	trace_sched_hmp_offload_abort(cpu, 1, desc);
#endif
...
static unsigned int hmp_idle_pull(int this_cpu)
{
	...
	if (!p)
		goto done;
#if 0/*def CONFIG_ARCH_HI6XXX*/
	sprintf(desc, "~~pull %d, %d, %s", hmp_slower_domain_min_load, *cur_freq, p->comm);
