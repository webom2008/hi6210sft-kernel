...
static inline unsigned int hi6402_dapm_reg_read(struct snd_soc_codec *codec,
		unsigned int reg)
{
	return hi6402_reg_read(codec, CODEC_BASE_ADDR + reg);
}
...
static inline int hi6402_dapm_reg_write(struct snd_soc_codec *codec,
		unsigned int reg, unsigned int value)
{
	return hi6402_reg_write(codec, CODEC_BASE_ADDR + reg, value);
}
...
static void hi6402_pll_enable(struct snd_soc_codec *codec, bool enable)
{
	struct hi6402_priv *priv = snd_soc_codec_get_drvdata(codec);
	...
	hi6402_irq_pll_enable(priv->p_irq, enable);
	mutex_unlock(&priv->dapm_mutex);
	return;
}

static void hi6402_mad_enable(struct snd_soc_codec *codec, bool enable)
{
	struct hi6402_priv *priv = snd_soc_codec_get_drvdata(codec);
	...
	BUG_ON(NULL == priv->p_irq);
	mutex_lock(&priv->dapm_mutex);
	hi6402_irq_mad_pll_enable(priv->p_irq, enable);
	mutex_unlock(&priv->dapm_mutex);
	return;
}
...
static void hi6402_mad_power_optimize(struct snd_soc_codec *codec, int enable)
{
	if(enable)
	{
		hi6402_dapm_reg_write(codec, HI6402_ANA_REG29, 0x0);		//1.8v power optimize
		hi6402_dapm_reg_write(codec, HI6402_ANA_REG30, 0x0);		//1.8v power optimize
		hi6402_dapm_reg_write(codec, HI6402_ANA_REG31, 0x0);		//1.8v power optimize
		hi6402_dapm_reg_write(codec, HI6402_ANA_REG35, 0x0);		//3.3v power optimize
		hi6402_dapm_reg_write(codec, HI6402_SC_MAD_CTRL0, 0x1);		//auto cg set
		hi6402_dapm_reg_write(codec, HI6402_DSP_CTRL6_DMAC, 0x3D);	//auto cg set
	} else {
		hi6402_dapm_reg_write(codec, HI6402_DSP_CTRL6_DMAC, 0x11);	//auto cg set
		hi6402_dapm_reg_write(codec, HI6402_SC_MAD_CTRL0, 0x3E);	//auto cg set
		hi6402_dapm_reg_write(codec, HI6402_ANA_REG29, 0x2D);		//1.8v power optimize
		hi6402_dapm_reg_write(codec, HI6402_ANA_REG30, 0x2D);		//1.8v power optimize
		hi6402_dapm_reg_write(codec, HI6402_ANA_REG31, 0x2D);		//1.8v power optimize
		hi6402_dapm_reg_write(codec, HI6402_ANA_REG35, 0x2D);		//3.3v power optimize
	}
	return;
}
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		hi6402_mad_power_optimize(codec, true);
		break;
	case SND_SOC_DAPM_POST_PMD:
		hi6402_mad_power_optimize(codec, false);
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		hi6402_pll_enable(codec, true);
		break;
	case SND_SOC_DAPM_POST_PMD:
		hi6402_pll_enable(codec, false);
		break;
	default :
		pr_warn("%s : power mode event err : %d\n", __FUNCTION__, event);
		break;
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* cp CP1 cfg */
		hi6402_cp_clk_select(codec, HI6402_CP_CLK_DIV_8);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* cp CP1 cfg */
		hi6402_cp_clk_select(codec, HI6402_CP_CLK_DIV_256);
		break;
	default :
...
static void hi6402_hp_power_on(struct snd_soc_codec *codec)
{
	...
	hi6402_dapm_reg_set_bit(codec, HI6402_POP_CLK_PD_REG,0 );
	hi6402_dapm_reg_write_bits(codec, HI6402_CP_CLK_SEL_REG, 0x02, 0x07);
	hi6402_dapm_reg_clr_bit(codec, HI6402_POP_CLK_PD_REG,0 );
	mdelay(1);
...
static void hi6402_hp_power_off(struct snd_soc_codec *codec)
{
	...
	/* shutdown cp clk before clk switch*/
	hi6402_dapm_reg_set_bit(codec, HI6402_POP_CLK_PD_REG,0 );
	hi6402_dapm_reg_write_bits(codec, HI6402_CP_CLK_SEL_REG, 0x07, 0x07);
	hi6402_dapm_reg_clr_bit(codec, HI6402_POP_CLK_PD_REG,0 );
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		hi6402_dapm_reg_clr_bit(codec, HI6402_S2_FS_CFG_L, HI6402_OL_SRC_FS_CFG);
		break;
	default :
		pr_warn("%s : event err : %d\n", __FUNCTION__, event);
		break;
...
int hi6402_s1_il_pga_power_mode_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* s1_il_pga_clk en */
		hi6402_dapm_reg_set_bit(codec, HI6402_S1_MODULE_CLK_REG, HI6402_S1_PGA_IL_EN_BIT);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* s1_il_pga_clk disable */
		hi6402_dapm_reg_clr_bit(codec, HI6402_S1_MODULE_CLK_REG, HI6402_S1_PGA_IL_EN_BIT);
		break;
	default :
		pr_warn("%s : event err : %d\n", __FUNCTION__, event);
		break;
...
int hi6402_s1_ir_pga_power_mode_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{	
	struct snd_soc_codec *codec = w->codec;
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* s1_irl_pga_clk en */
		hi6402_dapm_reg_set_bit(codec, HI6402_S1_MODULE_CLK_REG, HI6402_S2_PGA_IR_EN_BIT);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* s1_ir_pga_clk disable */
		hi6402_dapm_reg_clr_bit(codec, HI6402_S1_MODULE_CLK_REG, HI6402_S2_PGA_IR_EN_BIT);
		break;
	default :
		pr_warn("%s : event err : %d\n", __FUNCTION__, event);
		break;
...
int hi6402_s2_il_pga_power_mode_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		hi6402_dapm_reg_clr_bit(codec, HI6402_S2_MODULE_CLK_REG, HI6402_S2_PGA_IL_EN_BIT);
		break;
	default :
		pr_warn("%s : event err : %d\n", __FUNCTION__, event);
		break;
...
int hi6402_s2_ir_pga_power_mode_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		hi6402_dapm_reg_clr_bit(codec, HI6402_S2_MODULE_CLK_REG, HI6402_S2_PGA_IR_EN_BIT);
		break;
	default :
		pr_warn("%s : event err : %d\n", __FUNCTION__, event);
		break;
...
int hi6402_s3_il_pga_power_mode_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* s3_il_pga_clk en */
		hi6402_dapm_reg_set_bit(codec, HI6402_S3_MODULE_CLK_REG, HI6402_S3_PGA_IL_EN_BIT);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* s3_il_pga_clk disable */
		hi6402_dapm_reg_clr_bit(codec, HI6402_S3_MODULE_CLK_REG, HI6402_S3_PGA_IL_EN_BIT);
		break;
	default :
		pr_warn("%s : event err : %d\n", __FUNCTION__, event);
		break;
...
int hi6402_s3_ir_pga_power_mode_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* s3_ir_pga_clk en */
		hi6402_dapm_reg_set_bit(codec, HI6402_S3_MODULE_CLK_REG, HI6402_S3_PGA_IR_EN_BIT);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* s3_ir_pga_clk disable */
		hi6402_dapm_reg_clr_bit(codec, HI6402_S3_MODULE_CLK_REG, HI6402_S3_PGA_IR_EN_BIT);
		break;
	default :
		pr_warn("%s : event err : %d\n", __FUNCTION__, event);
		break;
...
int hi6402_s4_il_pga_power_mode_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* s4_il_pga_clk en */
		hi6402_dapm_reg_set_bit(codec, HI6402_S4_MODULE_CLK_REG, HI6402_S4_PGA_IL_EN_BIT);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* s4_il_pga_clk disable */
		hi6402_dapm_reg_clr_bit(codec, HI6402_S4_MODULE_CLK_REG, HI6402_S4_PGA_IL_EN_BIT);
		break;
	default :
		pr_warn("%s : event err : %d\n", __FUNCTION__, event);
		break;
...
int hi6402_s4_ir_pga_power_mode_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* s4_ir_pga_clk en */
		hi6402_dapm_reg_set_bit(codec, HI6402_S4_MODULE_CLK_REG, HI6402_S4_PGA_IR_EN_BIT);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* s4_ir_pga_clk disable */
		hi6402_dapm_reg_clr_bit(codec, HI6402_S4_MODULE_CLK_REG, HI6402_S4_PGA_IR_EN_BIT);
		break;
	default :
		pr_warn("%s : event err : %d\n", __FUNCTION__, event);
		break;
...
int hi6402_s2_ol_switch_power_mode_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		//hi6402_dapm_reg_set_bit(codec, HI6402_S2_MODULE_CLK_REG, HI6402_S2_PGA_OL_EN_BIT);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* s2 ol pga clk disable */
		//hi6402_dapm_reg_clr_bit(codec, HI6402_S2_MODULE_CLK_REG, HI6402_S2_PGA_OL_EN_BIT);
		break;
	default :
		pr_warn("%s : event err : %d\n", __FUNCTION__, event);
		break;
...
int hi6402_s2_or_switch_power_mode_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		//hi6402_dapm_reg_set_bit(codec, HI6402_S2_MODULE_CLK_REG, HI6402_S2_PGA_OR_EN_BIT);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* s2 or pga clk disable */
		//hi6402_dapm_reg_clr_bit(codec, HI6402_S2_MODULE_CLK_REG, HI6402_S2_PGA_OR_EN_BIT);
		break;
	default :
		pr_warn("%s : event err : %d\n", __FUNCTION__, event);
		break;
...
int hi6402_s4_ol_switch_power_mode_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		//hi6402_dapm_reg_set_bit(codec, HI6402_S4_MODULE_CLK_REG, HI6402_S4_PGA_OL_EN_BIT);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* s4 ol pga clk disable */
		//hi6402_dapm_reg_clr_bit(codec, HI6402_S4_MODULE_CLK_REG, HI6402_S4_PGA_OL_EN_BIT);
		break;
	default :
		pr_warn("%s : event err : %d\n", __FUNCTION__, event);
		break;
...
int hi6402_s4_or_switch_power_mode_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		//hi6402_dapm_reg_set_bit(codec, HI6402_S4_MODULE_CLK_REG, HI6402_S4_PGA_OR_EN_BIT);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* s4 or pga clk disable */
		//hi6402_dapm_reg_clr_bit(codec, HI6402_S4_MODULE_CLK_REG, HI6402_S4_PGA_OR_EN_BIT);
		break;
	default :
		pr_warn("%s : event err : %d\n", __FUNCTION__, event);
		break;
...
int hi6402_dacsl_power_mode_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
...
int hi6402_dacsr_power_mode_event(struct snd_soc_dapm_widget *w,
		struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* hsmic chg */
		hi6402_dapm_reg_clr_bit(codec, HI6402_MIC_DISCHG_REG, HI6402_HS_MIC_DISCHG_BIT);
		/* hsmic on */
		hi6402_dapm_reg_clr_bit(codec, HI6402_ANA_MIC_PD_REG, HI6402_HS_MIC_PD_BIT);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* hsmic pd */
		hi6402_dapm_reg_set_bit(codec, HI6402_ANA_MIC_PD_REG, HI6402_HS_MIC_PD_BIT);
		/* hsmic dischg */
		hi6402_dapm_reg_set_bit(codec, HI6402_MIC_DISCHG_REG, HI6402_HS_MIC_DISCHG_BIT);
		break;
	default :
		pr_warn("%s : event err : %d\n", __FUNCTION__, event);
		break;
...
static const struct snd_kcontrol_new hi6402_snd_controls[] = {
	...
	SOC_SINGLE("DACR MIXER GAIN1",
		HI6402_DACSL_MIXER_GAIN_REG, HI6402_DACSR_MIXER_1_GAIN_BIT, 3, 0),
...
static const struct snd_soc_dapm_widget hi6402_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY("AUDIO CLK",
		HI6402_AUD_CLK_EN, HI6402_AUD_CLK_EN_BIT, 0, NULL, 0),
	...
	SND_SOC_DAPM_SUPPLY("S2 TX CLK",
		HI6402_S2_PORT_CFG_L, HI6402_IF_TX_EN_BIT, 0,
		NULL, 0),
	SND_SOC_DAPM_SUPPLY("S2 RX CLK",
		HI6402_S2_PORT_CFG_L, HI6402_IF_RX_EN_BIT, 0,
		NULL, 0),
	SND_SOC_DAPM_SUPPLY("S2 INTERFACE CLK",
		HI6402_S2_FS_CFG_L, HI6402_PORT_CLK_EN, 0,
		NULL, 0),
	...
	SND_SOC_DAPM_SUPPLY("S4 TX CLK",
		HI6402_S4_PORT_CFG_L, HI6402_IF_TX_EN_BIT, 0,
		NULL, 0),
	SND_SOC_DAPM_SUPPLY("S4 RX CLK",
		HI6402_S4_PORT_CFG_L, HI6402_IF_RX_EN_BIT, 0,
		NULL, 0),
	SND_SOC_DAPM_SUPPLY("S4 INTERFACE CLK",
		HI6402_S4_FS_CFG_L, HI6402_PORT_CLK_EN, 0,
		NULL, 0),
	...
	SND_SOC_DAPM_SUPPLY("CP2",
		HI6402_CP2_PD_REG, HI6402_CP2_PD_BIT, 1,
		NULL, 0),

	/* ANALOG PGA */
	SND_SOC_DAPM_PGA_E("MAINMIC PGA",
		SND_SOC_NOPM, 0, 0,
		NULL, 0, hi6402_mainmic_pga_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("AUXMIC PGA",
		SND_SOC_NOPM, 0, 0,
		NULL, 0, hi6402_auxmic_pga_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("LINEINL PGA",
		SND_SOC_NOPM, 0, 0,
		NULL, 0, hi6402_lineinl_pga_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("LINEINR PGA",
		SND_SOC_NOPM, 0, 0,
		NULL, 0, hi6402_lineinr_pga_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_PGA_E("S1 IL PGA",
		SND_SOC_NOPM, 0, 0,
		NULL, 0, hi6402_s1_il_pga_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("S1 IR PGA",
		SND_SOC_NOPM, 0, 0,
		NULL, 0, hi6402_s1_ir_pga_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("S2 IL PGA",
		SND_SOC_NOPM, 0, 0,
		NULL, 0, hi6402_s2_il_pga_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("S2 IR PGA",
		SND_SOC_NOPM, 0, 0,
		NULL, 0, hi6402_s2_ir_pga_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("S3 IL PGA",
		SND_SOC_NOPM, 0, 0,
		NULL, 0, hi6402_s3_il_pga_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("S3 IR PGA",
		SND_SOC_NOPM, 0, 0,
		NULL, 0, hi6402_s3_ir_pga_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("S4 IL PGA",
		SND_SOC_NOPM, 0, 0,
		NULL, 0, hi6402_s4_il_pga_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("S4 IR PGA",
		SND_SOC_NOPM, 0, 0,
		NULL, 0, hi6402_s4_ir_pga_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	
	/* PGA FOR DAC */
	SND_SOC_DAPM_PGA_E("DACL PGA",
		SND_SOC_NOPM, 0, 0,
		NULL, 0, hi6402_dacl_pga_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_E("DACR PGA",
		SND_SOC_NOPM, 0, 0,
		NULL, 0, hi6402_dacr_pga_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	
	/* PGA FOR ADC */
	SND_SOC_DAPM_PGA("ADC1R",
		HI6402_DAC_ADC_CLK_REG, HI6402_ADC1R_EN_BIT, 0,
		NULL, 0),
	SND_SOC_DAPM_PGA("ADC1L",
		HI6402_DAC_ADC_CLK_REG, HI6402_ADC1L_EN_BIT, 0,
		NULL, 0),
	SND_SOC_DAPM_PGA("ADC0R",
		HI6402_DAC_ADC_CLK_REG, HI6402_ADC0R_EN_BIT, 0,
		NULL, 0),
	SND_SOC_DAPM_PGA("ADC0L",
		HI6402_DAC_ADC_CLK_REG, HI6402_ADC0L_EN_BIT, 0,
		NULL, 0),

	/* PGA FOR SIDETONE PGA */
	SND_SOC_DAPM_PGA_E("SIDE PGA",
		SND_SOC_NOPM, 0, 0,
		NULL, 0, hi6402_side_pga_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_SWITCH_E("DACSL SWITCH",
		SND_SOC_NOPM, 0, 0,
		&hi6402_dapm_dacsl_switch_controls,
		hi6402_dacsl_power_mode_event,
		SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_SWITCH_E("DACSR SWITCH",
		SND_SOC_NOPM, 0, 0,
		&hi6402_dapm_dacsr_switch_controls,
		hi6402_dacsr_power_mode_event,
		SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_SWITCH_E("S2 OL SWITCH",
		...
		hi6402_s2_ol_switch_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_SWITCH_E("S2 OR SWITCH",
		SND_SOC_NOPM, 0, 0,
		&hi6402_dapm_s2_or_switch_controls,
		hi6402_s2_or_switch_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_SWITCH_E("S4 OL SWITCH",
		...
		hi6402_s4_ol_switch_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_SWITCH_E("S4 OR SWITCH",
		SND_SOC_NOPM, 0, 0,
		&hi6402_dapm_s4_or_switch_controls,
		hi6402_s4_or_switch_power_mode_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_MUX("DACSL MUX",
		SND_SOC_NOPM, 0, 0,
		&hi6402_dapm_dacsl_mux_controls),	
	SND_SOC_DAPM_MUX("DACSR MUX",
		SND_SOC_NOPM, 0, 0,
		&hi6402_dapm_dacsr_mux_controls),	
				...
				struct snd_soc_dai *dai)
{
	...
	case 8000:
		...
		hi6402_dapm_reg_write_bits(codec, HI6402_S3_SRC_IN_MODE_CGF, 0x0A, 0x0E);
		hi6402_dapm_reg_write_bits(codec, HI6402_S3_SRC_OUT_MODE_CGF, 0x04, 0x06);
		break;
	case 16000:
		hi6402_dapm_reg_write_bits(codec, HI6402_S3_FS_CFG_L, 0x01, 0x07);
		hi6402_dapm_reg_set_bit(codec, HI6402_S3_IN_SRC_SEL, HI6402_S3_IL_SRC_BIT);
		hi6402_dapm_reg_set_bit(codec, HI6402_S3_OUT_SRC_SEL, HI6402_S3_OL_SRC_BIT);
		hi6402_dapm_reg_write_bits(codec, HI6402_S3_SRC_IN_MODE_CGF, 0x02, 0x0E);
		hi6402_dapm_reg_write_bits(codec, HI6402_S3_SRC_OUT_MODE_CGF, 0x00, 0x06);
		break;
	case 32000:
...
static void hi6402_init_chip(struct snd_soc_codec *codec)
{
	...
	hi6402_dapm_reg_write(codec, HI6402_S1_PORT_CFG_H, 0x40);
	...
	/* pga fade in en */
	hi6402_dapm_reg_set_bit(codec, HI6402_S1_PGA_IL_MODE_CFG_REG1, 7);
	hi6402_dapm_reg_set_bit(codec, HI6402_S1_PGA_IR_MODE_CFG_REG1, 7);
	hi6402_dapm_reg_set_bit(codec, HI6402_S1_PGA_OL_MODE_CFG_REG1, 7);
	hi6402_dapm_reg_set_bit(codec, HI6402_S1_PGA_OR_MODE_CFG_REG1, 7);

	hi6402_dapm_reg_set_bit(codec, HI6402_S2_PGA_IL_MODE_CFG_REG1, 7);
	hi6402_dapm_reg_set_bit(codec, HI6402_S2_PGA_IR_MODE_CFG_REG1, 7);
	hi6402_dapm_reg_set_bit(codec, HI6402_S2_PGA_OL_MODE_CFG_REG1, 7);
	hi6402_dapm_reg_set_bit(codec, HI6402_S2_PGA_OR_MODE_CFG_REG1, 7);

	hi6402_dapm_reg_set_bit(codec, HI6402_S3_PGA_IL_MODE_CFG_REG1, 7);
	hi6402_dapm_reg_set_bit(codec, HI6402_S3_PGA_IR_MODE_CFG_REG1, 7);
	hi6402_dapm_reg_set_bit(codec, HI6402_S3_PGA_OL_MODE_CFG_REG1, 7);
	hi6402_dapm_reg_set_bit(codec, HI6402_S3_PGA_OR_MODE_CFG_REG1, 7);

	hi6402_dapm_reg_set_bit(codec, HI6402_S4_PGA_IL_MODE_CFG_REG1, 7);
	hi6402_dapm_reg_set_bit(codec, HI6402_S4_PGA_IR_MODE_CFG_REG1, 7);
	hi6402_dapm_reg_set_bit(codec, HI6402_S4_PGA_OL_MODE_CFG_REG1, 7);
	hi6402_dapm_reg_set_bit(codec, HI6402_S4_PGA_OR_MODE_CFG_REG1, 7);

	hi6402_dapm_reg_set_bit(codec, HI6402_DACL_PGA_MODE_CFG_REG1, 7);
	hi6402_dapm_reg_set_bit(codec, HI6402_DACR_PGA_MODE_CFG_REG1, 7);

	hi6402_dapm_reg_set_bit(codec, HI6402_SIDE_PGA_MODE_CFG_REG1, 7);
	...
	/* mixer fade in out */
	hi6402_dapm_reg_write_bits(codec, HI6402_DACSL_MIXER_SEL, 0x09, 0x1F);
	hi6402_dapm_reg_write_bits(codec, HI6402_DACSL_MIXER_FD_REG, 0x49, 0x5F);

	hi6402_dapm_reg_write_bits(codec, HI6402_DACSR_MIXER_SEL, 0x09, 0x1F);
	hi6402_dapm_reg_write_bits(codec, HI6402_DACSR_MIXER_FD_REG, 0x49, 0x5F);

	hi6402_dapm_reg_write_bits(codec, HI6402_SHARE_MIXER_FD_REG_0, 0x09, 0x1F);
	hi6402_dapm_reg_write_bits(codec, HI6402_SHARE_MIXER_FD_REG_1, 0x49, 0x5F);

	hi6402_dapm_reg_write_bits(codec, HI6402_DACL_MIXER_FD_REG_0, 0x09, 0x1F);
	hi6402_dapm_reg_write_bits(codec, HI6402_DACL_MIXER_FD_REG_1, 0x49, 0x5F);

	hi6402_dapm_reg_write_bits(codec, HI6402_DACR_MIXER_FD_REG_0, 0x09, 0x1F);
	hi6402_dapm_reg_write_bits(codec, HI6402_DACR_MIXER_FD_REG_1, 0x49, 0x5F);
	...
	/* MUX OF ANADAC OUT */
	hi6402_dapm_reg_write(codec, HI6402_DAC_HP_MUX_SEL, 0x24);
	...
	/* CP1 always on */
	hi6402_dapm_reg_set_bit(codec, HI6402_PLL_CLK_CFG_REG, 0);
	hi6402_dapm_reg_write_bits(codec, HI6402_POP_CLK_PD_REG, 0x00, 0x03);
	hi6402_dapm_reg_clr_bit(codec, HI6402_CP_PD_REG, HI6402_CP_PD_BIT);

	/* i2v cfg */
	hi6402_dapm_reg_write_bits(codec, HI6402_DAC_HPL_I2V_CFG_REG, 0x0F, 0x3F);
	hi6402_dapm_reg_write_bits(codec, HI6402_DAC_HPR_I2V_CFG_REG, 0x0F, 0x3F);

	/* pop cfg */
	hi6402_dapm_reg_write_bits(codec, HI6402_HPL_POP_CFG1_REG, 0x01F, 0x3F);
	hi6402_dapm_reg_write(codec, HI6402_HPL_POP_CFG2_REG, 0x82);
	...
	/* MAD PATH INIT */
	hi6402_dapm_reg_write(codec, HI6402_SC_DSP_CTRL0, 0x02);
	//hi6402_dapm_reg_write(codec, HI6402_SC_MAD_CTRL0, 0x20);
}
...
static void hi6402_codec_shutdown(struct platform_device *pdev)
{
	...
	if (check_headset_pluged_in(codec)){
		pr_info("hi6402_codec_shutdown with headset\n");
	} else {
		pr_info("hi6402_codec_shutdown without headset\n");
	}
#endif
}
