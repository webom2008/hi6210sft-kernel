                                        ...
                                        struct snd_pcm_substream *substream )
{
    ...
    if (NULL == substream)
    {
        loge("hi6210_notify_pcm_trigger failed substream = NULL");
        return -EINVAL;
    }
    if (NULL == substream->runtime)
    {
        loge("hi6210_notify_pcm_trigger failed substream->runtime = NULL");
        return -EINVAL;
    }
    prtd = (struct hi6210_runtime_data *)substream->runtime->private_data;
    if (NULL == prtd)
...
STATIC snd_pcm_uframes_t hi6210_pcm_hifi_pointer(struct snd_pcm_substream *substream)
{
    ...
    if (NULL == substream)
    {
        loge("hi6210_pcm_hifi_pointer failed substream = NULL");
        return -EINVAL;
    }

    if (NULL == substream->runtime)
    {
        loge("hi6210_pcm_hifi_pointer failed substream->runtime = NULL");
        return -EINVAL;
    }
...
static int hi6210_pcm_hw_free(struct snd_pcm_substream *substream)
{
    ...
    if (NULL == substream)
    {
        loge("hi6210_pcm_hw_free failed substream = NULL");
        return -EINVAL;
    }

    if (NULL == substream->runtime)
    {
        loge("hi6210_pcm_hw_free failed substream->runtime = NULL");
        return -EINVAL;
    }
                    ...
                    size_t count, loff_t *data)
{
    ...
    if (NULL == buffer)
    {
        loge("input error: buffer is NULL\n");
        return -EINVAL;
    }
